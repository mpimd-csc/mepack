...
#include "proto.h"
#include <string.h>
typedef void (*trsylv_function) (const char *TRANSA, const char*TRANSB, float *sgn,  Int *M, Int *N, float * A, Int *LDA,float * B, Int * LDB, float *X, Int *LDX, float * SCALE, float *WORK, Int *INFO, FORTRAN_LEN_T l1, FORTRAN_LEN_T l2);
...
int main(int argc, char **argv)
{
    ...
    struct function_name trsylv_functions[] = {
        { &(FC_GLOBAL_(sla_trsylv2_l3,SLA_TRSYLV2_L3)), "SLA_TRSYLV2_L3", 1, 0,0 },
        { &(FC_GLOBAL_(sla_trsylv2_l3_unopt,SLA_TRSYLV2_L3_UNOPT)), "SLA_TRSYLV2_L3_UNOPT", 1, 0,0  },
        { &(FC_GLOBAL_(sla_trsylv2_l2,SLA_TRSYLV2_L2)), "SLA_TRSYLV2_L2", 0, 0 ,0 },
        { &(FC_GLOBAL_(sla_trsylv2_l2_reorder,SLA_TRSYLV2_L2_REORDER)), "SLA_TRSYLV2_L2_REORDER", 0, 0,0  },
        { &(FC_GLOBAL_(sla_trsylv2_l2_local_copy,SLA_TRSYLV2_L2_LOCAL_COPY)), "SLA_TRSYLV2_L2_LOCAL_COPY" ,0 ,0,0  },
        { &(FC_GLOBAL_(sla_trsylv2_l2_local_copy_32,SLA_TRSYLV2_L2_LOCAL_COPY_32)), "SLA_TRSYLV2_L2_LOCAL_COPY_32", 0,0 , 1},
        { &(FC_GLOBAL_(sla_trsylv2_l2_local_copy_64,SLA_TRSYLV2_L2_LOCAL_COPY_64)), "SLA_TRSYLV2_L2_LOCAL_COPY_64", 0,0 , 1},
        { &(FC_GLOBAL_(sla_trsylv2_l2_local_copy_96,SLA_TRSYLV2_L2_LOCAL_COPY_96)), "SLA_TRSYLV2_L2_LOCAL_COPY_96", 0,0 , 1},
        { &(FC_GLOBAL_(sla_trsylv2_l2_local_copy_128,SLA_TRSYLV2_L2_LOCAL_COPY_128)), "SLA_TRSYLV2_L2_LOCAL_COPY_128", 0,0,1 },
        { &(FC_GLOBAL_(sla_trsylv2_l2_unopt,SLA_TRSYLV2_L2_UNOPT)), "SLA_TRSYLV2_L2_UNOPT", 0,0, 0 },
        { &(FC_GLOBAL_(sla_trsylv2_dag,SLA_TRSYLV2_DAG)), "SLA_TRSYLV2_DAG", 1, 0, 0 },
        { &(FC_GLOBAL_(sla_trsylv2_l3_2s,SLA_TRSYLV2_L3_2S)), "SLA_TRSYLV2_L3_2S",1 , 1 ,0},
        { &(FC_GLOBAL_(sla_trsylv2_recursive,SLA_TRSYLV2_RECURSIVE)), "SLA_TRSYLV2_RECURSIVE", 0, 0 , 0},
        { NULL, NULL, 0,0, 0}
    };
    ...
    while ( trsylv_functions[i].fn != NULL ) {
        ...
        if ( trsylv_functions[i].chk_l3 ) {
            mepack_single_trsylv2_blocksize_mb_set(1);
            TRSYLV_CALL(30, "N", "N", &sign, &M, &N, NULL, &LDA, NULL, &LDB, NULL, &LDX, &SCALE, &WORK, &INFO);
            mepack_single_trsylv2_blocksize_mb_set(2);

            mepack_single_trsylv2_blocksize_nb_set(1);
            TRSYLV_CALL(31, "N", "N", &sign, &M, &N, NULL, &LDA, NULL, &LDB, NULL, &LDX, &SCALE, &WORK, &INFO);
            mepack_single_trsylv2_blocksize_nb_set(2);
        }
        if ( trsylv_functions[i].chk_l3_2s ) {
            mepack_single_trsylv2_blocksize_mb_set(4);
            mepack_single_trsylv2_blocksize_nb_set(4);
            mepack_single_trsylv2_blocksize_2stage_set(2);
            TRSYLV_CALL(33, "N", "N", &sign, &M, &N, NULL, &LDA, NULL, &LDB, NULL, &LDX, &SCALE, &WORK, &INFO);
            mepack_single_trsylv2_blocksize_2stage_set(512);
        }
