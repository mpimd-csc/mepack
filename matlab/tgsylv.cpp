...
struct tg_solver {
    ...
    virtual void solve(const char *opA, const char * opB, T sign, int M, int N,       T * Aptr, int lda, T* Bptr, int ldb, T* Cptr, int ldc, T* Dptr, int ldd, T *Yptr, int ldy, T * scale, T * buffer, int *info) = 0;
    virtual void solve_openmp(const char *opA, const char *opB, T sign, int M, int N, T * Aptr, int lda, T* Bptr, int ldb, T* Cptr, int ldc, T* Dptr, int ldd, T *Yptr, int ldy, T * scale, T * buffer, int *info) = 0;
...
template<>
struct tg_solver<double> {
    constexpr static const char *fn = "DLA_TGSYLV_L3";
    constexpr static const char *fn_openmp = "DLA_TGSYLV_DAG";

    static void solve(const char *opA, const char *opB, double sign, int M, int N, double * Aptr, int lda, double *Bptr, int ldb, double *Cptr, int ldc, double *Dptr, int ldd,  double *Yptr, int ldy, double * scale, double * buffer, int *info) {
        mepack_double_tgsylv_level3(opA, opB, sign, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Yptr, ldy, scale, buffer, info);
    };

    static void solve_openmp(const char *opA, const char *opB, double sign, int M, int N, double * Aptr, int lda, double *Bptr, int ldb, double *Cptr, int ldc, double *Dptr, int ldd, double *Yptr, int ldy, double * scale, double * buffer, int *info) {
        mepack_double_tgsylv_dag(opA, opB, sign, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Yptr, ldy, scale, buffer, info);
    };
...
template<>
struct tg_solver<float> {
    constexpr static const char *fn = "SLA_TGSYLV_L3";
    constexpr static const char *fn_openmp = "SLA_TGSYLV_DAG";

    static void solve(const char *opA, const char *opB, float sign, int M, int N, float * Aptr, int lda, float *Bptr, int ldb, float *Cptr, int ldc, float *Dptr, int ldd, float *Yptr, int ldy, float * scale, float * buffer, int *info) {
        mepack_single_tgsylv_level3(opA, opB, sign, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Yptr, ldy, scale, buffer, info);
    };

    static void solve_openmp(const char *opA, const char *opB, float sign, int M, int N, float * Aptr, int lda, float *Bptr, int ldb, float *Cptr, int ldc, float *Dptr, int ldd, float *Yptr, int ldy, float * scale, float * buffer, int *info) {
        mepack_single_tgsylv_dag(opA, opB, sign, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Yptr, ldy, scale, buffer, info);
    };
...
call_solver_op_optset(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D, ArrayType<T> Y,
                      ...
                      MepackOptions optset = MepackOptions())
{
    ...
    if ( optset.getMB() >= 0 ) {
        tg_solver<T>::blocksize_mb_set(optset.getMB());
    }
    if ( optset.getNB() >= 0 ) {
        tg_solver<T>::blocksize_nb_set(optset.getNB());
    }

    if ( optset.getISolver() >= 0 ) {
        tg_solver<T>::isolver_set(optset.getISolver());
    }
    ssize_t auxmem;
    if ( optset.getOpenMP()) {
        auxmem = mepack_memory(tg_solver<T>::fn_openmp, A.rows, B.rows);
    } else {
        auxmem = mepack_memory(tg_solver<T>::fn, A.rows, B.rows);
    }
    ...
    if ( optset.getOpenMP()) {
        tg_solver<T>::solve_openmp(opA.c_str(), opB.c_str(), sign, A.rows, B.rows, A.ptr(), A.ld, B.ptr(), B.ld, C.ptr(), C.ld, D.ptr(), D.ld, Y.ptr(), Y.ld, &scale, auxbuf, &info);
    } else {
        tg_solver<T>::solve(opA.c_str(), opB.c_str(), sign, A.rows, B.rows, A.ptr(), A.ld, B.ptr(), B.ld, C.ptr(), C.ld, D.ptr(), D.ld, Y.ptr(), Y.ld, &scale, auxbuf, &info);
    }
    if ( info != 0) {
        mexoct_error("MEPACK:TGSYLV_LEVEL3", "TGSYLV_LEVEL3 failed with info = %d", (int) info);
    }
...
template<typename T>
static std::tuple<ArrayType<T>>
call_solver(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D, ArrayType<T> Y)
{
    return call_solver_op_optset(A, B, C, D, Y);
}

template<typename T>
static std::tuple<ArrayType<T>>
call_solver_op(ArrayType<T>  A,ArrayType<T> B,  ArrayType<T> C, ArrayType<T> D, ArrayType<T> Y, std::string opA, std::string opB)
{
    return call_solver_op_optset(A, B, C, D, Y, opA, opB);
}

template<typename T>
static std::tuple<ArrayType<T>>
call_solver_optset(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D, ArrayType<T> Y, MepackOptions optset)
{
// #define DEBUG_ALL
#ifdef DEBUG_ALL
...
#endif
    return call_solver_op_optset(A, B, C, D, Y, "N", "N",optset);
}



MEXOCT_ENTRY(mepack_tgsylv,
R"===(
...
)===")
{
    ...
    auto trsylv_double            = makeFunctionExt<1>(call_solver<double>, "X",           ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleY);
    auto trsylv_double_optset     = makeFunctionExt<1>(call_solver_optset<double>, "X",    ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleY, ParamOptSet);
    auto trsylv_double_op         = makeFunctionExt<1>(call_solver_op<double>, "X",        ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleY, ParamOpAString, ParamOpBString);
    auto trsylv_double_op_optset  = makeFunctionExt<1>(call_solver_op_optset<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleY, ParamOpAString, ParamOpBString,ParamOptSet);



    auto trsylv_float            = makeFunctionExt<1>(call_solver<float>, "Xs",             ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatY);
    auto trsylv_float_optset     = makeFunctionExt<1>(call_solver_optset<float>, "Xs",      ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatY, ParamOptSet);
    auto trsylv_float_op         = makeFunctionExt<1>(call_solver_op<float>, "Xs",          ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatY, ParamOpAString, ParamOpBString);
    auto trsylv_float_op_optset  = makeFunctionExt<1>(call_solver_op_optset<float>, "Xs",   ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatY, ParamOpAString, ParamOpBString, ParamOptSet);


	auto parser = makeArgParser("mepack_tgsylv", trsylv_double,  trsylv_double_optset, trsylv_double_op, trsylv_double_op_optset,
