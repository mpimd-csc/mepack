...
template<>
struct cgsylv_solver<double> {
    constexpr static const char *fn = "DLA_GGCSYLV";
    ...
    static void solve(const char * facta, const char *factb, const char *opA, const char * opB, double sgn1, double sgn2, int M, int N ,
        ...
        mepack_double_ggcsylv(facta, factb, opA, opB, sgn1, sgn2, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Qptr, ldq, Zptr, ldz, QBptr, ldqb, ZBptr, ldzb,
...
template<>
struct cgsylv_solver<float> {
    constexpr static const char *fn = "SLA_GGCSYLV";
    ...
    static void solve(const char * facta, const char *factb, const char *opA, const char * opB, float sgn1, float sgn2, int M, int N ,
        ...
        mepack_single_ggcsylv(facta, factb, opA, opB, sgn1, sgn2, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Qptr, ldq, Zptr, ldz, QBptr, ldqb, ZBptr, ldzb,
                Xptr, ldx, Yptr, ldy, scale, buffer, ldwork, info);
    };
    static void blocksize_mb_set(int nb) {
        ...
        MepackOptions optset = MepackOptions(), bool factorize = false)
{
    ...
    if ( optset.getMB() >= 0 ) {
        cgsylv_solver<T>::blocksize_mb_set(optset.getMB());
    }
    if ( optset.getNB() >= 0 ) {
        cgsylv_solver<T>::blocksize_nb_set(optset.getNB());
    }

    if ( optset.getISolver() >= 0 ) {
        cgsylv_solver<T>::isolver_set(optset.getISolver());
    }

    if ( optset.getFSolver() >= 0 ) {
        cgsylv_solver<T>::fsolver_set(optset.getFSolver());
    }


    if ( optset.getBigNB() >= 0 ) {
        cgsylv_solver<T>::blocksize_big_set(optset.getBigNB());
    }
    ...
    if ( factorize ) {
        auxmem = mepack_memory_frontend(cgsylv_solver<T>::fn, (const char *){"N"}, (const char *){"N"}, A.rows, B.rows);
    } else {
        auxmem = mepack_memory_frontend(cgsylv_solver<T>::fn, (const char *){"F"}, (const char *){"F"}, A.rows, B.rows);
    }
    ...
    if ( factorize ) {
        cgsylv_solver<T>::solve("N", "N", opA.c_str(), opB.c_str(), sign1, sign2, A.rows, B.rows, A.ptr(), A.ld, B.ptr(), B.ld, C.ptr(), C.ld, D.ptr(), D.ld,
                QA.ptr(), QA.ld, ZA.ptr(), ZA.ld, QB.ptr(), QB.ld, ZB.ptr(), ZB.ld, X.ptr(), X.ld, Y.ptr(), Y.ld, &scale, auxbuf, auxmem, &info);
    } else {
        cgsylv_solver<T>::solve("F", "F", opA.c_str(), opB.c_str(), sign1, sign2, A.rows, B.rows, A.ptr(), A.ld, B.ptr(), B.ld, C.ptr(), C.ld, D.ptr(), D.ld,
                QA.ptr(), QA.ld, ZA.ptr(), ZA.ld, QB.ptr(), QB.ld, ZB.ptr(), ZB.ld, X.ptr(), X.ld, Y.ptr(), Y.ld, &scale, auxbuf, auxmem, &info);
    }
...
call_ggcsylv_op_optset(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
...
template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>>
call_ggcsylv(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D, ArrayType<T> X, ArrayType<T> Y)
{
    return call_ggcsylv_op_optset(A, B, C, D, X, Y);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>>
call_ggcsylv_op(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D, ArrayType<T> X, ArrayType<T> Y, std::string opA, std::string opB)
{
    return call_ggcsylv_op_optset(A, B, C, D, X, Y, opA, opB);
}

template<typename T>
std::tuple<ArrayType<T>,ArrayType<T>>
call_ggcsylv_optset(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D, ArrayType<T> X, ArrayType<T> Y, MepackOptions optset)
{
   return call_ggcsylv_op_optset(A, B, C, D, X, Y, "N", "N", optset);
}
...
call_10ret_ggcsylv_op_optset(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D, ArrayType<T> X, ArrayType<T> Y, std::string opA = std::string("N"), std::string opB = std::string("N"),
...
template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>>
call_10ret_ggcsylv(ArrayType<T>  A, ArrayType<T> B,ArrayType<T> C, ArrayType<T> D,  ArrayType<T> X, ArrayType<T> Y)
{

    return call_10ret_ggcsylv_op_optset(A, B, C, D, X, Y);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>>
call_10ret_ggcsylv_op(ArrayType<T>  A, ArrayType<T> B,ArrayType<T> C, ArrayType<T> D,  ArrayType<T> X, ArrayType<T> Y, std::string opA, std::string opB)
{
    return call_10ret_ggcsylv_op_optset(A, B, C, D, X, Y, opA, opB);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>>
call_10ret_ggcsylv_optset(ArrayType<T>  A, ArrayType<T> B,ArrayType<T> C, ArrayType<T> D,  ArrayType<T> X, ArrayType<T> Y, MepackOptions optset)
{
   return call_10ret_ggcsylv_op_optset(A, B, C, D, X, Y, "N", "N", optset);
}

template <typename T>
std::tuple<ArrayType<T>, ArrayType<T>>
call_ggcsylv_prefact_op_optset(ArrayType<T>  As, ArrayType<T> Bs, ArrayType<T> Cs, ArrayType<T> Ds,
...
template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>>
call_ggcsylv_prefact(ArrayType<T> As, ArrayType<T> Bs, ArrayType<T> Cs, ArrayType<T> Ds, ArrayType<T> QA, ArrayType<T> ZA, ArrayType<T> QB, ArrayType<T> ZB, ArrayType<T> X, ArrayType<T> Y)
{
    return call_ggcsylv_prefact_op_optset(As, Bs, Cs, Ds, QA, ZA, QB, ZB, X, Y);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>>
call_ggcsylv_prefact_op(ArrayType<T> As, ArrayType<T> Bs, ArrayType<T> Cs, ArrayType<T> Ds, ArrayType<T> QA, ArrayType<T> ZA, ArrayType<T> QB, ArrayType<T> ZB,
        ArrayType<T> X, ArrayType<T> Y, std::string opA, std::string opB)
{
    return call_ggcsylv_prefact_op_optset(As, Bs, Cs, Ds, QA, ZA, QB, ZB, X, Y, opA, opB);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>>
call_ggcsylv_prefact_optset(ArrayType<T> As, ArrayType<T> Bs, ArrayType<T> Cs, ArrayType<T> Ds, ArrayType<T> QA, ArrayType<T> ZA, ArrayType<T> QB, ArrayType<T> ZB,
        ArrayType<T> X, ArrayType<T> Y, MepackOptions optset)
{
   return call_ggcsylv_prefact_op_optset(As, Bs, Cs, Ds, QA, ZA, QB, ZB, X, Y, "N", "N", optset);
}



MEXOCT_ENTRY(mepack_csylv,
R"===(
[R, L] = mepack_csylv(A, B, C, D, E, F)
[R, L] = mepack_csylv(A, B, C, D, E, F, OpA, OpB)
[R, L] = mepack_csylv(A, B, C, D, E, F, OptSet)
[R, L] = mepack_csylv(A, B, C, D, E, F, OpA, OpB, OptSet)
 Input:
...
[R, L, AS, BS, CS, DS, QA, ZA, QB, ZB] = mepack_csylv(A, B, C, D, E, F)
[R, L, AS, BS, CS, DS, QA, ZA, QB, ZB] = mepack_csylv(A, B, C, D, E, F, OpA, OpB)
[R, L, AS, BS, CS, DS, QA, ZA, QB, ZB] = mepack_csylv(A, B, C, D, E, F, OptSet)
[R, L, AS, BS, CS, DS, QA, ZA, QB, ZB] = mepack_csylv(A, B, C, D, E, F, OpA, OpB, OptSet)
  Input:
    A      -- first coefficient matrix
    B      -- second coefficient matrix
    C      -- third coefficient matrix
...
[R, L] = mepack_csylv(AS, BS, CS, DS, QA, ZA, QB, ZB, E, F)
[R, L] = mepack_csylv(AS, BS, CS, DS, QA, ZA, QB, ZB, E, F, OpA, OpB)
[R, L] = mepack_csylv(AS, BS, CS, DS, QA, ZA, QB, ZB, E, F, OptSet)
[R, L] = mepack_csylv(AS, BS, CS, DS, QA, ZA, QB, ZB, E, F, OpA, OpB, OptSet)
  Input:
...
The mepack_csylv function solves the coupled Sylvester equation
...
The function uses the level-3 solver D/SLA_GGCSYLV of MEPACK.
...
)===")
{
    ...
    auto cgsylv_double                    = makeFunctionExt<2>(call_ggcsylv<double>, "X",  ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleE, ParamDoubleF);
    auto cgsylv_double_optset             = makeFunctionExt<2>(call_ggcsylv_optset<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleE, ParamDoubleF, ParamOptSet);
    auto cgsylv_double_op                 = makeFunctionExt<2>(call_ggcsylv_op<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleE, ParamDoubleF, ParamOpAString, ParamOpBString);
    auto cgsylv_double_op_optset          = makeFunctionExt<2>(call_ggcsylv_op_optset<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleE, ParamDoubleF, ParamOpAString, ParamOpBString, ParamOptSet);
    auto cgsylv_10ret_double               = makeFunctionExt<10>(call_10ret_ggcsylv<double>, return_str, ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleE, ParamDoubleF);
    auto cgsylv_10ret_double_optset        = makeFunctionExt<10>(call_10ret_ggcsylv_optset<double>, return_str, ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleE, ParamDoubleF, ParamOptSet);
    auto cgsylv_10ret_double_op            = makeFunctionExt<10>(call_10ret_ggcsylv_op<double>, return_str , ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleE, ParamDoubleF, ParamOpAString, ParamOpBString);
    auto cgsylv_10ret_double_op_optset     = makeFunctionExt<10>(call_10ret_ggcsylv_op_optset<double>, return_str, ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD,
                                                                                                     ParamDoubleE, ParamDoubleF, ParamOpAString,
                                                                                                                   ParamOpBString, ParamOptSet);
    auto cgsylv_prefact_double            = makeFunctionExt<2>(call_ggcsylv_prefact<double>, "X",           ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS,
                                                                                                         ParamDoubleQA, ParamDoubleZA, ParamDoubleQB, ParamDoubleZB,
                                                                                                         ParamDoubleE, ParamDoubleF);
    auto cgsylv_prefact_double_optset     = makeFunctionExt<2>(call_ggcsylv_prefact_optset<double>, "X",    ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS,
                                                                                                         ParamDoubleQA, ParamDoubleZA, ParamDoubleQB, ParamDoubleZB,
                                                                                                         ParamDoubleE, ParamDoubleF, ParamOptSet);
    auto cgsylv_prefact_double_op         = makeFunctionExt<2>(call_ggcsylv_prefact_op<double>, "X",        ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS,
                                                                                                         ParamDoubleQA, ParamDoubleZA, ParamDoubleQB, ParamDoubleZB,
                                                                                                         ParamDoubleE, ParamDoubleF, ParamOpAString, ParamOpBString);
    auto cgsylv_prefact_double_op_optset  = makeFunctionExt<2>(call_ggcsylv_prefact_op_optset<double>, "X", ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS,
                                                                                                         ParamDoubleQA, ParamDoubleZA, ParamDoubleQB, ParamDoubleZB,
                                                                                                         ParamDoubleE, ParamDoubleF, ParamOpAString, ParamOpBString, ParamOptSet);

    auto cgsylv_float                    = makeFunctionExt<2>(call_ggcsylv<float>, "X",  ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatE, ParamFloatF);
    auto cgsylv_float_optset             = makeFunctionExt<2>(call_ggcsylv_optset<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatE, ParamFloatF, ParamOptSet);
    auto cgsylv_float_op                 = makeFunctionExt<2>(call_ggcsylv_op<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatE, ParamFloatF, ParamOpAString, ParamOpBString);
    auto cgsylv_float_op_optset          = makeFunctionExt<2>(call_ggcsylv_op_optset<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatE, ParamFloatF, ParamOpAString, ParamOpBString, ParamOptSet);
    auto cgsylv_10ret_float               = makeFunctionExt<10>(call_10ret_ggcsylv<float>, return_str, ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatE, ParamFloatF);
    auto cgsylv_10ret_float_optset        = makeFunctionExt<10>(call_10ret_ggcsylv_optset<float>, return_str, ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatE, ParamFloatF, ParamOptSet);
    auto cgsylv_10ret_float_op            = makeFunctionExt<10>(call_10ret_ggcsylv_op<float>, return_str , ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatE, ParamFloatF, ParamOpAString, ParamOpBString);
    auto cgsylv_10ret_float_op_optset     = makeFunctionExt<10>(call_10ret_ggcsylv_op_optset<float>, return_str, ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatE, ParamFloatF, ParamOpAString,
                                                                                                                   ParamOpBString, ParamOptSet);
    auto cgsylv_prefact_float            = makeFunctionExt<2>(call_ggcsylv_prefact<float>, "X",           ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS,
                                                                                                         ParamFloatQA, ParamFloatZA, ParamFloatQB, ParamFloatZB,
                                                                                                         ParamFloatE, ParamFloatF);
    auto cgsylv_prefact_float_optset     = makeFunctionExt<2>(call_ggcsylv_prefact_optset<float>, "X",    ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS,
                                                                                                         ParamFloatQA, ParamFloatZA, ParamFloatQB, ParamFloatZB,
                                                                                                         ParamFloatE, ParamFloatF, ParamOptSet);
    auto cgsylv_prefact_float_op         = makeFunctionExt<2>(call_ggcsylv_prefact_op<float>, "X",        ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS,
                                                                                                         ParamFloatQA, ParamFloatZA, ParamFloatQB, ParamFloatZB,
                                                                                                         ParamFloatE, ParamFloatF, ParamOpAString, ParamOpBString);
    auto cgsylv_prefact_float_op_optset  = makeFunctionExt<2>(call_ggcsylv_prefact_op_optset<float>, "X", ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS,
                                                                                                         ParamFloatQA, ParamFloatZA, ParamFloatQB, ParamFloatZB,
	...
	auto parser = makeArgParser("mepack_ggcsylv", cgsylv_double,  cgsylv_double_optset, cgsylv_double_op, cgsylv_double_op_optset,
                                                 cgsylv_10ret_double,  cgsylv_10ret_double_optset, cgsylv_10ret_double_op, cgsylv_10ret_double_op_optset,
                                                 cgsylv_prefact_double, cgsylv_prefact_double_optset, cgsylv_prefact_double_op, cgsylv_prefact_double_op_optset,
                                                 cgsylv_10ret_float,  cgsylv_10ret_float_optset, cgsylv_10ret_float_op, cgsylv_10ret_float_op_optset,
                                                 cgsylv_prefact_float, cgsylv_prefact_float_optset, cgsylv_prefact_float_op, cgsylv_prefact_float_op_optset,
                                                 cgsylv_float, cgsylv_float_op, cgsylv_float_optset, cgsylv_float_op_optset);
