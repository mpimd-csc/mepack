...
template<>
struct tcg_solver<double> {
    constexpr static const char *fn = "DLA_TGCSYLV_L3";
    constexpr static const char *fn_openmp = "DLA_TGCSYLV_DAG";
    ...
    static void solve(const char *opA, const char *opB, double sign1, double sign2, int M, int N, double * Aptr, int lda, double *Bptr, int ldb, double *Cptr, int ldc, double *Dptr, int ldd,
        ...
        mepack_double_tgcsylv_level3(opA, opB, sign1, sign2, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Yptr, ldy, Xptr, ldx, scale, buffer, info);
    };
    ...
    static void solve_openmp(const char *opA, const char *opB, double sign1, double sign2, int M, int N, double * Aptr, int lda, double *Bptr, int ldb, double *Cptr, int ldc, double *Dptr, int ldd,
            double *Yptr, int ldy, double *Xptr, int ldx, double * scale, double * buffer, int *info) {
        mepack_double_tgcsylv_dag(opA, opB, sign1, sign2, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Yptr, ldy, Xptr, ldx, scale, buffer, info);
    };
...
template<>
struct tcg_solver<float> {
    constexpr static const char *fn = "SLA_TGCSYLV_L3";
    constexpr static const char *fn_openmp = "SLA_TGCSYLV_DAG";
    ...
    static void solve(const char *opA, const char *opB, float sign1, float sign2, int M, int N, float * Aptr, int lda, float *Bptr, int ldb, float *Cptr, int ldc, float *Dptr, int ldd,
        ...
        mepack_single_tgcsylv_level3(opA, opB, sign1, sign2, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Yptr, ldy, Xptr, ldx, scale, buffer, info);
    };
    ...
    static void solve_openmp(const char *opA, const char *opB, float sign1, float sign2, int M, int N, float * Aptr, int lda, float *Bptr, int ldb, float *Cptr, int ldc, float *Dptr, int ldd,
            float *Yptr, int ldy, float *Xptr, int ldx, float * scale, float * buffer, int *info) {
        mepack_single_tgcsylv_dag(opA, opB, sign1, sign2, M, N, Aptr, lda, Bptr, ldb, Cptr, ldc, Dptr, ldd, Yptr, ldy, Xptr, ldx, scale, buffer, info);
    };
                      ...
                      MepackOptions optset = MepackOptions())
{
    ...
    if ( optset.getMB() >= 0 ) {
        tcg_solver<T>::blocksize_mb_set(optset.getMB());
    }
    if ( optset.getNB() >= 0 ) {
        tcg_solver<T>::blocksize_nb_set(optset.getNB());
    }

    if ( optset.getISolver() >= 0 ) {
        tcg_solver<T>::isolver_set(optset.getISolver());
    }
    ssize_t auxmem;
    if ( optset.getOpenMP()) {
        auxmem = mepack_memory(tcg_solver<T>::fn_openmp, A.rows, B.rows);
    } else {
        auxmem = mepack_memory(tcg_solver<T>::fn, A.rows, B.rows);
    }
    ...
    if ( optset.getOpenMP()) {
        tcg_solver<T>::solve_openmp(opA.c_str(), opB.c_str(), sign1, sign2, A.rows, B.rows, A.ptr(), A.ld, B.ptr(), B.ld, C.ptr(), C.ld, D.ptr(), D.ld,
                Y.ptr(), Y.ld, X.ptr(), X.ld, &scale, auxbuf, &info);
    } else {
        tcg_solver<T>::solve(opA.c_str(), opB.c_str(), sign1, sign2, A.rows, B.rows, A.ptr(), A.ld, B.ptr(), B.ld, C.ptr(), C.ld, D.ptr(), D.ld,
                Y.ptr(), Y.ld, X.ptr(), X.ld, &scale, auxbuf, &info);
    }
    if ( info != 0) {
...
)===")
{
	...
	auto parser = makeArgParser("mepack_tgcsylv", tgcsylv_double,  tgcsylv_double_optset, tgcsylv_double_op, tgcsylv_double_op_optset,
