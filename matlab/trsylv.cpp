...
template<>
struct tr_solver<double> {
    constexpr static const char *fn = "DLA_TRSYLV_L3";
    constexpr static const char *fn_openmp = "DLA_TRSYLV_DAG";
    ...
    static void solve(const char *opA, const char *opB, double sign, int M, int N, double * Aptr, int lda, double *Bptr, int ldb, double *Yptr, int ldy, double * scale, double * buffer, int *info) {
        mepack_double_trsylv_level3(opA, opB, sign, M, N, Aptr, lda, Bptr, ldb, Yptr, ldy, scale, buffer, info);
    };
    ...
    static void solve_openmp(const char *opA, const char *opB, double sign, int M, int N, double * Aptr, int lda, double *Bptr, int ldb, double *Yptr, int ldy, double * scale, double * buffer, int *info) {
        mepack_double_trsylv_dag(opA, opB, sign, M, N, Aptr, lda, Bptr, ldb, Yptr, ldy, scale, buffer, info);
    };
...
template<>
struct tr_solver<float> {
    constexpr static const char *fn = "SLA_TRSYLV_L3";
    constexpr static const char *fn_openmp = "SLA_TRSYLV_DAG";
    ...
    static void solve(const char *opA, const char *opB, float sign, int M, int N, float * Aptr, int lda, float *Bptr, int ldb, float *Yptr, int ldy, float * scale, float * buffer, int *info) {
        mepack_single_trsylv_level3(opA, opB, sign, M, N, Aptr, lda, Bptr, ldb, Yptr, ldy, scale, buffer, info);
    };
    ...
    static void solve_openmp(const char *opA, const char *opB, float sign, int M, int N, float * Aptr, int lda, float *Bptr, int ldb, float *Yptr, int ldy, float * scale, float * buffer, int *info) {
        mepack_single_trsylv_dag(opA, opB, sign, M, N, Aptr, lda, Bptr, ldb, Yptr, ldy, scale, buffer, info);
    };
                      ...
                      MepackOptions optset = MepackOptions())
{
    ...
    if ( optset.getMB() >= 0 ) {
        tr_solver<T>::blocksize_mb_set(optset.getMB());
    }
    if ( optset.getNB() >= 0 ) {
        tr_solver<T>::blocksize_nb_set(optset.getNB());
    }

    if ( optset.getISolver() >= 0 ) {
        tr_solver<T>::isolver_set(optset.getISolver());
    }
    ssize_t auxmem;
    if ( optset.getOpenMP()) {
        auxmem = mepack_memory(tr_solver<T>::fn_openmp, A.rows, B.rows);
    } else {
        auxmem = mepack_memory(tr_solver<T>::fn, A.rows, B.rows);
    }
    ...
    if ( optset.getOpenMP()) {
        tr_solver<T>::solve_openmp(opA.c_str(), opB.c_str(), sign, A.rows, B.rows, A.ptr(), A.ld, B.ptr(), B.ld, Y.ptr(), Y.ld, &scale, auxbuf, &info);
    } else {
        tr_solver<T>::solve(opA.c_str(), opB.c_str(), sign, A.rows, B.rows, A.ptr(), A.ld, B.ptr(), B.ld, Y.ptr(), Y.ld, &scale, auxbuf, &info);
    }
    if ( info != 0) {
        mexoct_error("MEPACK:TRSYLV_LEVEL3", "TRSYLV_LEVEL3 failed with info = %d", (int) info);
    }
...
MEXOCT_ENTRY(mepack_trsylv,
R"===(
X = mepack_trsylv(A, B, Y)
X = mepack_trsylv(A, B, Y, OpA, OpB)
X = mepack_trsylv(A, B, Y, OptSet)
X = mepack_trsylv(A, B, Y, OpA, OpB, OptSet)
...
Xs = mepack_trsylv(As, Bs, Ys)
Xs = mepack_trsylv(As, Bs, Ys, OpA, OpB)
Xs = mepack_trsylv(As, Bs, Ys, OptSet)
Xs = mepack_trsylv(As, Bs, Ys, OpA, OpB, OptSet)
...
The function uses the level-3 solver D/SLA_TRSYLV_* of MEPACK.
...
)===")
{
	...
	auto parser = makeArgParser("mepack_trsylv", trsylv_double,  trsylv_double_optset, trsylv_double_op, trsylv_double_op_optset,
