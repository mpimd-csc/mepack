...
template<>
struct cgsylv_refine_solver<double> {
    constexpr static const char *fn = "DLA_GGCSYLV_REFINE";
    ...
    {
        mepack_double_ggcsylv_refine(TRANSA, TRANSB, GUESS, SGN1, SGN2, M, N, A, LDA, B, LDB, C, LDC, D, LDD, R, LDR, L, LDL, E, LDE, F, LDF,
...
template<>
struct cgsylv_refine_solver<float> {
    constexpr static const char *fn = "SLA_GGCSYLV_REFINE";
    ...
    {
        mepack_single_ggcsylv_refine(TRANSA, TRANSB, GUESS, SGN1, SGN2, M, N, A, LDA, B, LDB, C, LDC, D, LDD,
...
CGSylvDriver(ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
            ...
            MepackOptions optset = MepackOptions())
{
    ...
    if ( optset.getMB() >= 0 ) {
        cgsylv_refine_solver<T>::blocksize_mb_set(optset.getMB());
    }
    if ( optset.getNB() >= 0 ) {
        cgsylv_refine_solver<T>::blocksize_nb_set(optset.getNB());
    }

    if ( optset.getISolver() >= 0 ) {
        cgsylv_refine_solver<T>::isolver_set(optset.getISolver());
    }

    if ( optset.getBigNB() >= 0 ) {
        cgsylv_refine_solver<T>::blocksize_big_set(optset.getBigNB());
    }

    if ( optset.getFSolver() >= 0 ) {
        cgsylv_refine_solver<T>::fsolver_set(optset.getFSolver());
    }
    ...
    ssize_t auxmem;
    auxmem = mepack_memory_frontend(cgsylv_refine_solver<T>::fn, (const char *){"F"}, (const char *){"F"}, A.rows, B.rows);
    if ( auxmem < 0 ) {
    ...
    cgsylv_refine_solver<T>::solve(opA.c_str(), opB.c_str(), guess.c_str(), sign1, sign2 , A.rows, B.rows,
    ...
    if ( info != 0) {
        mexoct_error("MEPACK:CGSYLV_REFINE", "CGSYLV_REFINE failed with info = %d", (int) info);
    }
        ...
        std::string opA = std::string("N"), std::string opB = std::string("N"), MepackOptions optset = MepackOptions())
{
    ...
    auto ret = CGSylvDriver(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, R, L, opA, opB, "N", optset);
    return std::make_tuple(std::get<0>(ret), std::get<1>(ret));
}

template <typename T>
std::tuple<ArrayType<T>, ArrayType<T>, ArrayType<T>>
call_ggcsylv_op_optset_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F,
...
call_ggcsylv_x0_op_optset(
        ...
        std::string opA = std::string("N"), std::string opB = std::string("N"), MepackOptions optset = MepackOptions())
{
    auto ret = CGSylvDriver(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, R, L, opA, opB, "I", optset);
    return std::make_tuple(std::get<0>(ret), std::get<1>(ret));
}

template <typename T>
std::tuple<ArrayType<T>, ArrayType<T>,ArrayType<T>>
call_ggcsylv_x0_op_optset_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F,  ArrayType<T> R, ArrayType<T> L,
...
call_ggcsylv(
        ...
        ArrayType<T> E, ArrayType<T> F)
{
    return call_ggcsylv_op_optset(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>,ArrayType<T>>
call_ggcsylv_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F)
{
    return call_ggcsylv_op_optset_convlog(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F);
}
...
call_ggcsylv_x0(
        ...
        ArrayType<T> E, ArrayType<T> F, ArrayType<T> R, ArrayType<T> L)
{
    return call_ggcsylv_x0_op_optset(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, R, L);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>,ArrayType<T>>
call_ggcsylv_x0_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F, ArrayType<T> R, ArrayType<T> L)
{
    return call_ggcsylv_x0_op_optset_convlog(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, R, L);
}
...
{
    return call_ggcsylv_op_optset(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, opA, opB);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>,ArrayType<T>>
call_ggcsylv_op_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F,
        std::string opA, std::string opB)
{
    return call_ggcsylv_op_optset_convlog(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, opA, opB);
}
...
call_ggcsylv_x0_op(
        ...
        std::string opA, std::string opB)
{
    return call_ggcsylv_x0_op_optset(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, R, L, opA, opB);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>,ArrayType<T>>
call_ggcsylv_x0_op_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F, ArrayType<T> R, ArrayType<T> L,
        std::string opA, std::string opB)
{
    return call_ggcsylv_x0_op_optset_convlog(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, R, L, opA, opB);
}
...
call_ggcsylv_opt(
        ...
        MepackOptions opt)
{
    return call_ggcsylv_op_optset(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, "N", "N",  opt);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>,ArrayType<T>>
call_ggcsylv_opt_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F,
        MepackOptions opt)
{
    return call_ggcsylv_op_optset_convlog(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, "N", "N", opt);
}
...
call_ggcsylv_x0_opt(
        ...
        MepackOptions opt)
{
    return call_ggcsylv_x0_op_optset(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, R, L, "N", "N", opt);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>,ArrayType<T>>
call_ggcsylv_x0_opt_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F, ArrayType<T> R, ArrayType<T> L,
        MepackOptions opt)
{
    return call_ggcsylv_x0_op_optset_convlog(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V , E, F, R, L, "N", "N",  opt);
}
...
{
    return call_ggcsylv_op_optset(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, opA, opB, opt);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>,ArrayType<T>>
call_ggcsylv_op_opt_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F,
        std::string opA, std::string opB, MepackOptions opt)
{
    return call_ggcsylv_op_optset_convlog(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E,F, opA, opB, opt);
}

template<typename T>
std::tuple<ArrayType<T>,ArrayType<T>>
call_ggcsylv_x0_op_opt(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F, ArrayType<T> R, ArrayType<T> L,
        std::string opA, std::string opB, MepackOptions opt)
{
    return call_ggcsylv_x0_op_optset(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, R, L, opA, opB, opt );
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>,ArrayType<T>>
call_ggcsylv_x0_op_opt_convlog(
        ArrayType<T> A, ArrayType<T> B, ArrayType<T> C, ArrayType<T> D,
        ArrayType<T> AS, ArrayType<T> BS, ArrayType<T> CS, ArrayType<T> DS,
        ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> U, ArrayType<T> V,
        ArrayType<T> E, ArrayType<T> F, ArrayType<T> R, ArrayType<T> L,
        std::string opA, std::string opB, MepackOptions opt)
{
    return call_ggcsylv_x0_op_optset_convlog(A, B, C, D, AS, BS, CS, DS, Q, Z, U, V, E, F, R, L, opA, opB, opt);
}

MEXOCT_ENTRY(mepack_csylv_refine,
R"===(
...
)===")
{
    ...
    auto cgsylv_double                         = makeFunctionExt<2>(call_ggcsylv<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF);
    auto cgsylv_double_convlog                 = makeFunctionExt<3>(call_ggcsylv_convlog<double>, "X, convlog", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF);

    auto cgsylv_double_x0                      = makeFunctionExt<2>(call_ggcsylv_x0<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamDoubleR0, ParamDoubleL0);
    auto cgsylv_double_x0_convlog              = makeFunctionExt<3>(call_ggcsylv_x0_convlog<double>, "X, convlog", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamDoubleR0, ParamDoubleL0);

    auto cgsylv_double_op                      = makeFunctionExt<2>(call_ggcsylv_op<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamOpAString, ParamOpBString);
    auto cgsylv_double_op_convlog              = makeFunctionExt<3>(call_ggcsylv_op_convlog<double>, "X, convlog", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamOpAString, ParamOpBString);

    auto cgsylv_double_x0_op                   = makeFunctionExt<2>(call_ggcsylv_x0_op<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV,
                                                                                                    ParamDoubleE, ParamDoubleF, ParamDoubleR0, ParamDoubleL0, ParamOpAString, ParamOpBString);
    auto cgsylv_double_x0_op_convlog           = makeFunctionExt<3>(call_ggcsylv_x0_op_convlog<double>, "X, convlog", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD,
    ...
    auto cgsylv_double_opt                      = makeFunctionExt<2>(call_ggcsylv_opt<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamOptSet);
    auto cgsylv_double_opt_convlog              = makeFunctionExt<3>(call_ggcsylv_opt_convlog<double>, "X, convlog", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamOptSet);

    auto cgsylv_double_x0_opt                   = makeFunctionExt<2>(call_ggcsylv_x0_opt<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamDoubleR0, ParamDoubleL0, ParamOptSet);
    auto cgsylv_double_x0_opt_convlog           = makeFunctionExt<3>(call_ggcsylv_x0_opt_convlog<double>, "X, convlog", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamDoubleR0, ParamDoubleL0, ParamOptSet);

    auto cgsylv_double_op_opt                   = makeFunctionExt<2>(call_ggcsylv_op_opt<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamOpAString, ParamOpBString, ParamOptSet);
    auto cgsylv_double_op_opt_convlog              = makeFunctionExt<3>(call_ggcsylv_op_opt_convlog<double>, "X, convlog", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamOpAString, ParamOpBString, ParamOptSet);

    auto cgsylv_double_x0_op_opt                   = makeFunctionExt<2>(call_ggcsylv_x0_op_opt<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamDoubleR0, ParamDoubleL0, ParamOpAString, ParamOpBString, ParamOptSet);
    auto cgsylv_double_x0_op_opt_convlog           = makeFunctionExt<3>(call_ggcsylv_x0_op_opt_convlog<double>, "X, convlog", ParamDoubleA, ParamDoubleB, ParamDoubleC, ParamDoubleD, ParamDoubleAS, ParamDoubleBS, ParamDoubleCS, ParamDoubleDS, ParamDoubleQ, ParamDoubleZ, ParamDoubleU, ParamDoubleV, ParamDoubleE, ParamDoubleF, ParamDoubleR0, ParamDoubleL0, ParamOpAString, ParamOpBString, ParamOptSet);

    auto cgsylv_float                         = makeFunctionExt<2>(call_ggcsylv<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF);
    auto cgsylv_float_convlog                 = makeFunctionExt<3>(call_ggcsylv_convlog<float>, "X, convlog", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF);

    auto cgsylv_float_x0                      = makeFunctionExt<2>(call_ggcsylv_x0<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamFloatR0, ParamFloatL0);
    auto cgsylv_float_x0_convlog              = makeFunctionExt<3>(call_ggcsylv_x0_convlog<float>, "X, convlog", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamFloatR0, ParamFloatL0);

    auto cgsylv_float_op                      = makeFunctionExt<2>(call_ggcsylv_op<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamOpAString, ParamOpBString);
    auto cgsylv_float_op_convlog              = makeFunctionExt<3>(call_ggcsylv_op_convlog<float>, "X, convlog", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamOpAString, ParamOpBString);

    auto cgsylv_float_x0_op                   = makeFunctionExt<2>(call_ggcsylv_x0_op<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamFloatR0, ParamFloatL0, ParamOpAString, ParamOpBString);
    auto cgsylv_float_x0_op_convlog           = makeFunctionExt<3>(call_ggcsylv_x0_op_convlog<float>, "X, convlog", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamFloatR0, ParamFloatL0, ParamOpAString, ParamOpBString);

    auto cgsylv_float_opt                      = makeFunctionExt<2>(call_ggcsylv_opt<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamOptSet);
    auto cgsylv_float_opt_convlog              = makeFunctionExt<3>(call_ggcsylv_opt_convlog<float>, "X, convlog", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamOptSet);

    auto cgsylv_float_x0_opt                   = makeFunctionExt<2>(call_ggcsylv_x0_opt<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamFloatR0, ParamFloatL0, ParamOptSet);
    auto cgsylv_float_x0_opt_convlog           = makeFunctionExt<3>(call_ggcsylv_x0_opt_convlog<float>, "X, convlog", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamFloatR0, ParamFloatL0, ParamOptSet);

    auto cgsylv_float_op_opt                   = makeFunctionExt<2>(call_ggcsylv_op_opt<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamOpAString, ParamOpBString, ParamOptSet);
    auto cgsylv_float_op_opt_convlog              = makeFunctionExt<3>(call_ggcsylv_op_opt_convlog<float>, "X, convlog", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamOpAString, ParamOpBString, ParamOptSet);

    auto cgsylv_float_x0_op_opt                   = makeFunctionExt<2>(call_ggcsylv_x0_op_opt<float>, "X", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamFloatR0, ParamFloatL0, ParamOpAString, ParamOpBString, ParamOptSet);
    auto cgsylv_float_x0_op_opt_convlog           = makeFunctionExt<3>(call_ggcsylv_x0_op_opt_convlog<float>, "X, convlog", ParamFloatA, ParamFloatB, ParamFloatC, ParamFloatD, ParamFloatAS, ParamFloatBS, ParamFloatCS, ParamFloatDS, ParamFloatQ, ParamFloatZ, ParamFloatU, ParamFloatV, ParamFloatE, ParamFloatF, ParamFloatR0, ParamFloatL0, ParamOpAString, ParamOpBString, ParamOptSet);

	auto parser = makeArgParser("mepack_ggcsylv_refine", cgsylv_double,  cgsylv_double_convlog
                                                      , cgsylv_double_x0, cgsylv_double_x0_convlog
                                                      , cgsylv_double_op,  cgsylv_double_op_convlog
                                                      , cgsylv_double_x0_op , cgsylv_double_x0_op_convlog
                                                      , cgsylv_double_opt,  cgsylv_double_opt_convlog
                                                      , cgsylv_double_x0_opt, cgsylv_double_x0_opt_convlog
                                                      , cgsylv_double_op_opt,  cgsylv_double_op_opt_convlog
                                                      , cgsylv_double_x0_op_opt, cgsylv_double_x0_op_opt_convlog
                                                      , cgsylv_float,  cgsylv_float_convlog
                                                      , cgsylv_float_x0, cgsylv_float_x0_convlog
                                                      , cgsylv_float_op,  cgsylv_float_op_convlog
                                                      , cgsylv_float_x0_op, cgsylv_float_x0_op_convlog
                                                      , cgsylv_float_opt,  cgsylv_float_opt_convlog
                                                      , cgsylv_float_x0_opt, cgsylv_float_x0_opt_convlog
                                                      , cgsylv_float_op_opt,  cgsylv_float_op_opt_convlog
                                                      , cgsylv_float_x0_op_opt, cgsylv_float_x0_op_opt_convlog
                                                      );
