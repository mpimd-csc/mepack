...
template<>
struct sylv_solver<double> {
    constexpr static const char *fn = "DLA_GESYLV";
    ...
    static void solve(const char * facta, const char *factb, const char *opA, const char * opB, double sgn, int M, int N ,
        ...
        mepack_double_gesylv(facta, factb, opA, opB, sgn, M, N, Aptr, lda, Bptr, ldb, Qptr, ldq, Zptr, ldz, Yptr, ldy, scale, buffer, ldwork, info);
    };
...
template<>
struct sylv_solver<float> {
    constexpr static const char *fn = "SLA_GESYLV";
    ...
    static void solve(const char * facta, const char *factb, const char *opA, const char * opB, float sgn, int M, int N ,
        ...
        mepack_single_gesylv(facta, factb, opA, opB, sgn, M, N, Aptr, lda, Bptr, ldb, Qptr, ldq, Zptr, ldz, Yptr, ldy, scale, buffer, ldwork, info);
    };
    static void blocksize_mb_set(int nb) {
        ...
        MepackOptions optset = MepackOptions(), bool factorize = false)
{
    ...
    if ( optset.getMB() >= 0 ) {
        sylv_solver<T>::blocksize_mb_set(optset.getMB());
    }
    if ( optset.getNB() >= 0 ) {
        sylv_solver<T>::blocksize_nb_set(optset.getNB());
    }

    if ( optset.getISolver() >= 0 ) {
        sylv_solver<T>::isolver_set(optset.getISolver());
    }

    if ( optset.getFSolver() >= 0 ) {
        sylv_solver<T>::fsolver_set(optset.getFSolver());
    }


    if ( optset.getBigNB() >= 0 ) {
        sylv_solver<T>::blocksize_big_set(optset.getBigNB());
    }
...
call_sylv_op_optset(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> Y, std::string opA = std::string("N"), std::string opB = std::string("N"), MepackOptions optset = MepackOptions())
{
...
template<typename T>
std::tuple<ArrayType<T>>
call_sylv(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> Y)
{
    return call_sylv_op_optset(A, B, Y);
}

template<typename T>
std::tuple<ArrayType<T>>
call_sylv_op(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> Y, std::string opA, std::string opB)
{
    return call_sylv_op_optset(A, B, Y, opA, opB);
}

template<typename T>
std::tuple<ArrayType<T>>
call_sylv_optset(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> Y, MepackOptions optset)
{
   return call_sylv_op_optset(A, B, Y, "N", "N", optset);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>>
call_5ret_sylv_op_optset(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> Y, std::string opA = std::string("N"), std::string opB = std::string("N"),
...
template<typename T>
std::tuple<ArrayType<T>,ArrayType<T>,ArrayType<T>, ArrayType<T>, ArrayType<T>>
call_5ret_sylv(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> Y)
{

    return call_5ret_sylv_op_optset(A, B, Y);
}

template<typename T>
std::tuple<ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>>
call_5ret_sylv_op(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> Y, std::string opA, std::string opB)
{
    return call_5ret_sylv_op_optset(A, B, Y, opA, opB);
}

template<typename T>
std::tuple<ArrayType<T>,ArrayType<T>, ArrayType<T>, ArrayType<T>, ArrayType<T>>
call_5ret_sylv_optset(ArrayType<T>  A, ArrayType<T> B, ArrayType<T> Y, MepackOptions optset)
{
   return call_5ret_sylv_op_optset(A, B, Y, "N", "N", optset);
}

template <typename T>
std::tuple<ArrayType<T>>
call_sylv_prefact_op_optset(ArrayType<T>  S, ArrayType<T> Ts, ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> Y,
...
template<typename T>
std::tuple<ArrayType<T>>
call_sylv_prefact(ArrayType<T> S, ArrayType<T> Ts, ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> Y)
{
    return call_sylv_prefact_op_optset(S, Ts, Q, Z, Y);
}

template<typename T>
std::tuple<ArrayType<T>>
call_sylv_prefact_op(ArrayType<T>  S, ArrayType<T> Ts, ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> Y, std::string opA, std::string opB)
{
    return call_sylv_prefact_op_optset(S, Ts, Q, Z, Y, opA, opB);
}

template<typename T>
std::tuple<ArrayType<T>>
call_sylv_prefact_optset(ArrayType<T>  S, ArrayType<T> Ts, ArrayType<T> Q, ArrayType<T> Z, ArrayType<T> Y, MepackOptions optset)
{
   return call_sylv_prefact_op_optset(S, Ts, Q, Z, Y, "N", "N", optset);
}



MEXOCT_ENTRY(mepack_sylv,
R"===(
X = mepack_sylv(A, B, Y)
X = mepack_sylv(A, B, Y, OpA, OpB)
X = mepack_sylv(A, B, Y, OptSet)
X = mepack_sylv(A, B, Y, OpA, OpB, OptSet)
 Input:
...
[X, S, T, Q, Z] = mepack_sylv(A, B, Y)
[X, S, T, Q, Z] = mepack_sylv(A, B, Y, OpA, OpB)
[X, S, T, Q, Z] = mepack_sylv(A, B, Y, OptSet)
[X, S, T, Q, Z] = mepack_sylv(A, B, Y, OpA, OpB, OptSet)
  Input:
    A      -- first coefficient matrix
    B      -- second coefficient matrix
    Y      -- right hand side
...
X = mepack_sylv(S, T, Q, Z, Y)
X = mepack_sylv(S, T, Q, Z, Y, OpA, OpB)
X = mepack_sylv(S, T, Q, Z, Y, OptSet)
X = mepack_sylv(S, T, Q, Z, Y, OpA, OpB, OptSet)
  Input:
...
The mepack_sylv function solves the Sylvester equation
...
)===")
{
    ...
    auto sylv_double                    = makeFunctionExt<1>(call_sylv<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleY);
    auto sylv_double_optset             = makeFunctionExt<1>(call_sylv_optset<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleY, ParamOptSet);
    auto sylv_double_op                 = makeFunctionExt<1>(call_sylv_op<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleY, ParamOpAString, ParamOpBString);
    auto sylv_double_op_optset          = makeFunctionExt<1>(call_sylv_op_optset<double>, "X", ParamDoubleA, ParamDoubleB, ParamDoubleY, ParamOpAString, ParamOpBString, ParamOptSet);
    auto sylv_3ret_double               = makeFunctionExt<5>(call_5ret_sylv<double>, "[X, S, T, Q, Z]", ParamDoubleA, ParamDoubleB, ParamDoubleY);
    auto sylv_3ret_double_optset        = makeFunctionExt<5>(call_5ret_sylv_optset<double>, "[X, S, T, Q, Z]", ParamDoubleA, ParamDoubleB, ParamDoubleY, ParamOptSet);
    auto sylv_3ret_double_op            = makeFunctionExt<5>(call_5ret_sylv_op<double>, "[X, S, T, Q, Z]", ParamDoubleA, ParamDoubleB, ParamDoubleY, ParamOpAString, ParamOpBString);
    auto sylv_3ret_double_op_optset     = makeFunctionExt<5>(call_5ret_sylv_op_optset<double>, "[X, S, T, Q, Z]", ParamDoubleA, ParamDoubleB, ParamDoubleY, ParamOpAString,
                                                                                                                    ParamOpBString, ParamOptSet);
    auto sylv_prefact_double            = makeFunctionExt<1>(call_sylv_prefact<double>, "X",           ParamDoubleS, ParamDoubleT, ParamDoubleQ, ParamDoubleZ, ParamDoubleY);
    auto sylv_prefact_double_optset     = makeFunctionExt<1>(call_sylv_prefact_optset<double>, "X",    ParamDoubleS, ParamDoubleT, ParamDoubleQ, ParamDoubleZ, ParamDoubleY, ParamOptSet);
    auto sylv_prefact_double_op         = makeFunctionExt<1>(call_sylv_prefact_op<double>, "X",        ParamDoubleS, ParamDoubleT, ParamDoubleQ, ParamDoubleZ, ParamDoubleY,
                                                                                                         ParamOpAString, ParamOpBString);
    auto sylv_prefact_double_op_optset  = makeFunctionExt<1>(call_sylv_prefact_op_optset<double>, "X", ParamDoubleS, ParamDoubleT, ParamDoubleQ, ParamDoubleZ, ParamDoubleY,
                                                                                                         ParamOpAString, ParamOpBString, ParamOptSet);

    auto sylv_float                    = makeFunctionExt<1>(call_sylv<float>, "Xs", ParamFloatA, ParamFloatB, ParamFloatY);
    auto sylv_float_optset             = makeFunctionExt<1>(call_sylv_optset<float>, "Xs", ParamFloatA, ParamFloatB, ParamFloatY, ParamOptSet);
    auto sylv_float_op                 = makeFunctionExt<1>(call_sylv_op<float>, "Xs", ParamFloatA, ParamFloatB, ParamFloatY, ParamOpAString, ParamOpBString);
    auto sylv_float_op_optset          = makeFunctionExt<1>(call_sylv_op_optset<float>, "Xs", ParamFloatA, ParamFloatB, ParamFloatY, ParamOpAString, ParamOpBString, ParamOptSet);
    auto sylv_3ret_float               = makeFunctionExt<5>(call_5ret_sylv<float>, "[Xs, Ss, Ts, Qs, Zs]", ParamFloatA, ParamFloatB,  ParamFloatY);
    auto sylv_3ret_float_optset        = makeFunctionExt<5>(call_5ret_sylv_optset<float>, "[Xs, Ss, Ts, Qs, Zs]", ParamFloatA, ParamFloatB, ParamFloatY, ParamOptSet);
    auto sylv_3ret_float_op            = makeFunctionExt<5>(call_5ret_sylv_op<float>, "[Xs, Ss, Ts, Qs, Zs]", ParamFloatA, ParamFloatB, ParamFloatY, ParamOpAString, ParamOpBString);
    auto sylv_3ret_float_op_optset     = makeFunctionExt<5>(call_5ret_sylv_op_optset<float>, "[Xs, Ss, Ts, Qs, Zs]", ParamFloatA, ParamFloatB, ParamFloatY,
                                                                                                      ParamOpAString, ParamOpBString, ParamOptSet);
    auto sylv_prefact_float            = makeFunctionExt<1>(call_sylv_prefact<float>, "Xs",           ParamFloatS, ParamFloatT, ParamFloatQ, ParamFloatZ, ParamFloatY);
    auto sylv_prefact_float_optset     = makeFunctionExt<1>(call_sylv_prefact_optset<float>, "Xs",    ParamFloatS, ParamFloatT, ParamFloatQ, ParamFloatZ, ParamFloatY, ParamOptSet);
    auto sylv_prefact_float_op         = makeFunctionExt<1>(call_sylv_prefact_op<float>, "Xs",        ParamFloatS, ParamFloatT, ParamFloatQ, ParamFloatZ, ParamFloatY, ParamOpAString,
                                                                                                      ParamOpBString);
    auto sylv_prefact_float_op_optset  = makeFunctionExt<1>(call_sylv_prefact_op_optset<float>, "Xs", ParamFloatS, ParamFloatT, ParamFloatQ, ParamFloatZ, ParamFloatY,
                                                                                                      ParamOpAString, ParamOpBString, ParamOptSet);
	...
	auto parser = makeArgParser("mepack_sylv", sylv_double,  sylv_double_optset, sylv_double_op, sylv_double_op_optset,
                                                 sylv_3ret_double,  sylv_3ret_double_optset, sylv_3ret_double_op, sylv_3ret_double_op_optset,
                                                 sylv_prefact_double, sylv_prefact_double_optset, sylv_prefact_double_op, sylv_prefact_double_op_optset,
                                                 sylv_3ret_float,  sylv_3ret_float_optset, sylv_3ret_float_op, sylv_3ret_float_op_optset,
                                                 sylv_prefact_float, sylv_prefact_float_optset, sylv_prefact_float_op, sylv_prefact_float_op_optset,
                                                 sylv_float, sylv_float_op, sylv_float_optset, sylv_float_op_optset);
