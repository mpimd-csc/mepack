        ...
        MepackOptions optset = MepackOptions(), bool factorize = false)
{
    ...
    auxmem = mepack_memory_frontend(sylv2_solver<T>::fn, factA.c_str(), factB.c_str(), A.rows, B.rows);
    ...
    sylv2_solver<T>::solve(factA.c_str(), factB.c_str(), opA.c_str(), opB.c_str(), sign, A.rows, B.rows, A.ptr(), A.ld, B.ptr(), B.ld, Q.ptr(), Q.ld, Z.ptr(), Z.ld, Y.ptr(), Y.ld, &scale, auxbuf, auxmem, &info);
