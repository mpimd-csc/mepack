...
GSteinDriver( ArrayType<T>& A, ArrayType<T>& B, ArrayType<T>& Y, ArrayType<T>& Q, ArrayType<T>& Z, std::string opA = std::string("N"), MepackOptions optset = MepackOptions(), bool factorize = false)
{
    ...
    auxmem = mepack_memory_frontend(gstein_solver<T>::fn, factA.c_str(), factA.c_str(), A.rows, A.columns);
    ...
    gstein_solver<T>::solve(factA.c_str(), opA.c_str(), A.rows, A.ptr(), A.ld, B.ptr(), B.ld, Q.ptr(), Q.ld, Z.ptr(), Z.ld, Y.ptr(), Y.ld, &scale, auxbuf, auxmem, &info);
