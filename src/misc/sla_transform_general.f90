...
SUBROUTINE SLA_TRANSFORM_GENERAL(TRANS, M, N, X, LDX, QA, LDQA, QB, LDQB, WORK )
    ...
    REAL QA(LDQA, *), QB(LDQB, *), X(LDX, *), WORK(*)
    ...
    IF (LSAME(TRANS, 'T')) THEN
        ! Transform Y <- Q**T * Y * Q
        CALL SGEMM("T", "N", M, N, M, ONE, QA, LDQA, X, LDX, ZERO, WORK, M)
        CALL SGEMM("N", "N", M, N, N, ONE, WORK, M, QB, LDQB, ZERO, X, LDX)
    ELSE
        ! Transform X <- Q * X * Q**T
        CALL SGEMM("N", "N", M, N, M, ONE, QA, LDQA, X, LDX, ZERO, WORK, M)
        CALL SGEMM("N", "T", M, N, N, ONE, WORK, M, QB, LDQB, ZERO, X, LDX)
    END IF
    RETURN
END SUBROUTINE
