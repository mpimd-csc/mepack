...
SUBROUTINE DLA_TRANSFORM_STANDARD(TRANS, M, X, LDX, Q, LDQ, WORK)
    ...
    IF (LSAME(TRANS, 'T')) THEN
        ! Transform Y <- Q**T * Y * Q
        CALL DGEMM("T", "N", M, M, M, ONE, Q, LDQ, X, LDX, ZERO, WORK, M)
        CALL DGEMM("N", "N", M, M, M, ONE, WORK, M, Q, LDQ, ZERO, X, LDX)
    ELSE
        ! Transform X <- Q * X * Q**T
        CALL DGEMM("N", "N", M, M, M, ONE, Q, LDQ, X, LDX, ZERO, WORK, M)
        CALL DGEMM("N", "T", M, M, M, ONE, WORK, M, Q, LDQ, ZERO, X, LDX)
    END IF
    RETURN
END SUBROUTINE
