...
/**
 ...
 \brief Level-3 Bartels-Stewart Algorithm for the Stein equation with OpenMP 4
 ...
 \remark The routine is a wrapper around \ref dla_trstein_dag.
...
void mepack_double_trstein_dag(const char *TRANSA, int M, double * A, int LDA, double *X, int LDX, double * SCALE, double *WORK, int *INFO)
{
    ...
    FC_GLOBAL_(dla_trstein_dag,DLA_TRSTEIN_DAG)(TRANSA, &_M, A, &_LDA, X, &_LDX, SCALE, WORK, &_INFO, c1len);
    *INFO =(int) _INFO;
    return;
}
...
/**
 ...
 \remark The algorithm is implemented using DAG Scheduling
 \remark The routine is a wrapper around \ref sla_trstein_dag.
...
void mepack_single_trstein_dag(const char *TRANSA, int M, float * A, int LDA, float *X, int LDX, float * SCALE, float *WORK, int *INFO)
{
    ...
    FC_GLOBAL_(sla_trstein_dag,SLA_TRSTEIN_DAG)(TRANSA, &_M, A, &_LDA, X, &_LDX, SCALE, WORK, &_INFO, c1len);
    *INFO =(int) _INFO;
    return;
}
