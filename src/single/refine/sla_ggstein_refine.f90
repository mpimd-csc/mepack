...
!  ===========
!
!   SUBROUTINE SLA_GGSTEIN_REFINE(TRANS, GUESS, M ,   LDA, B, LDB, X, LDX, Y, LDY, &
...
!           REAL TAU
!           REAL A(LDA,*), B(LDB,*), Q(LDQ, *), Z(LDZ, *), X(LDX, *), Y(LDY,*), WORK(*)
!           REAL CONVLOG(*)
!
!> \par Purpose:
!  =============
...
!> \ingroup sgltglyap
!
SUBROUTINE SLA_GGSTEIN_REFINE(TRANS, GUESS, M ,  &
    ...
    IF (INFO .NE. 0 ) THEN
        CALL XERROR_HANDLER ('SLA_GGSTEIN_REFINE', -INFO)
        RETURN
    END IF
    ...
    TOL = REAL(M) * ( NRMA ** 2 + NRMB**2) * EPS * TAU
    ...
    DO WHILE ( IT .LE. MAXIT .AND. .NOT. LASTSTEP)
        ...
        ELSE
            ...
            IF ( BTRANS ) THEN  ! Transposed case
                ...
                CALL SGEMM("N", "N", M, M, M,  ONE, WORK(WS), M, B, LDB, ONE, WORK(RS), M)
            ELSE
                ...
                CALL SGEMM("N", "T", M, M, M,  ONE, WORK(WS), M, B, LDB, ONE, WORK(RS), M)
            END IF
        END IF
        ...
        SELECT CASE(L3SOLVER)
            CASE (FRONTEND_SOLVER_LEVEL3)
                CALL SLA_TGSTEIN_L3(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_LEVEL2)
                CALL SLA_TGSTEIN_L2(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_DAG)
                CALL SLA_TGSTEIN_DAG(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_2STAGE)
                CALL SLA_TGSTEIN_L3_2S(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_RECURSIVE)
                CALL SLA_TGSTEIN_RECURSIVE(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        END SELECT
    ...
    RETURN
END SUBROUTINE SLA_GGSTEIN_REFINE
