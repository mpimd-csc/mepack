...
!  ===========
!
!   SUBROUTINE SLA_GELYAP_REFINE(TRANS, GUESS, M, A, LDA, X, LDX, Y, LDY, &
!                            & AS, LDAS, Q, LDQ, &
!                            & MAXIT, TAU, CONVLOG, &
!                            & WORK, LDWORK, INFO)
!           CHARACTER TRANS(1), GUESS(1)
!           INTEGER M , LDA, LDAS, LDQ, LDX, LDY, MAXIT, INFO, LDWORK
!           REAL TAU
!           REAL A(LDA,*), Q(LDQ, *), X(LDX, *), Y(LDY,*), CONVLOG(*), WORK(*)
!
!> \par Purpose:
!  =============
!>
!> \verbatim
!> SLA_GELYAP_REFINE solves a standard Lyapunov equation of the following forms
!>
!>    A * X  +  X * A^T = SCALE * Y                                              (1)
!>
!> or
...
!>          The array Q contains the Schur vectors for A as returned by SGEES.
!> \endverbatim
!>
!> \param[in] LDQ
!> \verbatim
...
SUBROUTINE SLA_GELYAP_REFINE(TRANS, GUESS, M ,   A, LDA,  X, LDX, Y, LDY, &
    ...
    EXTERNAL SLA_TRLYAP_L3
    EXTERNAL SLA_TRLYAP_DAG
    EXTERNAL SLA_TRLYAP_L3_2S
    ...
    L3SOLVER = TRLYAP_FRONTEND_SOLVER()
    ...
    IF (INFO .NE. 0 ) THEN
        CALL XERROR_HANDLER ('SLA_GELYAP_REFINE', -INFO)
        RETURN
    END IF
    ...
    CASE (FRONTEND_SOLVER_LEVEL3)
        CALL SLA_TRLYAP_L3(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    CASE (FRONTEND_SOLVER_LEVEL2)
        CALL SLA_TRLYAP_L2(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    CASE (FRONTEND_SOLVER_DAG)
        CALL SLA_TRLYAP_DAG(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    CASE (FRONTEND_SOLVER_2STAGE)
        CALL SLA_TRLYAP_L3_2S(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    CASE (FRONTEND_SOLVER_RECURSIVE)
        CALL SLA_TRLYAP_RECURSIVE(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    END SELECT
    ...
    DO WHILE ( IT .LE. MAXIT .AND. .NOT. LASTSTEP)
        ! If we are in the first iteration and we do no have an initial guess,
        ! we set the initial to zero an only copy the right hand side.
        IF ( IT .EQ. 1 .AND.  .NOT. BGUESS ) THEN
            CALL SLACPY("All", M, M, Y(1,1), LDY, WORK(RS), M)
        ELSE
            CALL SLACPY("All", M, M, Y(1,1), LDY, WORK(RS), M)
            IF ( BTRANS ) THEN  ! Transposed case
                CALL SGEMM("T", "N", M, M, M, MONE, A, LDA, X, LDX, ONE, WORK(RS), M)
                CALL SGEMM("N", "N", M, M, M, MONE, X, LDX, A, LDA, ONE, WORK(RS), M)
            ELSE
                CALL SGEMM("N", "N", M, M, M, MONE, A, LDA, X, LDX, ONE, WORK(RS), M)
                CALL SGEMM("N", "T", M, M, M, MONE, X, LDX, A, LDA, ONE, WORK(RS), M)
            END IF
        END IF
        ...
        SELECT CASE(L3SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL SLA_TRLYAP_L3(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        CASE (FRONTEND_SOLVER_LEVEL2)
            CALL SLA_TRLYAP_L2(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        CASE (FRONTEND_SOLVER_DAG)
            CALL SLA_TRLYAP_DAG(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        CASE (FRONTEND_SOLVER_2STAGE)
            CALL SLA_TRLYAP_L3_2S(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        CASE (FRONTEND_SOLVER_RECURSIVE)
            CALL SLA_TRLYAP_RECURSIVE(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        END SELECT
        ...
        ! Transform X <- Q * X * Q**T
        CALL SLA_TRANSFORM_STANDARD("NonTranspose", M, WORK(RS), M, Q, LDQ, WORK(WS))
    ...
    RETURN
END SUBROUTINE SLA_GELYAP_REFINE
