...
!           REAL TAU
!           REAL A(LDA,*), Q(LDQ, *), X(LDX, *), WORK(*), CONVLOG(*)
!
!> \par Purpose:
!  =============
!>
!> \verbatim
!> SLA_GESTEIN_REFINE solves a standard Stein equation of the following forms
!>
!>    A * X * A^T  -  X  = SCALE * Y                                              (1)
!>
!> or
...
SUBROUTINE SLA_GESTEIN_REFINE(TRANS, GUESS, M, A, LDA, X, LDX, Y, LDY, &
    ...
    IF (INFO .NE. 0 ) THEN
        CALL XERROR_HANDLER ('SLA_GESTEIN_REFINE', -INFO)
        RETURN
    END IF
    ...
    DO WHILE ( IT .LE. MAXIT .AND. .NOT. LASTSTEP)
        ...
        SELECT CASE(L3SOLVER)
            CASE (FRONTEND_SOLVER_LEVEL3)
                CALL SLA_TRSTEIN_L3(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_LEVEL2)
                CALL SLA_TRSTEIN_L2(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_DAG)
                CALL SLA_TRSTEIN_DAG(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_2STAGE)
                CALL SLA_TRSTEIN_L3_2S(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_RECURSIVE)
                CALL SLA_TRSTEIN_RECURSIVE(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        END SELECT
    ...
    RETURN
END SUBROUTINE SLA_GESTEIN_REFINE
