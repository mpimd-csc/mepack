...
!> \see SLA_TRSYLV_L2_LOCAL_COPY_64
...
SUBROUTINE SLA_GESYLV_REFINE(TRANSA, TRANSB, GUESS, SGN, M , N,  &
    ...
    EXTERNAL SLA_TRSYLV_L3
    EXTERNAL SLA_TRSYLV_L3_2S
    ...
    IF (INFO .NE. 0 ) THEN
        CALL XERROR_HANDLER ('SLA_GESYLV_REFINE', -INFO)
        RETURN
    END IF
    ...
    SELECT CASE(L3SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL SLA_TRSYLV_L3(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, &
                & SCALE, WORK, IINFO )
        CASE (FRONTEND_SOLVER_LEVEL2)
            SELECT CASE(ISOLVER)
                CASE(1)
                    IF ( MAXMN .LE. 32 ) THEN
                        CALL SLA_TRSYLV_L2_LOCAL_COPY_32(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                            & X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 64) THEN
                        CALL SLA_TRSYLV_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                            & X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 96) THEN
                        CALL SLA_TRSYLV_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                            & X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 128) THEN
                        CALL SLA_TRSYLV_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                            & X, LDX, SCALE, WORK, IINFO)
                    ELSE
                        CALL SLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                            & X, LDX, SCALE, WORK, IINFO)
                    END IF
                CASE(2)
                    IF ( M .GT. 128 .OR. N .GT. 128) THEN
                        CALL SLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                            & X, LDX, SCALE, WORK, IINFO)
                    ELSE
                        CALL SLA_TRSYLV_L2_LOCAL_COPY(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                            & X, LDX, SCALE, WORK, IINFO)
                    END IF
                CASE(3)
                    CALL SLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                        & X, LDX, SCALE, WORK, IINFO)
                CASE(4)
                    CALL SLA_TRSYLV_L2(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                        & X, LDX, SCALE, WORK, IINFO)
                CASE(5)
                    CALL SLA_TRSYLV_L2_UNOPT(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                        & X, LDX, SCALE, WORK, IINFO)
                CASE(6)
                    CALL SLA_TRSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                        & X, LDX, SCALE, WORK, IINFO)
            END SELECT
        CASE (FRONTEND_SOLVER_DAG)
            CALL SLA_TRSYLV_DAG(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, &
                & SCALE, WORK, IINFO )

        CASE (FRONTEND_SOLVER_2STAGE)
            CALL SLA_TRSYLV_L3_2S(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, &
                & SCALE, WORK, IINFO )
    ...
    DO WHILE ( IT .LE. MAXIT .AND. .NOT. LASTSTEP)
        ...
        ELSE
            CALL SLACPY("All", M, N, Y(1,1), LDY, WORK(RS), M)
            CALL SGEMM(TRANSA, "N", M, N, M, MONE, A, LDA, X, LDX, ONE, WORK(RS), M)
            CALL SGEMM("N", TRANSB, M, N, N, SGN*MONE, X, LDX, B, LDB, ONE, WORK(RS), M)
        END IF
        ...
        SELECT CASE(L3SOLVER)
            CASE (FRONTEND_SOLVER_LEVEL3)
                CALL SLA_TRSYLV_L3(TRANSA, TRANSB, SGN, M, N, AS, LDAS, BS, LDBS, &
                    & WORK(RS), M, &
                    & SCALE, WORK(WS), IINFO )
            CASE (FRONTEND_SOLVER_LEVEL2)
                SELECT CASE(ISOLVER)
                    CASE(1)
                        IF ( MAXMN .LE. 32 ) THEN
                            CALL SLA_TRSYLV_L2_LOCAL_COPY_32(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                &  WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE IF ( MAXMN .LE. 64) THEN
                            CALL SLA_TRSYLV_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE IF ( MAXMN .LE. 96) THEN
                            CALL SLA_TRSYLV_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE IF ( MAXMN .LE. 128) THEN
                            CALL SLA_TRSYLV_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN,M, N,  AS, LDAS, BS, LDBS, &
                                & WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE
                            CALL SLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & WORK(RS), M, SCALE, WORK(WS), IINFO)
                        END IF
                    CASE(2)
                        IF ( M .GT. 128 .OR. N .GT. 128) THEN
                            CALL SLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE
                            CALL SLA_TRSYLV_L2_LOCAL_COPY(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & WORK(RS), M, SCALE, WORK(WS), IINFO)
                        END IF
                    CASE(3)
                        CALL SLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                            & WORK(RS), M, SCALE, WORK(WS), IINFO)
                    CASE(4)
                        CALL SLA_TRSYLV_L2(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                            & WORK(RS), M, SCALE, WORK(WS), IINFO)
                    CASE(5)
                        CALL SLA_TRSYLV_L2_UNOPT(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                            & WORK(RS), M, SCALE, WORK(WS), IINFO)
                    CASE(6)
                        CALL SLA_TRSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                            & WORK(RS), M, SCALE, WORK(WS), IINFO)
                END SELECT
            CASE (FRONTEND_SOLVER_DAG)
                CALL SLA_TRSYLV_DAG(TRANSA, TRANSB, SGN, M, N, AS, LDAS, BS, LDBS, &
                    & WORK(RS), M, &
                    & SCALE, WORK(WS), IINFO )

            CASE (FRONTEND_SOLVER_2STAGE)
                CALL SLA_TRSYLV_L3_2S(TRANSA, TRANSB, SGN, M, N, AS, LDAS, BS, LDBS, &
                    & WORK(RS), M, &
                    & SCALE, WORK(WS), IINFO )
    ...
    RETURN
END SUBROUTINE SLA_GESYLV_REFINE
