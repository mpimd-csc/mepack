...
!  ===========
!
!   SUBROUTINE SLA_GGSYLV_REFINE(TRANSA, TRANSB, GUESS, SGN, M , N,  &
...
!       REAL :: Q(LDQ, *), Z(LDZ, *), U(LDU, *), V(LDV, *), X ( LDX , * ), Y (LDY, *)
!       REAL :: CONVLOG(*), WORK(*)
!
!> \par Purpose:
!  =============
...
SUBROUTINE SLA_GGSYLV_REFINE(TRANSA, TRANSB, GUESS, SGN, M , N,  &
    ...
    REAL :: NRMR, SCALE
    REAL :: NRMRHS, NRMA, NRMB, NRMC, NRMD, EPS , TOL
    ...
    IF (INFO .NE. 0 ) THEN
        CALL XERROR_HANDLER ('SLA_GGSYLV_REFINE', -INFO)
        RETURN
    END IF
    ...
    SELECT CASE(L3SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL SLA_TGSYLV_L3(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, &
                & SCALE, WORK, IINFO )
        ...
        CASE (FRONTEND_SOLVER_DAG)
            CALL SLA_TGSYLV_DAG(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, &
                & SCALE, WORK, IINFO )

        CASE (FRONTEND_SOLVER_2STAGE)
            CALL SLA_TGSYLV_L3_2S(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, &
                & SCALE, WORK, IINFO )
        ...
        CASE (FRONTEND_SOLVER_GARDINER_LAUB)
           CALL SLA_TGSYLV_GARDINER_LAUB(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, &
                & C, LDC, D, LDD,  X, LDX, SCALE, WORK, IINFO)
    ...
    DO WHILE ( IT .LE. MAXIT .AND. .NOT. LASTSTEP)
        ...
        ELSE
            ...
            CALL SGEMM(TRANSA, "N", M, N, M, ONE, C, LDC, X, LDX, ZERO, WORK(WS), M)
            CALL SGEMM("N", TRANSB, M, N, N, SGN*MONE, WORK(WS), M , D, LDD, ONE, WORK(RS), M)
        END IF
        ...
        IF ( BTRANSA .AND. BTRANSB ) THEN   !(N,N)
            CALL SLA_TRANSFORM_GENERAL("Transpose", M, N, WORK(RS), M, Q, LDQ, V, LDV, WORK(WS))
        ...
        ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN   !(T,N)
            CALL SLA_TRANSFORM_GENERAL("Transpose", M, N, WORK(RS), M, Z, LDZ, V, LDV, WORK(WS))
        ELSE !(T,T)
            CALL SLA_TRANSFORM_GENERAL("Transpose", M, N, WORK(RS), M, Z, LDZ, U, LDU, WORK(WS))
        END IF
        ...
        SELECT CASE(L3SOLVER)
            CASE (FRONTEND_SOLVER_LEVEL3)
                CALL SLA_TGSYLV_L3(TRANSA, TRANSB, SGN, M, N, AS, LDAS, BS, LDBS, CS, LDCS, DS, LDDS, &
                    & WORK(RS), M, &
                    & SCALE, WORK(WS), IINFO )
            CASE (FRONTEND_SOLVER_LEVEL2)
                SELECT CASE(ISOLVER)
                    CASE(1)
                        IF ( MAXMN .LE. 32 ) THEN
                            CALL SLA_TGSYLV_L2_LOCAL_COPY_32(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE IF ( MAXMN .LE. 64) THEN
                            CALL SLA_TGSYLV_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE IF ( MAXMN .LE. 96) THEN
                            CALL SLA_TGSYLV_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE IF ( MAXMN .LE. 128) THEN
                            CALL SLA_TGSYLV_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN,M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE
                            CALL SLA_TGSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                        END IF
                    CASE(2)
                        IF ( M .GT. 128 .OR. N .GT. 128) THEN
                            CALL SLA_TGSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                        ELSE
                            CALL SLA_TGSYLV_L2_LOCAL_COPY(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                        END IF
                    CASE(3)
                        CALL SLA_TGSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                            & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                    CASE(4)
                        CALL SLA_TGSYLV_L2(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                            & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                    CASE(5)
                        CALL SLA_TGSYLV_L2_UNOPT(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                            & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                    CASE(6)
                        CALL SLA_TGSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                            & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
                END SELECT
            CASE (FRONTEND_SOLVER_DAG)
                CALL SLA_TGSYLV_DAG(TRANSA, TRANSB, SGN, M, N, AS, LDAS, BS, LDBS, CS, LDCS, DS, LDDS, &
                    & WORK(RS), M, &
                    & SCALE, WORK(WS), IINFO )

            CASE (FRONTEND_SOLVER_2STAGE)
                CALL SLA_TGSYLV_L3_2S(TRANSA, TRANSB, SGN, M, N, AS, LDAS, BS, LDBS, CS, LDCS, DS, LDDS, &
                    & WORK(RS), M, &
                    & SCALE, WORK(WS), IINFO )
            ...
            CASE (FRONTEND_SOLVER_RECURSIVE)
               CALL SLA_TGSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                    & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_GARDINER_LAUB)
               CALL SLA_TGSYLV_GARDINER_LAUB(TRANSA, TRANSB, SGN, M, N,  AS, LDAS, BS, LDBS, &
                    & CS, LDCS, DS, LDDS,  WORK(RS), M, SCALE, WORK(WS), IINFO)
        END SELECT
        ...
        !
        IF ( BTRANSA .AND. BTRANSB ) THEN   !(N,N)
            CALL SLA_TRANSFORM_GENERAL("NonTranspose", M, N, WORK(RS), M, Z, LDZ, U, LDU, WORK(WS))
        ELSE IF ( BTRANSA .AND. .NOT. BTRANSB ) THEN   !(N,T)
            CALL SLA_TRANSFORM_GENERAL("NonTranspose", M, N, WORK(RS), M, Z, LDZ, V, LDV, WORK(WS))
        ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN   !(T,N)
            CALL SLA_TRANSFORM_GENERAL("NonTranspose", M, N, WORK(RS), M, Q, LDQ, U, LDU, WORK(WS))
        ELSE !(T,T)
            CALL SLA_TRANSFORM_GENERAL("NonTranspose", M, N, WORK(RS), M, Q, LDQ, V, LDV, WORK(WS))
        END IF
    ...
    RETURN
END SUBROUTINE SLA_GGSYLV_REFINE
