...
!  Definition:
!  ===========
!       FUNCTION SLA_RESIDUAL_STEIN(TRANS, M, A, LDA, X, LDX, Y, LDY, SCALE)
!           CHARACTER TRANS(!)
!           REAL SCALE
...
FUNCTION SLA_RESIDUAL_STEIN(TRANS, M, A, LDA, X, LDX, Y, LDY, SCALE)
    ...
    END IF
    IF (INFO .NE. 0 ) THEN
        SLA_RESIDUAL_STEIN = REAL(INFO)
        CALL XERROR_HANDLER("SLA_RESIDUAL_STEIN", -INFO)
        RETURN
    END IF

    IF ( M .EQ. 0 ) THEN
        SLA_RESIDUAL_STEIN = 0.0
        RETURN
    END IF

    ALLOCATE(WORK(M,M), STAT = ALLOC_STATUS)
    IF ( ALLOC_STATUS .NE. 0) THEN
        SLA_RESIDUAL_STEIN = REAL(-1000)
        RETURN
    END IF
    ...
    SLA_RESIDUAL_STEIN = NRMR / ( (NRMA **2 + DONE) * NRMX + SCALE * NRMY)

    DEALLOCATE(WORK)
    DEALLOCATE(WORK2)
    RETURN

END FUNCTION SLA_RESIDUAL_STEIN
