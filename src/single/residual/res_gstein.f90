...
!  Definition:
!  ===========
!       FUNCTION SLA_RESIDUAL_GSTEIN(TRANS, M, A, LDA, B, LDB, X, LDX, Y, LDY, SCALE)
!           CHARACTER TRANS(!)
!           REAL SCALE
...
FUNCTION SLA_RESIDUAL_GSTEIN(TRANS, M, A, LDA, B, LDB, X, LDX, Y, LDY, SCALE)
    ...
    INTEGER :: M, LDA, LDB, LDX, LDY
    REAL :: A(LDA, *), B(LDB, *), X(LDX, *), Y(LDY, *)
    REAL :: SCALE, SLA_RESIDUAL_GSTEIN
    ...
    END IF
    IF (INFO .NE. 0 ) THEN
        SLA_RESIDUAL_GSTEIN = REAL(INFO)
        CALL XERROR_HANDLER("SLA_RESIDUAL_GSTEIN", -INFO)
        RETURN
    END IF

    IF ( M .EQ. 0 ) THEN
        SLA_RESIDUAL_GSTEIN = 0.0
        RETURN
    END IF

    ALLOCATE(WORK(M,M), STAT = ALLOC_STATUS)
    IF ( ALLOC_STATUS .NE. 0) THEN
        SLA_RESIDUAL_GSTEIN = REAL(-1000)
        RETURN
    END IF
    ...
    CALL SGEMM("N", TRANS2, M, M, M, MONE, WORK2, M, A, LDA, SCALE, WORK, M)

    CALL SGEMM(TRANS, "N", M, M, M, DONE, B, LDB, X, LDX, DZERO, WORK2, M)
    CALL SGEMM("N", TRANS2, M, M, M, DONE, WORK2, M, B, LDB, DONE, WORK, M)

    NRMR = SLANGE("F", M, M, WORK(1,1), M, DUMMY)

    SLA_RESIDUAL_GSTEIN = NRMR / ( (NRMA**2 + NRMB**2) + NRMX + SCALE * NRMY)
...
END FUNCTION SLA_RESIDUAL_GSTEIN
