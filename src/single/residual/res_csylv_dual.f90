...
!> \ingroup residual
!
FUNCTION SLA_RESIDUAL_CSYLV_DUAL(TRANSA, TRANSB, SGN1, SGN2, M, N, A, LDA, B, LDB, C, LDC, D, LDD,  &
        & R, LDR, L, LDL, E, LDE, F, LDF, SCALE)
    IMPLICIT NONE
    CHARACTER :: TRANSA, TRANSB
    INTEGER :: M, N, LDA, LDB, LDC, LDD, LDE, LDF, LDR, LDL
    REAL :: A(LDA, *), B(LDB, *), C(LDC,*), D(LDD, *), E(LDE, *), F(LDF, *), R(LDR, *), L(LDL, *)
    REAL :: SCALE, SGN1, SGN2, SLA_RESIDUAL_CSYLV_DUAL
    ...
    IF (INFO .NE. 0 ) THEN
        SLA_RESIDUAL_CSYLV_DUAL = REAL(INFO)
        CALL XERROR_HANDLER("SLA_RESIDUAL_CSYLV", -INFO)
        RETURN
    END IF

    IF ( M .EQ. 0 .OR. N .EQ. 0) THEN
        SLA_RESIDUAL_CSYLV_DUAL = 0.0
        RETURN
    END IF

    ALLOCATE(WORK(M,N), STAT = ALLOC_STATUS)
    IF ( ALLOC_STATUS .NE. 0) THEN
        SLA_RESIDUAL_CSYLV_DUAL = REAL(-1000)
        RETURN
    END IF
    ...
    CALL SGEMM(TRANSAI, "N", M, N, M, MONE, A, LDA, R, LDR, SCALE, WORK, M)
    CALL SGEMM(TRANSAI, "N", M, N, M, MONE, C, LDC, L, LDL, DONE, WORK, M)
    NRMR1 = SLANGE("F", M, N, WORK(1,1), M, DUMMY)

    CALL SLACPY("A", M, N, F, LDF, WORK(1,1), M)
    CALL SGEMM("N", TRANSBI, M, N, N, MONE*SGN1, R, LDR, B, LDB, SCALE, WORK, M)
    CALL SGEMM("N", TRANSBI, M, N, N, MONE*SGN2 , L, LDL, D, LDD, DONE, WORK, M)
    NRMR2 = SLANGE("F", M, N, WORK(1,1), M, DUMMY)


    SLA_RESIDUAL_CSYLV_DUAL = MAX(NRMR1/ ( (NRMA  * NRMR + NRMC * NRML ) + SCALE * NRME), &
...
END FUNCTION SLA_RESIDUAL_CSYLV_DUAL
