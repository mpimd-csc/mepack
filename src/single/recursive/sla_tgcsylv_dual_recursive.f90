...
!  Definition:
!  ===========
!       SUBROUTINE SLA_TGCSYLV_DUAL_RECURSIVE ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
!> \ingroup sgltgsylv
!
RECURSIVE SUBROUTINE SLA_TGCSYLV_DUAL_RECURSIVE(TRANSA, TRANSB, SGN1, SGN2, M, N, A, LDA, B, LDB, C, LDC, D, LDD,&
        & E, LDE,  F, LDF, SCALE, WORK, INFO)
    ...
    EXTERNAL SLA_TGCSYLV_DUAL_KERNEL_44NN
    EXTERNAL SLA_TGCSYLV_DUAL_KERNEL_44NT
    EXTERNAL SLA_TGCSYLV_DUAL_KERNEL_44TN
    EXTERNAL SLA_TGCSYLV_DUAL_KERNEL_44TT
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGCSYLV_DUAL_RECURSIVE', -INFO)
        RETURN
    END IF
    ...
    IF (M .LE. BOUND .AND. N .LE. BOUND) THEN
            ...
            IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB) THEN
                !(N,N)
                CALL SLA_TGCSYLV_DUAL_KERNEL_44NN(SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, E, LDE, F, LDF, SCALE, IINFO)
            ELSE IF ( .NOT. BTRANSA .AND. BTRANSB) THEN
                !(N,T)
                CALL SLA_TGCSYLV_DUAL_KERNEL_44NT(SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, E, LDE, F, LDF, SCALE, IINFO)
            ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
                !(T,N)
                CALL SLA_TGCSYLV_DUAL_KERNEL_44TN(SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, E, LDE, F, LDF, SCALE, IINFO)

            ELSE IF ( BTRANSA .AND. BTRANSB) THEN
                !(T,T)
                CALL SLA_TGCSYLV_DUAL_KERNEL_44TT(SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, E, LDE, F, LDF, SCALE, IINFO)
            ENDIF
            INFO = IINFO
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        IF ( N .LE. M/2 .OR. N .LE. BOUND ) THEN
            ...
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N', 'N', SGN1, SGN2, M1, N, A, LDA, B, LDB, C, LDC, D,LDD, &
                & E, LDE, F, LDF,  SCALE, WORK, INFO )

            CALL SGEMM("T", "N", M2, N, M1, -ONE, A(1,M1+1), LDA, E(1,1), LDE, SCALE, E(M1+1,1), LDE)
            CALL SGEMM("T", "N", M2, N, M1, -ONE, C(1,M1+1), LDC, F(1,1), LDF, SCALE, E(M1+1,1), LDE)

            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N', 'N', SGN1, SGN2, M2, N, A(M1+1,M1+1), LDA, B, LDB, C(M1+1,M1+1), LDC, D,LDD, &
        ...
        ELSE IF ( M .LE. N/2  .OR. M .LE. BOUND ) THEN
            ...
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N', 'N', SGN1, SGN2, M, N2, A, LDA, B(N1+1,N1+1), LDB, C, LDC, D(N1+1,N1+1),LDD, &
                & E(1,N1+1), LDE, F(1,N1+1), LDF,  SCALE, WORK, INFO )

            CALL SGEMM("N", "T", M, N1, N2, -SGN1, E(1,N1+1), LDE, B(1,N1+1), LDB, SCALE, F, LDF)
            CALL SGEMM("N", "T", M, N1, N2, -SGN2, F(1,N1+1), LDF, D(1,N1+1), LDD, SCALE, F, LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N', 'N', SGN1, SGN2, M, N1, A, LDA, B, LDB, C, LDC, D,LDD, &
        ...
        ELSE
            ...
            ! E12
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N','N', SGN1, SGN2,  M1, N2, A(1,1), LDA, B(N1+1,N1+1), LDB, C(1,1), LDC, &
            ...
            CALL SGEMM("T", "N", M2, N2, M1, -ONE, C(1,M1+1), LDC, F(1,N1+1), LDF, SCALE, E(M1+1,N1+1), LDE)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N','N', SGN1, SGN2,  M2, N2, A(M1+1,M1+1), LDA, B(N1+1,N1+1), LDB, C(M1+1,M1+1), LDC, &
            ...
            ! E11
            CALL SGEMM("N", "T", M1, N1, N2, -SGN1, E(1,N1+1), LDE, B(1,N1+1), LDB, SCALE, F(1,1), LDF)
            CALL SGEMM("N", "T", M1, N1, N2, -SGN2, F(1,N1+1), LDF, D(1,N1+1), LDD, SCALE, F(1,1), LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N','N', SGN1, SGN2,  M1, N1, A(1,1), LDA, B(1,1), LDB, C(1,1), LDC, &
            ...
            CALL SGEMM("T", "N", M2, N1, M1, -ONE, C(1,M1+1), LDC, F(1,1), LDF, SCALE, E(M1+1,1), LDE)
            CALL SGEMM("N", "T", M2, N1, N2, -SGN1, E(M1+1,N1+1), LDE, B(1,N1+1), LDB, ONE, F(M1+1,1), LDF)
            CALL SGEMM("N", "T", M2, N1, N2, -SGN2, F(M1+1,N1+1), LDF, D(1,N1+1), LDD, ONE, F(M1+1,1), LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N','N', SGN1, SGN2,  M2, N1, A(M1+1,M1+1), LDA, B(1,1), LDB, C(M1+1,M1+1), LDC, &
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        IF ( N .LE. M/2 .OR. N .LE. BOUND ) THEN
            ...
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N', 'T', SGN1, SGN2, M1, N, A, LDA, B, LDB, C, LDC, D,LDD, &
                & E, LDE, F, LDF,  SCALE, WORK, INFO )

            CALL SGEMM("T", "N", M2, N, M1, -ONE, A(1,M1+1), LDA, E(1,1), LDE, SCALE, E(M1+1,1), LDE)
            CALL SGEMM("T", "N", M2, N, M1, -ONE, C(1,M1+1), LDC, F(1,1), LDF, SCALE, E(M1+1,1), LDE)

            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N', 'T', SGN1, SGN2, M2, N, A(M1+1,M1+1), LDA, B, LDB, C(M1+1,M1+1), LDC, D,LDD, &
                & E(M1+1,1), LDE, F(M1+1,1), LDF,  SCAL, WORK, INFO )
        ...
        ELSE IF ( M .LE. N/2  .OR. M .LE. BOUND ) THEN
            ...
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N', 'T', SGN1, SGN2, M, N1, A, LDA, B(1,1), LDB, C, LDC, D(1,1),LDD, &
                & E(1,1), LDE, F(1,1), LDF,  SCALE, WORK, INFO )

            CALL SGEMM("N", "N", M, N2, N1, -SGN1, E(1,1), LDE, B(1,N1+1), LDB, SCALE, F(1,N1+1), LDF)
            CALL SGEMM("N", "N", M, N2, N1, -SGN2, F(1,1), LDF, D(1,N1+1), LDD, SCALE, F(1,N1+1), LDF)

            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N', 'T', SGN1, SGN2, M, N2, A, LDA, B(N1+1,N1+1), LDB, C, LDC, D(N1+1,N1+1),LDD, &
        ...
        ELSE
            ...
            ! E11
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N','T', SGN1, SGN2,  M1, N1, A(1,1), LDA, B(1,1), LDB, C(1,1), LDC, D(1,1), LDD, &
                & E(1,1), LDE, F(1,1), LDF, SCALE, WORK, INFO)

            ! E21
            CALL SGEMM("T", "N", M2, N1, M1, -ONE, A(1,M1+1), LDA, E(1,1), LDE, SCALE, E(M1+1,1), LDE)
            CALL SGEMM("T", "N", M2, N1, M1, -ONE, C(1,M1+1), LDC, F(1,1), LDF, SCALE, E(M1+1,1), LDE)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N','T', SGN1, SGN2,  M2, N1, A(M1+1,M1+1), LDA, B(1,1), LDB, C(M1+1,M1+1), LDC, &
                & D(1,1), LDD, E(M1+1,1), LDE, F(M1+1,1), LDF, SCAL, WORK, INFO)
            ...
            ! E12
            CALL SGEMM("N", "N", M1, N2, N1, -SGN1, E(1,1), LDE, B(1,N1+1), LDB, SCALE, F(1,N1+1), LDF)
            CALL SGEMM("N", "N", M1, N2, N1, -SGN2, F(1,1), LDF, D(1,N1+1), LDD, SCALE, F(1,N1+1), LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N','T', SGN1, SGN2,  M1, N2, A(1,1), LDA, B(N1+1,N1+1), LDB, C(1,1), LDC, &
            ...
            ! E22
            CALL SGEMM("T", "N", M2, N2, M1, -ONE, A(1,M1+1), LDA, E(1,N1+1), LDE, SCALE, E(M1+1,N1+1), LDE)
            CALL SGEMM("T", "N", M2, N2, M1, -ONE, C(1,M1+1), LDC, F(1,N1+1), LDE, SCALE, E(M1+1,N1+1), LDE)
            CALL SGEMM("N", "N", M2, N2, N1, -SGN1, E(M1+1,1), LDE, B(1,N1+1), LDB, ONE , F(M1+1,N1+1), LDF)
            CALL SGEMM("N", "N", M2, N2, N1, -SGN2, F(M1+1,1), LDF, D(1,N1+1), LDD, ONE , F(M1+1,N1+1), LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('N','T', SGN1, SGN2,  M2, N2, A(M1+1,M1+1), LDA, B(N1+1,N1+1), LDB, C(M1+1,M1+1), LDC, &
                & D(N1+1,N1+1), LDD,  E(M1+1,N1+1), LDE, F(M1+1,N1+1), LDF, SCAL, WORK, INFO)
            IF (SCAL.NE.ONE) THEN
                E(1:M, 1:N1) = SCAL * E(1:M, 1:N1)
                F(1:M, 1:N1) = SCAL * F(1:M, 1:N1)
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        IF ( N .LE. M/2 .OR. N .LE. BOUND ) THEN
            ...
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'N', SGN1, SGN2, M2, N, A(M1+1,M1+1), LDA, B, LDB, C(M1+1,M1+1), LDC, D,LDD, &
                & E(M1+1,1), LDE, F(M1+1,1), LDF,  SCALE, WORK, INFO )

            CALL SGEMM("N", "N", M1, N, M2, -ONE, A(1,M1+1), LDA, E(M1+1,1), LDE, SCALE, E(1,1), LDE)
            CALL SGEMM("N", "N", M1, N, M2, -ONE, C(1,M1+1), LDC, F(M1+1,1), LDF, SCALE, E(1,1), LDE)

            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'N', SGN1, SGN2, M1, N, A(1,1), LDA, B, LDB, C(1,1), LDC, D,LDD, &
                & E(1,1), LDE, F(1,1), LDF,  SCAL, WORK, INFO )
        ...
        ELSE IF ( M .LE. N/2  .OR. M .LE. BOUND ) THEN
            ...
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'N', SGN1, SGN2, M, N2, A, LDA, B(N1+1,N1+1), LDB, C, LDC, D(N1+1,N1+1),LDD, &
                & E(1,N1+1), LDE, F(1,N1+1), LDF,  SCALE, WORK, INFO )

            CALL SGEMM("N", "T", M, N1, N2, -SGN1, E(1,N1+1), LDE, B(1,N1+1), LDB, SCALE, F, LDF)
            CALL SGEMM("N", "T", M, N1, N2, -SGN2, F(1,N1+1), LDF, D(1,N1+1), LDD, SCALE, F, LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'N', SGN1, SGN2, M, N1, A, LDA, B, LDB, C, LDC, D,LDD, &
                & E(1,1), LDE, F(1,1), LDF,  SCAL, WORK, INFO )
        ...
        ELSE
            ...
            ! E22
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'N', SGN1, SGN2, M2, N2, A(M1+1,M1+1), LDA, B(N1+1,N1+1), LDB, C(M1+1,M1+1), LDC, &
            ...
            CALL SGEMM("N", "N", M1, N2, M2, -ONE, C(1,M1+1), LDC, F(M1+1,N1+1), LDF, SCALE, E(1,N1+1), LDE)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'N', SGN1, SGN2, M1, N2, A(1,1), LDA, B(N1+1,N1+1), LDB, C(1,1), LDC, &
                & D(N1+1,N1+1),LDD, E(1,N1+1), LDE, F(1,N1+1), LDF,  SCAL, WORK, INFO)
            ...
            ! E21
            CALL SGEMM("N", "T", M2, N1, N2, -SGN1, E(M1+1,N1+1), LDE, B(1,N1+1), LDB, SCALE, F(M1+1,1), LDF)
            CALL SGEMM("N", "T", M2, N1, N2, -SGN2, F(M1+1,N1+1), LDF, D(1,N1+1), LDD, SCALE, F(M1+1,1), LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'N', SGN1, SGN2, M2, N1, A(M1+1,M1+1), LDA, B(1,1), LDB, C(M1+1,M1+1), LDC, &
                & D(1,1),LDD, E(M1+1,1), LDE, F(M1+1,1), LDF,  SCAL, WORK, INFO )
            ...
            CALL SGEMM("N", "N", M1, N1, M2, -ONE, C(1,M1+1), LDC, F(M1+1,1), LDF, SCALE, E(1,1), LDE)
            CALL SGEMM("N", "T", M1, N1, N2, -SGN1, E(1,N1+1), LDE, B(1,N1+1), LDB, ONE, F(1,1), LDF)
            CALL SGEMM("N", "T", M1, N1, N2, -SGN2, F(1,N1+1), LDF, D(1,N1+1), LDD, ONE, F(1,1), LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'N', SGN1, SGN2, M1, N1, A(1,1), LDA, B(1,1), LDB, C(1,1), LDC, &
                & D(1,1),LDD, E(1,1), LDE, F(1,1), LDF,  SCAL, WORK, INFO)
            IF (SCAL.NE.ONE) THEN
                E(1:M, N1+1:N) = SCAL*E(1:M, N1+1:N)
                F(1:M, N1+1:N) = SCAL*F(1:M, N1+1:N)
    ...
    ELSE
        ...
        IF ( N .LE. M/2 .OR. N .LE. BOUND ) THEN
            ...
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'T', SGN1, SGN2, M2, N, A(M1+1,M1+1), LDA, B, LDB, C(M1+1,M1+1), LDC, D,LDD, &
                & E(M1+1,1), LDE, F(M1+1,1), LDF,  SCALE, WORK, INFO )

            CALL SGEMM("N", "N", M1, N, M2, -ONE, A(1,M1+1), LDA, E(M1+1,1), LDE, SCALE, E(1,1), LDE)
            CALL SGEMM("N", "N", M1, N, M2, -ONE, C(1,M1+1), LDC, F(M1+1,1), LDF, SCALE, E(1,1), LDE)

            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'T', SGN1, SGN2, M1, N, A(1,1), LDA, B, LDB, C(1,1), LDC, D,LDD, &
                & E(1,1), LDE, F(1,1), LDF,  SCAL, WORK, INFO )
        ...
        ELSE IF ( M .LE. N/2  .OR. M .LE. BOUND ) THEN
            ...
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'T', SGN1, SGN2, M, N1, A, LDA, B(1,1), LDB, C, LDC, D(1,1),LDD, &
                & E(1,1), LDE, F(1,1), LDF,  SCALE, WORK, INFO)

            CALL SGEMM("N", "N", M, N2, N1, -SGN1, E(1,1), LDE, B(1,N1+1), LDB, SCALE, F(1,N1+1), LDF)
            CALL SGEMM("N", "N", M, N2, N1, -SGN2, F(1,1), LDF, D(1,N1+1), LDD, SCALE, F(1,N1+1), LDF)

            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'T', SGN1, SGN2, M, N2, A, LDA, B(N1+1,N1+1), LDB, C, LDC, D(N1+1,N1+1),LDD, &
                & E(1,N1+1), LDE, F(1,N1+1), LDF,  SCAL, WORK, INFO  )
        ...
        ELSE
            ...
            ! E21
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'T', SGN1, SGN2, M2, N1, A(M1+1,M1+1), LDA, B, LDB, C(M1+1,M1+1), LDC, D,LDD, &
                & E(M1+1,1), LDE, F(M1+1,1), LDF,  SCALE, WORK, INFO  )
            ...
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'T', SGN1, SGN2, M1, N1, A, LDA, B, LDB, C, LDC, D,LDD, &
            ...
            ! E22
            CALL SGEMM("N","N", M2, N2, N1, -SGN1, E(M1+1,1), LDE, B(1,N1+1), LDB, SCALE, F(M1+1,N1+1), LDF)
            CALL SGEMM("N","N", M2, N2, N1, -SGN2, F(M1+1,1), LDF, D(1,N1+1), LDD, SCALE, F(M1+1,N1+1), LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'T', SGN1, SGN2, M2, N2, A(M1+1,M1+1), LDA, B(N1+1,N1+1), LDB, C(M1+1,M1+1), LDC, &
                & D(N1+1,N1+1),LDD, E(M1+1,N1+1), LDE, F(M1+1,N1+1), LDF,  SCAL, WORK, INFO )
            IF (SCAL.NE.ONE) THEN
                E(1:M, 1:N1) = SCAL*E(1:M, 1:N1)
                F(1:M, 1:N1) = SCAL*F(1:M, 1:N1)
            ...
            CALL SGEMM("N", "N", M1, N2, M2, -ONE, A(1,M1+1), LDA, E(M1+1,N1+1), LDE, SCALE, E(1,N1+1), LDE)
            CALL SGEMM("N", "N", M1, N2, M2, -ONE, C(1,M1+1), LDC, F(M1+1,N1+1), LDF, SCALE, E(1,N1+1), LDE)
            CALL SGEMM("N", "N", M1, N2, N1, -SGN1, E(1,1), LDE, B(1,N1+1), LDB, ONE, F(1,N1+1), LDF)
            CALL SGEMM("N", "N", M1, N2, N1, -SGN2, F(1,1), LDF, D(1,N1+1), LDD, ONE, F(1,N1+1), LDF)
            CALL SLA_TGCSYLV_DUAL_RECURSIVE('T', 'T', SGN1, SGN2, M1, N2, A, LDA, B(N1+1,N1+1), LDB, C, LDC, &
                & D(N1+1,N1+1),LDD, E(1,N1+1), LDE, F(1,N1+1), LDF,  SCAL, WORK, INFO )
            IF (SCAL.NE.ONE) THEN
                E(1:M, 1:N1) = SCAL*E(1:M, 1:N1)
                F(1:M, 1:N1) = SCAL*F(1:M, 1:N1)
