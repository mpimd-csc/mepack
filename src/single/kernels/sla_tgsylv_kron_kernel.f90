...
!  Definition:
!  ===========
!      SLA_TGSYLV_KRON_KERNEL(TRANSA, TRANSB, SGN, M, N, FA, LDA, FB, LDB, FC, LDC, FD, LDD, X, LDX, SCAL, INFO)
...
!> \param[in] FA
!> \verbatim
!>          FA is REAL array, dimension (LDA,M)
...
!> \param[in] FB
!> \verbatim
!>          FB is REAL array, dimension (LDB,N)
...
!> \param[in] FC
!> \verbatim
!>          FC is REAL array, dimension (LDC,M)
...
!> \param[in] FD
!> \verbatim
!>          FD is REAL array, dimension (LDD,N)
...
SUBROUTINE SLA_TGSYLV_KRON_KERNEL(TRANSA, TRANSB, SGN, M, N, FA, LDA, FB, LDB, FC, LDC, FD, LDD, X, LDX, SCAL, INFO)
    ...
    IF ( INFO .NE. 1 ) THEN
        ...
        IF ( INFO .NE. 0 ) THEN
            CALL XERBLA('SLA_TGSYLV_KRON_KERNEL',-INFO)
            RETURN
        END IF
    ELSE
        INFO = 0
    ...
    ! Solve
    CALL SGETC2X(DIMZ,Z,LDZ,IPIV,JPIV,IINFO)
    IF (IINFO .NE. 0 ) THEN
        INFO = 4
        RETURN
    END IF
    CALL SGESC2(DIMZ,Z,LDZ,RHS,IPIV,JPIV,LSCAL)
    IF (SCAL .NE. ONE) THEN
        INFO = 1
    ENDIF
    SCAL = LSCAL
