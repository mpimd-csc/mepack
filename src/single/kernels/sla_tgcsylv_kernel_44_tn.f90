...
!> \brief Solver for a 4x4 generalized coupled Sylvester equation (TRANSA = T, TRANSB = N)
!
!  Definition:
!  ===========
!
!       SUBROUTINE SLA_TGCSYLV_KERNEL_44TN (SGN1, SGN2, M, N, A, LDA, B, LDB, C, LDC, D, LDD, E, LDE, F, LDF, SCALE, INFO)
...
!> \verbatim
!>
!> SLA_TGCSYLV_KERNEL_44TN solves a generalized Sylvester equation of the following form
!>
!>    A**T * R  + SGN1 * L * B = SCALE * E                              (1)
!>    C**T * R  + SGN2 * L * D = SCALE * F                              (1)
!>
    ...
    DO WHILE ( K .LE. M )
        ...
        IF ( KH .LT. M ) THEN
            IF ( KB .EQ. 1 ) THEN
                ...
                SELECT CASE(N)
                CASE(1)
                    E(KE:M, 1) = E(KE:M, 1) - A(K,KE:M) * E(K,1)
                    F(KE:M, 1) = F(KE:M, 1) - A(K,KE:M) * E(K,1)
                CASE(2)
                    E(KE:M, 1) = E(KE:M, 1) - A(K,KE:M) * E(K,1)
                    F(KE:M, 1) = F(KE:M, 1) - C(K,KE:M) * E(K,1)
                    E(KE:M, 2) = E(KE:M, 2) - A(K,KE:M) * E(K,2)
                    F(KE:M, 2) = F(KE:M, 2) - C(K,KE:M) * E(K,2)
                CASE(3)
                    E(KE:M, 1) = E(KE:M, 1) - A(K,KE:M) * E(K,1)
                    F(KE:M, 1) = F(KE:M, 1) - C(K,KE:M) * E(K,1)
                    E(KE:M, 2) = E(KE:M, 2) - A(K,KE:M) * E(K,2)
                    F(KE:M, 2) = F(KE:M, 2) - C(K,KE:M) * E(K,2)
                    E(KE:M, 3) = E(KE:M, 3) - A(K,KE:M) * E(K,3)
                    F(KE:M, 3) = F(KE:M, 3) - C(K,KE:M) * E(K,3)
                CASE(4)
                    E(KE:M, 1) = E(KE:M, 1) - A(K,KE:M) * E(K,1)
                    F(KE:M, 1) = F(KE:M, 1) - C(K,KE:M) * E(K,1)
                    E(KE:M, 2) = E(KE:M, 2) - A(K,KE:M) * E(K,2)
                    ...
                    E(KE:M, 3) = E(KE:M, 3) - A(K,KE:M) * E(K,3)
                    F(KE:M, 3) = F(KE:M, 3) - C(K,KE:M) * E(K,3)
                    E(KE:M, 4) = E(KE:M, 4) - A(K,KE:M) * E(K,4)
                    F(KE:M, 4) = F(KE:M, 4) - C(K,KE:M) * E(K,4)
                END SELECT
            ELSE
                SELECT CASE(N)
                CASE(1)
                    E(KE:M, 1) = E(KE:M, 1) - A(K,KE:M) * E(K,1) - A(KH,KE:M) * E(KH,1)
                    F(KE:M, 1) = F(KE:M, 1) - C(K,KE:M) * E(K,1) - C(KH,KE:M) * E(KH,1)

                CASE(2)
                    E(KE:M, 1) = E(KE:M, 1) - A(K,KE:M) * E(K,1) - A(KH,KE:M) * E(KH,1)
                    F(KE:M, 1) = F(KE:M, 1) - C(K,KE:M) * E(K,1) - C(KH,KE:M) * E(KH,1)
                    E(KE:M, 2) = E(KE:M, 2) - A(K,KE:M) * E(K,2) - A(KH,KE:M) * E(KH,2)
                    F(KE:M, 2) = F(KE:M, 2) - C(K,KE:M) * E(K,2) - C(KH,KE:M) * E(KH,2)
                ...
                CASE(3)
                    ...
                    E(KE:M, 2) = E(KE:M, 2) - A(K,KE:M) * E(K,2) - A(KH,KE:M) * E(KH,2)
                    F(KE:M, 2) = F(KE:M, 2) - C(K,KE:M) * E(K,2) - C(KH,KE:M) * E(KH,2)
                    E(KE:M, 3) = E(KE:M, 3) - A(K,KE:M) * E(K,3) - A(KH,KE:M) * E(KH,3)
                    F(KE:M, 3) = F(KE:M, 3) - C(K,KE:M) * E(K,3) - C(KH,KE:M) * E(KH,3)
                ...
                CASE(4)
                    ...
                    E(KE:M, 3) = E(KE:M, 3) - A(K,KE:M) * E(K,3) - A(KH,KE:M) * E(KH,3)
                    F(KE:M, 3) = F(KE:M, 3) - C(K,KE:M) * E(K,3) - C(KH,KE:M) * E(KH,3)
                    E(KE:M, 4) = E(KE:M, 4) - A(K,KE:M) * E(K,4) - A(KH,KE:M) * E(KH,4)
                    F(KE:M, 4) = F(KE:M, 4) - C(K,KE:M) * E(K,4) - C(KH,KE:M) * E(KH,4)
