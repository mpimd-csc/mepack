...
!>
!> \verbatim
!> SLA_TRSYLV_DAG solves a Sylvester equation of the following forms
!>
...
SUBROUTINE SLA_TRSYLV_DAG ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    EXTERNAL ISLA_TRSYLV_SOLVE_BLOCK
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TRSYLV_DAG', -INFO)
        RETURN
    END IF
...
END SUBROUTINE
SUBROUTINE ISLA_TRSYLV_SOLVE_BLOCK(TRANSA, TRANSB, M,N,K,KH,KB,L,LH,LB, &
        & A, LDA, B, LDB, X, LDX, SGN, SCAL, INFO1)
    USE MEPACK_OPTIONS_ISOLVER
    USE MEPACK_OPTIONS_BLOCKSIZE_SINGLE
    IMPLICIT NONE
    INTEGER M, N, K, KH, KB, L, LH, LB, LDA, LDB, LDX, INFO1, MAXNB, ISOLVER
    REAL SGN, SCAL, A(LDA,*), B(LDB, *), X(LDX,*)
    CHARACTER TRANSA(1), TRANSB(1)
    REAL ONE, WORK(128,128)
    PARAMETER (ONE = 1.0)
