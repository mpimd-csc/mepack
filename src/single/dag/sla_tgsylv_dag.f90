...
SUBROUTINE SLA_TGSYLV_DAG ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, INFO)
    ...
    REAL SGN, SCALE
    INTEGER M, N, LDA, LDB, LDC, LDD, LDX, INFO
    REAL A(LDA, M), B(LDB, N) , C(LDC, M), D(LDD, N), X(LDX, N)
    REAL WORK(*)
    ...
    EXTERNAL ISLA_TGSYLV_SOLVE_BLOCK
    EXTERNAL LSAME
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGSYLV_DAG', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                CALL ISLA_TGSYLV_SOLVE_BLOCK(TRANSA, TRANSB, M,N, BLO, K,KH,KB,L,LH,LB,A,LDA,B,LDB,C,LDC,D,LDD,&
                ...
                IF ( K .GT. 1 ) THEN
                    ...
                    !$omp task firstprivate(KB, LB, K, L, WORKOFFSET) &
                    !$omp depend(in:X(K,L)) depend(out:WORK(WORKOFFSET)) default(shared)
                    CALL SGEMM('N', 'N', KB, LB, LB, ONE, X(K,L), LDX, D(L,L), LDD, ZERO, WORK(WORKOFFSET), KB)
                    !$omp end task
                END IF
                ...
                IF ( LH .LT. N  ) THEN
                    ...
                    !$omp task firstprivate(KB, LB, K, L, WORKOFFSET) &
                    !$omp depend(in:X(K,L)) depend(out:WORK(WORKOFFSET)) default(shared)
                    CALL SGEMM("N", "N", KB, LB, M-K+1, ONE, C(K,K), LDC, X(K,L), LDX, ZERO, WORK(WORKOFFSET), KB)
                    !$omp end task
                ...
                IF ( K .GT. 1 ) THEN
                    ...
                    DO WHILE (J .GT. 0)
                        ...
                        !$omp task firstprivate(J,K,L,JB,KB,LB,WORKOFFSET) default(shared) &
                        !$omp& depend(in:WORK(WORKOFFSET)) depend(inout:X(J,L))
                        CALL SGEMM('N', 'N', JB, LB, KB, -SGN*ONE, C(J,K), LDC, WORK(WORKOFFSET), KB, ONE, X(J,L), LDX)
                        !$omp end task
                ...
                IF ( LH .LT. N  ) THEN
                    ...
                    DO WHILE (JH .LE. N)
                        ...
                        !$omp task firstprivate(J,K,L,JB,KB,LB,WORKOFFSET) default(shared) &
                        !$omp& depend(in:WORK(WORKOFFSET)) depend(inout:X(K,J))
                        CALL SGEMM("N", "N", KB, JB , LB, -SGN*ONE, WORK(WORKOFFSET), KB, D(L,J), LDD, ONE, X(K,J), LDX)
                        !$omp end task
                        JH = JH + 1
                    END DO
                END IF
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( K .GT. 1 ) THEN
                    ...
                    !$omp task firstprivate(KB, LB, K, L, WORKOFFSET) &
                    !$omp depend(in:X(K,L)) depend(out:WORK(WORKOFFSET)) default(shared)
                    CALL SGEMM('N', 'T', KB, LB, LB, ONE, X(K,L), LDX, D(L,L), LDD, ZERO, WORK(WORKOFFSET), KB)
                    !$omp end task
                ...
                IF ( L .GT. 1 ) THEN
                    ...
                    DO WHILE (J .GT. 0)
                        ...
                        !$omp task firstprivate(J,K,L,JB,KB,LB,WORKOFFSET) default(shared) &
                        !$omp& depend(in:WORK(WORKOFFSET)) depend(inout:X(K,J))
                        CALL SGEMM('N', 'T', KB, JB, LB, -SGN*ONE, WORK(WORKOFFSET), KB, D(J,L), LDD, ONE, X(K,J), LDX)
                        !$omp end task
                        J = J - 1
                    END DO
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE ( KH .LE. M )
                ...
                IF ( LH .LT. N ) THEN
                    ...
                    !$omp task firstprivate(KB, LB, K, L, WORKOFFSET, KH ) &
                    !$omp depend(in:X(K,L)) depend(out:WORK(WORKOFFSET)) default(shared)
                    CALL SGEMM("T", "N", KB, LB, KH, ONE, C(1,K), LDC, X(1,L), LDX, ZERO, WORK(WORKOFFSET), KB)
                    !$omp end task
                ...
                IF ( KH  .LT. M ) THEN
                    ...
                    DO WHILE (JH .LE. M)
                        ...
                        !$omp task firstprivate(J,K,L,JB,KB,LB,WORKOFFSET) default(shared) &
                        !$omp& depend(in:WORK(WORKOFFSET)) depend(inout:X(J,L))
                        CALL SGEMM("T", "N", JB, LB, KB, -SGN*ONE, C(K,J), LDC, WORK(WORKOFFSET), KB, ONE, X(J, L), LDX)
                        !$omp end task
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE ( KH .LE. M )
                ...
                IF ( KH .LT. M  ) THEN
                    ...
                    !$omp task firstprivate(KB, LB, K, L, WORKOFFSET) &
                    !$omp depend(in:X(K,L)) depend(out:WORK(WORKOFFSET)) default(shared)
                    CALL SGEMM('N', 'T', KB, LB, LB, ONE, X(K,L), LDX, D(L,L), LDB, ZERO, WORK(WORKOFFSET), KB)
                    !$omp end task
...
!> \internal
SUBROUTINE ISLA_TGSYLV_SOLVE_BLOCK(TRANSA, TRANSB, M,N,BLO, K,KH,KB,L,LH,LB,A,LDA,B,LDB,C,LDC,D,LDD,X,LDX,SGN,SCAL, &
        & WORK, INFO1 )
    USE MEPACK_OPTIONS_ISOLVER
    USE OMP_LIB
    USE MEPACK_OPTIONS_BLOCKSIZE_SINGLE
    ...
    REAL A(LDA,M), B(LDB,N), C(LDC, M), D(LDD,N),  X(LDX,N), SGN, SCAL, WORK(*)
