...
!  Definition:
!  ===========
!       SUBROUTINE SLA_TGCSYLV_DAG ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
SUBROUTINE SLA_TGCSYLV_DAG ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGCSYLV_DAG', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                CALL ISLA_TGCSYLV_BLOCK(TRANSA, TRANSB, SGN1, SGN2, M, N, K, L, KB, LB, A, LDA, B, LDB, C, LDC, D, LDD, &
                ...
                IF ( K .GT. 1 ) THEN
                    ...
                    DO WHILE (J .GT. 0)
                        ...
                        CALL SGEMM('N', 'N', JB , LB, KB, -ONE, C(J,K), LDC, E(K,L), LDE, ONE, F(J,L), LDF)
                        !$omp end task
                ...
                IF ( LH .LT. N  ) THEN
                    ...
                    DO WHILE ( JH .LE. N )
                        ...
                        CALL SGEMM("N", "N", KB, JB, LB, -ONE*SGN1, F(K,L), LDF, B(L,J), LDB, ONE, E(K,J), LDE)
                        !$omp end task
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( L .GT. 1 ) THEN
                    ...
                    DO WHILE (J .GT. 0)
                        ...
                        CALL SGEMM('N', 'T', KB, JB, LB, -ONE*SGN1, F(K,L), LDF, B(J,L), LDB, ONE, E(K,J), LDE)
                        !$omp end task
            ...
            DO WHILE ( KH .LE. M )
                ...
                IF ( KH  .LT. M ) THEN
                    ...
                    DO WHILE ( JH .LE. M )
                        ...
                        CALL SGEMM("T", "N", JB, LB, KB, -ONE, C(K,J), LDC, E(K,L), LDE, ONE, F(J, L), LDF)
                        !$omp end task
...
SUBROUTINE ISLA_TGCSYLV_BLOCK(TRANSA, TRANSB, SGN1, SGN2, M, N ,K, L, KB, LB, A, LDA, B, LDB, C, LDC, D, LDD, &
        & E, LDE, F, LDF, SCAL, INFO1)
    USE MEPACK_OPTIONS_ISOLVER
    USE MEPACK_OPTIONS_BLOCKSIZE_SINGLE
    IMPLICIT NONE
