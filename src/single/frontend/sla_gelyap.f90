...
!>
!> \verbatim
!> SLA_GELYAP solves a Lyapunov equation of the following forms
!>
!>    A  * X  +  X * A**T = SCALE * Y                              (1)
!>
!> or
...
SUBROUTINE SLA_GELYAP(FACT, TRANS, M , A, LDA, Q, LDQ, X, LDX, SCALE, WORK, LDWORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_GELYAP', -INFO)
        RETURN
    END IF
    ...
    IF (.NOT. BFACT) THEN
        ...
        IF ( AHESS ) THEN
            ASHAPE = 'H'
        END IF
        CALL SHGEES(ASHAPE, "Vectors", "NoSort", DUMMY, M, A, LDA, SDIM, WORK(1), WORK(M+1), Q, &
