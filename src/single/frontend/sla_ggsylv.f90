...
SUBROUTINE SLA_GGSYLV(FACTA, FACTB, TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF (.NOT. BFACTA) THEN
        ...
        IF ( AHESS ) THEN
            ASHAPE = 'H'
        END IF
        CALL SHGGES(ASHAPE, "Vectors", "Vectors", "NoSort", DUMMY, M, A, LDA, C, LDC, SDIM, WORK(1), WORK(M+1), &
    ...
    IF (.NOT. BFACTB) THEN
        ...
        IF ( BHESS ) THEN
            BSHAPE = 'H'
        END IF
        CALL SHGGES(BSHAPE, "Vectors", "Vectors", "NoSort", DUMMY, N, B, LDB, D, LDD, SDIM, WORK(1), WORK(N+1), &
