...
!  ===========
!
!   SUBROUTINE SLA_GESYLV(FACTA, FACTB, TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, QA, LDQA, QB, LDQB,
...
SUBROUTINE SLA_GESYLV(FACTA, FACTB, TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, QA, LDQA, QB, LDQB, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_GESYLV', -INFO)
        RETURN
    END IF
    ...
    SELECT CASE(SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL SLA_TRSYLV_L3(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO )
        CASE (FRONTEND_SOLVER_LEVEL2)
            SELECT CASE(ISOLVER)
                CASE(1)
                    IF ( MAXMN .LE. 32 ) THEN
                        CALL SLA_TRSYLV_L2_LOCAL_COPY_32(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 64) THEN
                        CALL SLA_TRSYLV_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 96) THEN
                        CALL SLA_TRSYLV_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 128) THEN
                        CALL SLA_TRSYLV_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB,X, LDX, SCALE, WORK, IINFO)
                    ELSE
                        CALL SLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    END IF
                CASE(2)
                    IF ( M .GT. 128 .OR. N .GT. 128) THEN
                        CALL SLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE
                        CALL SLA_TRSYLV_L2_LOCAL_COPY(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    END IF
                CASE(3)
                    CALL SLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                CASE(4)
                    CALL SLA_TRSYLV_L2(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                CASE(5)
                    CALL SLA_TRSYLV_L2_UNOPT(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                CASE(6)
                    CALL SLA_TRSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB,  X, LDX, SCALE, WORK, IINFO)

            END SELECT
        CASE (FRONTEND_SOLVER_DAG)
            CALL SLA_TRSYLV_DAG(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_2STAGE)
            CALL SLA_TRSYLV_L3_2S(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
    ...
    SELECT CASE(SOLVER)
        ...
        CASE (FRONTEND_SOLVER_RECURSIVE)
            CALL SLA_TRSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_LAPACK)
            CALL STRSYL(TRANSA, TRANSB, ISGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, IINFO)
    END SELECT
    IF ( IINFO .NE. 0) THEN
        INFO = 3
        RETURN
