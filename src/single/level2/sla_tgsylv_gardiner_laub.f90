...
!  ===========
!
!       SUBROUTINE SLA_TGSYLV_GARDINER_LAUB ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, &
...
!> \ingroup sgltgsylv
!
SUBROUTINE SLA_TGSYLV_GARDINER_LAUB ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGSYLV_GARDINER_LAUB', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( K <= N)
            ...
            IF ( LTMP  .OR. (K .EQ. N)) THEN
                ...
                DO I = 1, M
                    CALL SCOPY(M, A(1,I), IONE, WORK(1,I), IONE)
                    CALL SSCAL(M, B(K,K), WORK(1,I), IONE)
                    CALL SAXPY(M, SGN*D(K,K), C(1,I), IONE, WORK(1,I), IONE)
                END DO
                ...
                DO WHILE ( L < M )
                    IF ( WORK(L+1,L) .NE. ZERO ) THEN
                        AA = WORK(L,L)
                        BB = WORK(L+1,L)
                        CALL SROTG(AA,BB,CC,SS)
                        CALL SROT(M,WORK(L,L),2*M,WORK(L+1,L),2*M,CC,SS)
                        CALL SROT(IONE,X(L,K),IONE,X(L+1,K),IONE,CC,SS)
                        L = L +2
                    ELSE
                        L = L +1
                    END IF
                END DO
                CALL STRSV("U","N","N",M,WORK,2*M,X(1,K),IONE)


                CALL SGEMV("N",M,M,ONE,A,LDA, X(1,K),IONE, ZERO, WORK(1,1),IONE)
                CALL SGEMV("N",M,M,SGN,C,LDA, X(1,K),IONE, ZERO, WORK(1,2),IONE)

                DO L = K+1,N
                    CALL SAXPY(M,-B(K,L),WORK(1,1),IONE,X(1,L),IONE)
                    CALL SAXPY(M,-D(K,L),WORK(1,2),IONE,X(1,L),IONE)
                END DO
                K = K + 1
            ELSE
                ...
                DO L = 1, M
                    CALL SCOPY(M,A(1,L),IONE,WORK(1,2*L-1),ITWO)
                    CALL SSCAL(M,B(K,K),WORK(1,2*L-1),ITWO)
                    CALL SAXPY(M,SGN*D(K,K),C(1,L),IONE,WORK(1,2*L-1),ITWO)

                    CALL SCOPY(M,A(1,L),IONE,WORK(1,2*L),ITWO)
                    CALL SSCAL(M,B(K+1,K),WORK(1,2*L),ITWO)
                    CALL SAXPY(M,SGN*D(K+1,K),C(1,L),IONE,WORK(1,2*L),ITWO)

                    IF ( B(K+1,K) .NE. ZERO ) THEN
                        CALL SCOPY(M,A(1,L),IONE,WORK(2,2*L-1),ITWO)
                        CALL SSCAL(M,B(K,K+1),WORK(2,2*L-1),ITWO)
                    ELSE
                        CALL SCOPY(M,C(1,L),IONE,WORK(2,2*L-1),ITWO)
                        CALL SSCAL(M,SGN*D(K,K+1),WORK(2,2*L-1),ITWO)
                    END IF

                    CALL SCOPY(M,A(1,L),IONE,WORK(2,2*L),ITWO)
                    CALL SSCAL(M,B(K+1,K+1),WORK(2,2*L),ITWO)
                    CALL SAXPY(M,SGN*D(K+1,K+1),C(1,L),IONE,WORK(2,2*L),ITWO)
                END DO


                CALL SCOPY(M, X(1,K), IONE, WORK(1,2*M+1),ITWO)
                CALL SCOPY(M, X(1,K+1),IONE, WORK(2,2*M+1),ITWO)
                ...
                DO WHILE ( L >= 1  )
                    ...
                    IF ( LTMP  ) THEN
                        ...
                        CALL SGEMV("N",L-4, 4, -ONE, WORK(1,L-3),2*M,WORK(L-3,2*M+1), &
                    ...
                    ELSE
                        ...
                        CALL SLA_SMALL_SOLVE4( 2, MAT, RHS, IINFO, MACHINE_PRECISION, SMALL_NUMBER)
                        WORK(L-1:L,2*M+1) = RHS(1:2)
                        IF ( IINFO .NE. 0 )  INFO = 100


                        CALL SGEMV("N",L-2,2, -ONE, WORK(1,L-1),2*M,WORK(L-1,2*M+1), &
                            & IONE,ONE,WORK(1,2*M+1),IONE)
                        L = L - 2
                    END IF
                END DO
                ...
                CALL SCOPY(M, WORK(1,2*M+1),ITWO, X(1,K), IONE)
                CALL SCOPY(M, WORK(2,2*M+1),ITWO, X(1,K+1),IONE)

                ! Precompute the Update January 2021
                CALL SGEMV("N",M,M,ONE,A,LDA, X(1,K),IONE, ZERO, WORK(1,1),IONE)
                CALL SGEMV("N",M,M,SGN,C,LDA, X(1,K),IONE, ZERO, WORK(M+1,1),IONE)
                CALL SGEMV("N",M,M,ONE,A,LDA, X(1,K+1),IONE, ZERO, WORK(1,2),IONE)
                CALL SGEMV("N",M,M,SGN,C,LDA, X(1,K+1),IONE, ZERO, WORK(M+1,2),IONE)

                DO L = K+2,N
                    CALL SAXPY(M,-B(K,L),WORK(1,1),IONE, X(1,L),IONE)
                    CALL SAXPY(M,-D(K,L),WORK(M+1,1),IONE, X(1,L),IONE)

                    CALL SAXPY(M,-B(K+1,L),WORK(1,2),IONE, X(1,L),IONE)
                    CALL SAXPY(M,-D(K+1,L),WORK(M+1,2),IONE, X(1,L),IONE)
                END DO
        ...
        DO WHILE ( K <= N)
            ...
            IF ( LTMP  .OR. (K .EQ. N)) THEN
                ...
                DO WHILE ( L < M )
                    IF ( WORK(L+1,L) .NE. ZERO ) THEN
                        AA = WORK(L,L)
                        BB = WORK(L+1,L)
                        CALL SROTG(AA,BB,WORK(2*L-1,2*M+1),WORK(2*L,2*M+1))
                        CALL SROT(M,WORK(L,L),2*M,WORK(L+1,L),2*M,WORK(2*L-1,2*M+1),WORK(2*L,2*M+1))
                        WORK(L+1,L) = ONE
                        L = L +2
                    ELSE
                        L = L +1
                    END IF
                END DO
                CALL STRSV("U","T","N",M,WORK,2*M,X(1,K),IONE)

                L = 1
                DO WHILE ( L < M )
                    IF ( WORK(L+1,L) .EQ. ONE ) THEN
                        CALL SROT(IONE,X(L,K),IONE,X(L+1,K),IONE,WORK(2*L-1,2*M+1),-WORK(2*L,2*M+1))
                        L = L +2
                    ELSE
                        L = L +1
                    END IF
                ...
                CALL SGEMV("T",M,M,ONE, A, LDA, X(1,K),IONE, ZERO, WORK(1,1),IONE)
                CALL SGEMV("T",M,M,SGN, C, LDA, X(1,K),IONE, ZERO, WORK(1,2),IONE)
                DO L = K+1,N
                    CALL SAXPY(M,-B(K,L),WORK(1,1),IONE,X(1,L),IONE)
                    CALL SAXPY(M,-D(K,L),WORK(1,2),IONE,X(1,L),IONE)
                END DO
            ...
            ELSE
                ...
                DO L = 1, M
                    ...
                    CALL SCOPY(M,A(1,L),IONE,WORK(1,2*L),ITWO)
                    CALL SSCAL(M,B(K,K+1),WORK(1,2*L),ITWO)
                    CALL SAXPY(M,SGN*D(K,K+1),C(1,L),IONE,WORK(1,2*L),ITWO)

                    IF ( B(K+1,K) .NE. ZERO ) THEN
                        CALL SCOPY(M,A(1,L),IONE,WORK(2,2*L-1),ITWO)
                        CALL SSCAL(M,B(K+1,K),WORK(2,2*L-1),ITWO)
                    ELSE
                        CALL SCOPY(M,C(1,L),IONE,WORK(2,2*L-1),ITWO)
                        CALL SSCAL(M,SGN*D(K+1,K),WORK(2,2*L-1),ITWO)
                    END IF
                ...
                DO WHILE ( L <= 2*M  )
                    IF ( (L+3 <= 2*M) .AND. ( WORK(L+2,L) .NE. ZERO) ) THEN
                        ...
                        IF ( L+4 .LE. 2*M ) THEN
                            CALL SGEMV("T",4, 2*M-L-3, -ONE, WORK(L,L+4),2*M,WORK(L,2*M+1), &
                    ...
                    ELSE
                        ...
                        IF ( IINFO .NE. 0 )  INFO = 100
                        CALL SGEMV("T",2, 2*M-L-1, -ONE, WORK(L,L+2),2*M,WORK(L,2*M+1), &
                ...
                ! Precompute the Update January 2021
                CALL SGEMV("T",M,M,ONE,A,LDA, X(1,K),IONE, ZERO, WORK(1,1),IONE)
                CALL SGEMV("T",M,M,SGN,C,LDA, X(1,K),IONE, ZERO, WORK(M+1,1),IONE)
                CALL SGEMV("T",M,M,ONE,A,LDA, X(1,K+1),IONE, ZERO, WORK(1,2),IONE)
                CALL SGEMV("T",M,M,SGN,C,LDA, X(1,K+1),IONE, ZERO, WORK(M+1,2),IONE)
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( K >= 1 )
            ...
            IF ( K .EQ. 1 .OR. LTMP ) THEN
                ...
                !Update January 2021
                DO L = 1,K-1
                    CALL SAXPY(M,-B(L,K),WORK(1,1),IONE,X(1,L),IONE)
                    CALL SAXPY(M,-D(L,K),WORK(1,2),IONE,X(1,L),IONE)
                END DO
                K = K - 1
            ELSE
                WORK(1:2*M, 1:2*M+1) = ZERO
                ...
                DO L = 1, M
                    CALL SCOPY(M,A(1,L),IONE,WORK(1,2*L-1),ITWO)
                    CALL SSCAL(M,B(K-1,K-1),WORK(1,2*L-1),ITWO)
                    CALL SAXPY(M,SGN*D(K-1,K-1),C(1,L),IONE,WORK(1,2*L-1),ITWO)
                    ...
                    CALL SCOPY(M,A(1,L),IONE,WORK(2,2*L),ITWO)
                    CALL SSCAL(M,B(K,K),WORK(2,2*L),ITWO)
                    CALL SAXPY(M,SGN*D(K,K),C(1,L),IONE,WORK(2,2*L),ITWO)

                END DO

                CALL SCOPY(M, X(1,K-1), IONE, WORK(1,2*M+1),ITWO)
                CALL SCOPY(M, X(1,K),IONE, WORK(2,2*M+1),ITWO)
                ...
                CALL SCOPY(M, WORK(1,2*M+1),ITWO, X(1,K-1), IONE)
                CALL SCOPY(M, WORK(2,2*M+1),ITWO, X(1,K),IONE)
                ...
                DO L = 1,K-2
                    CALL SAXPY(M,-B(L,K),  WORK(1,1),IONE,X(1,L),IONE)
                    CALL SAXPY(M,-D(L,K),  WORK(M+1,1),IONE, X(1,L),IONE)

                    CALL SAXPY(M,-B(L,K-1),WORK(1,2),IONE, X(1,L),IONE)
                    CALL SAXPY(M,-D(L,K-1),WORK(M+1,2),IONE, X(1,L),IONE)
                END DO
