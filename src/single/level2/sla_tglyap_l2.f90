...
!> \brief Level-2 Bartels-Stewart Algorithm for the generalized Lyapunov equation
!
!  Definition:
!  ===========
!
...
SUBROUTINE SLA_TGLYAP_L2 ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGLYAP_L2', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( LE .GT. 0 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    ...
                    CALL STRMV('Upper', 'NoTrans','NoUnit', LE, A(1,1), LDA, WORK, IONE)
                    DO IT = 1,LE-1
                    ...
                    CALL SSYR2 ("U", LE, -ONE, WORK, IONE, B(1,L), IONE, X(1,1), LDX)

                    WORK(1:LE) = X(1:LE,L)
                    CALL STRMV('Upper', 'NoTrans','NoUnit', LE, B(1,1), LDB, WORK, IONE)
                    CALL SSYR2 ("U", LE, -ONE, WORK, IONE, A(1,L), IONE, X(1,1), LDX)

                ELSE
                    WORK(1:LE) = X(1:LE,L)
                    WORK(M+1:M+LE) = X(1:LE,LH)
                    CALL STRMV('Upper', 'NoTrans','NoUnit', LE, A, LDA, WORK, IONE)
                    CALL STRMV('Upper', 'NoTrans','NoUnit', LE, A, LDA, WORK(M+1), IONE)
                    ...
                    CALL SSYR2 ("U", LE, -ONE, WORK, IONE, B(1,L), IONE, X(1,1), LDX)
                    CALL SSYR2 ("U", LE, -ONE, WORK(M+1), IONE, B(1,LH), IONE, X(1,1), LDX)

                    WORK(1:LE) = X(1:LE,L)
                    WORK(M+1:M+LE) = X(1:LE,LH)

                    CALL STRMV('Upper', 'NoTrans','NoUnit', LE, B, LDB, WORK, IONE)
                    CALL STRMV('Upper', 'NoTrans','NoUnit', LE, B, LDB, WORK(M+1), IONE)

                    CALL SSYR2 ("U", LE, -ONE, WORK, IONE, A(1,L), IONE, X(1,1), LDX)
                    CALL SSYR2 ("U", LE, -ONE, WORK(M+1), IONE, A(1,LH), IONE, X(1,1), LDX)
        ...
        DO WHILE ( L .LE. M )
            ...
            IF ( LH .LT. M ) THEN
                IF ( LB .EQ. IONE ) THEN
                    ...
                    CALL STRMV("Upper", "Trans", "NoUnit", M-LH, A(LH+1,LH+1), LDA, WORK, IONE)
                    DO IT = LH+1, M-1
                        IF (A(IT+1,IT).NE.ZERO) THEN
                    ...
                    WORK(2*M+1:2*M+M-LH) = B(L,LH+1:M)
                    CALL SSYR2("L", M-LH, -ONE, WORK, IONE, WORK(2*M+1), IONE, X(LH+1, LH+1), LDX)
                    ! CALL SSYR2("L", M-LH, -ONE, WORK, IONE, B(L,LH+1), LDB, X(LH+1, LH+1), LDX)

                    WORK(1:M-LH) = X(LH+1:M, L)
                    CALL STRMV("Upper", "Trans", "NoUnit", M-LH, B(LH+1,LH+1), LDB, WORK, IONE)

                    WORK(2*M+1:2*M+M-LH) = A(L,LH+1:M)
                    CALL SSYR2("L", M-LH, -ONE, WORK(2*M+1), IONE, WORK, IONE, X(LH+1,LH+1), LDX)
                    ! CALL SSYR2("L", M-LH, -ONE, A(L,LH+1), LDA, WORK, IONE, X(LH+1,LH+1), LDX)

                ELSE
                    WORK(1:M-LH) = X(LH+1:M, L)
                    WORK(M+1:M+M-LH) = X(LH+1:M, LH)
                    CALL STRMV("Upper", "Trans", "NoUnit", M-LH, A(LH+1,LH+1), LDA, WORK, IONE)
                    CALL STRMV("Upper", "Trans", "NoUnit", M-LH, A(LH+1,LH+1), LDA, WORK(M+1), IONE)
                    ...
                    WORK(2*M+1:2*M+M-LH) = B(L,LH+1:M)
                    WORK(3*M+1:3*M+M-LH) = B(LH,LH+1:M)
                    CALL SSYR2("L", M-LH, -ONE, WORK, IONE, WORK(2*M+1), IONE, X(LH+1, LH+1), LDX)
                    CALL SSYR2("L", M-LH, -ONE, WORK(M+1), IONE, WORK(3*M+1), IONE, X(LH+1, LH+1), LDX)

                    ! CALL SSYR2("L", M-LH, -ONE, WORK, IONE, B(L,LH+1), LDB, X(LH+1, LH+1), LDX)
                    ! CALL SSYR2("L", M-LH, -ONE, WORK(M+1), IONE, B(LH,LH+1), LDB, X(LH+1, LH+1), LDX)

                    WORK(1:M-LH) = X(LH+1:M, L)
                    WORK(M+1:M+M-LH) = X(LH+1:M, LH)

                    CALL STRMV("Upper", "Trans", "NoUnit", M-LH, B(LH+1,LH+1), LDB, WORK, IONE)
                    CALL STRMV("Upper", "Trans", "NoUnit", M-LH, B(LH+1,LH+1), LDB, WORK(M+1), IONE)

                    WORK(2*M+1:2*M+M-LH) = A(L,LH+1:M)
                    WORK(3*M+1:3*M+M-LH) = A(LH,LH+1:M)
                    CALL SSYR2("L", M-LH, -ONE, WORK(2*M+1), IONE, WORK, IONE, X(LH+1,LH+1), LDX)
                    CALL SSYR2("L", M-LH, -ONE, WORK(3*M+1), IONE, WORK(M+1), IONE, X(LH+1,LH+1), LDX)

                    ! CALL SSYR2("L", M-LH, -ONE, A(L,LH+1), LDA, WORK, IONE, X(LH+1,LH+1), LDX)
                    ! CALL SSYR2("L", M-LH, -ONE, A(LH,LH+1), LDA, WORK(M+1), IONE, X(LH+1,LH+1), LDX)
