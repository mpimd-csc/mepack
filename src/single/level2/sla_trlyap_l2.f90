...
SUBROUTINE SLA_TRLYAP_L2 ( TRANSA, M, A, LDA, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TRLYAP_L2', -INFO)
        RETURN
    END IF
            ...
            IF ( LH .LT. M ) THEN
                IF ( M-LH .LE. 256) THEN
                    IF ( LB .EQ. IONE ) THEN
                        ...
                        CALL SSYR2("L", M-LH, -ONE, X(LH+1,L), IONE, WORKI, IONE, X(LH+1, LH+1), LDX)
                    ELSE
                        WORKI(1:M-LH) = A(L,LH+1:M)
                        CALL SSYR2("L", M-LH, -ONE, X(LH+1,L), IONE, WORKI, IONE, X(LH+1, LH+1), LDX)
                        WORKI(1:M-LH) = A(LH,LH+1:M)
                        CALL SSYR2("L", M-LH, -ONE, X(LH+1,LH),IONE, WORKI, IONE, X(LH+1, LH+1), LDX)
                    ENDIF

                ELSE
                    IF ( LB .EQ. IONE ) THEN
                        CALL SSYR2("L", M-LH, -ONE, X(LH+1,L), IONE, A(L,LH+1), LDA, X(LH+1, LH+1), LDX)
                    ELSE
                        CALL SSYR2("L", M-LH, -ONE, X(LH+1,L), IONE, A(L,LH+1), LDA, X(LH+1, LH+1), LDX)
                        CALL SSYR2("L", M-LH, -ONE, X(LH+1,LH),IONE, A(LH,LH+1), LDA, X(LH+1, LH+1), LDX)
                    ENDIF
                END IF
            END IF
