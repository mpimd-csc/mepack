...
SUBROUTINE SLA_TGSTEIN_L2 ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGSTEIN_L2', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( LE .GT. 0 ) THEN
                IF (LB.EQ.1) THEN
                    ...
                    CALL SGEMV("N", LE, LE, ONE, B(1,1),  LDB, X(1,L), IONE, ZERO, WORK, IONE)
                    CALL SSYR2("U", LE, ONE, WORK,IONE, B(1, L), IONE, X(1,1), LDX)

                    WORK(1:LE) = X(L,L) * B(1:LE,L)
                    CALL SGER(LE, LE, ONE, WORK, IONE, B(1,L),IONE, X(1,1), LDX)
                ...
                ELSE
                    ...
                    CALL SGEMV("N", LE, LE, ONE, B(1,1),  LDB, X(1,LH), IONE, ZERO, WORK, IONE)
                    CALL SSYR2("U", LE, ONE, WORK,IONE, B(1, LH), IONE, X(1,1), LDX)

                    WORK(1:LE) = X(L,L) * B(1:LE,L) + X(L,LH) * B(1:LE, LH)
                    CALL SGER(LE, LE, ONE, WORK, IONE, B(1,L),IONE, X(1,1), LDX)

                    WORK(1:LE) = X(L,LH) * B(1:LE,L) + X(LH,LH) * B(1:LE, LH)
                    CALL SGER(LE, LE, ONE, WORK, IONE, B(1,LH),IONE, X(1,1), LDX)
        ...
        DO WHILE ( L .LE. M )
            ...
            IF ( LH .LT. M ) THEN
                IF (LB .EQ. 1 ) THEN
                    ...
                    CALL SGEMV("T", M-LH, M-LH, ONE, B(LH+1,LH+1), LDB, X(L,LH+1), LDX, ZERO, WORK, IONE)
                ...
                ELSE
                    ...
                    !
                    !     CALL SGEMV("T", M-LH, M-LH, ONE, B(LH+1,LH+1), LDB, X(L,LH+1), LDB, ZERO, WORK, IONE)
                    !     CALL SSYR2("L", M-LH, ONE, WORKA, IONE, WORK, IONE, X(LH+1,LH+1), LDX)
                    !     CALL SGEMV("T", M-LH, M-LH, ONE, B(LH+1,LH+1), LDB, X(LH,LH+1), LDB, ZERO, WORK, IONE)
                    !     CALL SSYR2("L", M-LH, ONE, WORKB, IONE, WORK, IONE, X(LH+1,LH+1), LDX)
                    !
                    !
                    !     WORKC(1:I) = X(L,L) * WORKA(1:I) + X(LH, L) * WORKB(1:I)
                    ...
                    ! ELSE
                        ...
                        CALL SGER(I, I, ONE, B(L,LH+1), LDB, WORK(1), IONE, X(LH+1, LH+1), LDX)
                        CALL SGER(I, I, ONE, B(LH,LH+1), LDB, WORK(M+1), IONE, X(LH+1, LH+1), LDX)
