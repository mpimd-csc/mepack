...
!  Definition:
!  ===========
!       SUBROUTINE SLA_TGCSYLV_L2 ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
SUBROUTINE SLA_TGCSYLV_L2 ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGCSYLV_L2', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            IF ( K .GT. 1 ) THEN
                IF ( KB .EQ. IONE ) THEN
                    CALL SGER(KE, N, -ONE, A(1,K) , IONE, E(K,1), LDE, E(1,1), LDE)
                    CALL SGER(KE, N, -ONE, C(1,K) , IONE, E(K,1), LDE, F(1,1), LDF)

                ELSE
                    CALL SGER(KE, N, -ONE, A(1,K) ,  IONE, E(K,1), LDE, E(1,1), LDE)
                    CALL SGER(KE, N, -ONE, A(1,KH) , IONE, E(KH,1), LDE, E(1,1), LDE)
                    CALL SGER(KE, N, -ONE, C(1,K) ,  IONE, E(K,1), LDE, F(1,1), LDF)
                    CALL SGER(KE, N, -ONE, C(1,KH) , IONE, E(KH,1), LDE, F(1,1), LDF)
                END IF
            END IF
            K = K - 1
        END DO
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( K .LE. M )
            ...
            IF ( KH .LT. M ) THEN
                IF ( KB.EQ.1 ) THEN
                    CALL SGER(M-KH, N, -ONE, A(K,KH+1), LDA, E(K,1), LDE, E(KH+1, 1), LDE)
                    CALL SGER(M-KH, N, -ONE, C(K,KH+1), LDC, E(K,1), LDE, F(KH+1, 1), LDF)


                ELSE
                    CALL SGER(M-KH, N, -ONE, A(K,KH+1), LDA, E(K,1), LDE, E(KH+1, 1), LDE)
                    CALL SGER(M-KH, N, -ONE, A(KH,KH+1), LDA, E(KH,1), LDE, E(KH+1, 1), LDE)
                    CALL SGER(M-KH, N, -ONE, C(K,KH+1), LDC, E(K,1), LDE, F(KH+1, 1), LDF)
                    CALL SGER(M-KH, N, -ONE, C(KH,KH+1), LDC, E(KH,1), LDE, F(KH+1, 1), LDF)
