...
!>  \li Replaced all BLAS calls except of SGER by Fortran intrinsic,
...
SUBROUTINE SLA_TGCSYLV_DUAL_L2 ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGCSYLV_DUAL_L2', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( L .GT. 1 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL SGER(M, LE, -SGN1*ONE, E(1,L), IONE, B(1,L), IONE, F(1,1), LDF)
                    CALL SGER(M, LE, -SGN2*ONE, F(1,L), IONE, D(1,L), IONE, F(1,1), LDF)
                ELSE
                    CALL SGER(M, LE, -SGN1*ONE, E(1,L),  IONE, B(1,L),  IONE, F(1,1), LDF)
                    CALL SGER(M, LE, -SGN1*ONE, E(1,LH), IONE, B(1,LH), IONE, F(1,1), LDF)
                    CALL SGER(M, LE, -SGN2*ONE, F(1,L),  IONE, D(1,L),  IONE, F(1,1), LDF)
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                ...
                ELSE
                    CALL SGER(M, N-LH, -SGN1*ONE, E(1,L),  IONE, B(L,LH+1),  LDB, F(1,LH+1), LDF)
                    CALL SGER(M, N-LH, -SGN1*ONE, E(1,LH), IONE, B(LH,LH+1), LDB, F(1,LH+1), LDF)
