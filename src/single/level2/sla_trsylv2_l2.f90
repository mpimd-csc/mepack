...
!> \verbatim
!>
!> SLA_TRSYLV2_L2 solves a generalized Sylvester equation of the following forms
!>
...
SUBROUTINE SLA_TRSYLV2_L2 ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TRSYLV2_L2', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            IF ( K .GT. 1 ) THEN
                ...
                IF ( KB .EQ. 1 ) THEN
                    WORK(1:N) = X(K,1:N)
                    CALL STRMV('Upper', 'Trans', 'NoUnit', N, B, LDB, WORK, IONE)
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE. ZERO) WORK(IT) = WORK(IT)+B(IT+1,IT)*X(K,IT+1)
                    END DO

                    CALL SGER(KE, N, -ONE, A(1,K) , IONE, WORK, IONE, X(1,1), LDX)
                ELSE
                    WORK(1:N) = X(K,1:N)
                    WORK(N+1:2*N) = X(KH,1:N)
                    CALL STRMV('Upper', 'Trans', 'NoUnit', N, B, LDB, WORK, IONE)
                    CALL STRMV('Upper', 'Trans', 'NoUnit', N, B, LDB, WORK(N+1), IONE)
                    DO IT = 1,N-1
                    ...
                    CALL SGER(KE, N, -ONE, A(1,K) , IONE, WORK, IONE, X(1,1), LDX)
                    CALL SGER(KE, N, -ONE, A(1,KH) , IONE, WORK(N+1), IONE, X(1,1), LDX)
                ENDIF
            END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            IF ( K .GT. 1 ) THEN
                IF ( KB .EQ. IONE ) THEN
                    WORK(1:N) = X(K,1:N)
                    CALL STRMV('Upper', 'NoTrans', 'NoUnit', N, B, LDB, WORK, IONE)
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE.ZERO) THEN
                ...
                ELSE
                    ...
                    CALL STRMV('Upper', 'NoTrans', 'NoUnit', N, B, LDB, WORK, IONE)
                    CALL STRMV('Upper', 'NoTrans', 'NoUnit', N, B, LDB, WORK(N+1), IONE)
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE.ZERO) THEN
                            L = 2*(IT+1)
                            WORK(IT+1)     = WORK(IT+1) + B(IT+1,IT) * X(K,IT)
            ...
            IF ( KH .LT. M ) THEN
                IF (KB .EQ. IONE ) THEN
                    ...
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE. ZERO) WORK(IT) = WORK(IT)+B(IT+1,IT)*X(K,IT+1)
                    END DO
                    CALL SGER(M-KH, N, -ONE, A(K,KH+1), LDA, WORK, IONE, X(KH+1, 1), LDX)
                ELSE
                    WORK(1:N) = X(K,1:N)
                    WORK(N+1:2*N) = X(KH,1:N)
                    ...
                    CALL SGER(M-KH, N, -ONE, A(K,KH+1), LDA, WORK, IONE, X(KH+1, 1), LDX)
                    CALL SGER(M-KH, N, -ONE, A(KH,KH+1), LDA, WORK(N+1), IONE, X(KH+1, 1), LDX)
