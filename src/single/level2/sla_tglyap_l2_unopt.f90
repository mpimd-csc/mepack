...
!> \endverbatim
!> \attention The algorithm is implemented using BLAS level 2 operations but works with inefficient SSYR2 calls in the case
!> TRANS="T".
!>
!
!  Arguments:
...
SUBROUTINE SLA_TGLYAP_L2_UNOPT ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGLYAP_L2_UNOPT', -INFO)
        RETURN
    END IF
