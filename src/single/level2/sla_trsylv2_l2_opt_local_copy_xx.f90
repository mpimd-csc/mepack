...
! BODY OF SLA_TRSYLV2_L2_LOCAL_COPY
! Arguments
...
REAL AL(MAXNB,MAXNB), BL(MAXNB,MAXNB),  XL(MAXNB,MAXNB), WORKA(MAXNB)
REAL WORKB(MAXNB)
...
IF ( INFO .LT. 0 ) THEN
    CALL XERROR_HANDLER('SLA_TRSYLV2_L2_LOCAL_COPY_XX', -INFO)
    RETURN
END IF
...
IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
    ...
    DO WHILE ( L .LE. N )
        ...
        IF ( LH .LT. N ) THEN
            IF ( LB .EQ. IONE ) THEN
                WORKA(1:M) = XL(1:M,L)
                CALL STRMV('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORKA, IONE)
                DO IT = 1,M-1
            ...
            ELSE
                ...
                WORKB(1:M) = XL(1:M,LH)
                CALL STRMV('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORKA, IONE)
                CALL STRMV('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORKB, IONE)
...
ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
    ...
    DO WHILE ( L .LE. N )
        ...
        IF ( LH .LT. N ) THEN
            IF ( LB .EQ. IONE ) THEN
                WORKA(1:M) = XL(1:M, L)
                CALL STRMV("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORKA, IONE)
                DO IT = 1, M-1
            ...
            ELSE
                ...
                WORKB(1:M) = XL(1:M, LH)
                CALL STRMV("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORKA, IONE)
                CALL STRMV("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORKB, IONE)
