...
! BODY OF DLA_GSYLV_L2_LOCAL_COPY
! Copyright (C) Martin Koehler, 2017-2023
...
REAL AL(MAXNB,MAXNB), BL(MAXNB,MAXNB), CL(MAXNB,MAXNB), DL(MAXNB,MAXNB), XL(MAXNB,MAXNB), WORKA(MAXNB)
REAL WORKB(MAXNB)
...
!IBM* ALIGN(64,AL,BL,CL,DL,XL,WORKA,WORKB,MAT,RHS,IPIV,JPIV)
...
IF ( INFO .LT. 0 ) THEN
    CALL XERROR_HANDLER('DLA_GSYLV_L2_LOCAL_COPY_XX', -INFO)
    RETURN
END IF
...
IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
    ...
    DO WHILE ( L .LE. N )
        ...
        IF ( LH .LT. N ) THEN
            IF ( LB .EQ. IONE ) THEN
                ! WORKA(1:M) = XL(1:M,L)
                ! CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORKA, IONE)
                ! DO IT = 1,M-1
                ...
                WORKA(1:M) = 0.0
                DO IT = 1, M - 1
                    IF (AL(IT+1,IT).EQ.ZERO) THEN
                        WORKA(1:IT) = WORKA(1:IT) + AL(1:IT,IT) * XL(IT, L)
                    ELSE
                        WORKA(1:IT+1) = WORKA(1:IT+1) + AL(1:IT+1,IT) * XL(IT, L)
                    END IF
                END DO
                WORKA(1:M) = WORKA(1:M) + AL(1:M, M) * XL(M,L)
                ...
                ! WORKA(1:M) = XL(1:M,L)
                ! CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORKA, IONE)

                WORKA(1:M) = 0.0
                DO IT = 1, M
                    WORKA(1:IT) = WORKA(1:IT) + CL(1:IT,IT) * XL(IT, L)
                END DO

                DO IT = LH+1, N
                    XL(1:M,IT) =  XL(1:M,IT) - SGN*WORKA(1:M)*DL(L,IT)
                END DO
            ...
            ELSE
                ...
                ! WORKB(1:M) = XL(1:M,LH)
                ! CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORKA, IONE)
                ! CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORKB, IONE)
                ! DO IT = 1,M-1
                ! IF (AL(IT+1,IT).NE.ZERO) THEN
                ! WORKA(IT+1) = WORKA(IT+1) + AL(IT+1,IT) * XL(IT,L)
                ...
                DO IT = 1, M - 1
                    IF ( AL(IT+1,IT) .EQ. ZERO ) THEN
                        WORKA(1:IT) = WORKA(1:IT) + AL(1:IT,IT) * XL(IT, L)
                        WORKB(1:IT) = WORKB(1:IT) + AL(1:IT,IT) * XL(IT, LH)
                    ELSE
                        WORKA(1:IT+1) = WORKA(1:IT+1) + AL(1:IT+1,IT) * XL(IT, L)
                        WORKB(1:IT+1) = WORKB(1:IT+1) + AL(1:IT+1,IT) * XL(IT, LH)
                    END IF
                    ! IF (AL(IT+1,IT).NE.ZERO) THEN
                    !      WORKA(IT+1) = WORKA(IT+1) + AL(IT+1,IT) * XL(IT,L)
                    !      WORKB(IT+1) = WORKB(IT+1) + AL(IT+1,IT) * XL(IT,LH)
                ...
                END DO
                WORKA(1:M) = WORKA(1:M) + AL(1:M, M) * XL(M,L)
                WORKB(1:M) = WORKB(1:M) + AL(1:M, M) * XL(M,LH)
                ...
                ! WORKB(1:M) = XL(1:M,LH)
                ! CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORKA, IONE)
                ! CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORKB, IONE)
                ...
                DO IT = 1, M
                    WORKA(1:IT) = WORKA(1:IT) + CL(1:IT,IT) * XL(IT, L)
                    WORKB(1:IT) = WORKB(1:IT) + CL(1:IT,IT) * XL(IT, LH)
                END DO

                DO IT = LH+1, N
                    XL(1:M,IT) =  XL(1:M,IT) - SGN*( WORKA(1:M)*DL(L,IT) + WORKB(1:M) * DL(LH,IT) )
                END DO
    ...
    DO WHILE ( L .GT. 0 )
        ...
        IF ( L .GT. 1 ) THEN
            IF ( LB .EQ. IONE ) THEN
                ...
                WORKB(1:M) = XL(1:M,L)
                CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORKB, IONE)
                ! CALL SGER(M, LE, -SGN*ONE, WORK, IONE, DL(1,L), IONE, XL(1,1), LDXL)

                DO IT = 1, LE
                    XL(1:M, IT) = XL(1:M,IT) - WORKA(1:M)*BL(IT,L) - (SGN*DL(IT,L)) * WORKB(1:M)
                END DO
            ...
            ELSE
                ...
                DO IT = 1,M-1
                    ...
                    END IF
                END DO
                ! CALL SGER(M, LE, -ONE, WORKA, IONE, BL(1,L), IONE, XL(1,1), LDXL)
                ! CALL SGER(M, LE, -ONE, WORKB, IONE, BL(1,LH), IONE, XL(1,1), LDXL)
                ...
                ! CALL SGER(M, LE, -SGN*ONE, WORKA, IONE, DL(1,L), IONE, XL(1,1), LDXL)
                ! CALL SGER(M, LE, -SGN*ONE, WORKB, IONE, DL(1,LH), IONE, XL(1,1), LDXL)
                DO IT = 1, LE
                    XL(1:M, IT) = XL(1:M,IT) - WORKA(1:M)*(SGN*DL(IT,L)) - (SGN*DL(IT,LH)) * WORKB(1:M)
                END DO
...
ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
    ...
    DO WHILE ( L .LE. N )
        ...
        IF ( LH .LT. N ) THEN
            IF ( LB .EQ. IONE ) THEN
                WORKA(1:M) = XL(1:M, L)
                CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORKA, IONE)
                DO IT = 1, M-1
                ...
                WORKB(1:M) = XL(1:M, L)
                CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, CL, LDCL, WORKB, IONE)

                ! CALL SGER(M, N-LH, -ONE, WORK, IONE, BL(L,LH+1), LDBL, XL(1,LH+1), LDXL)
                ! CALL SGER(M, N-LH, -SGN*ONE, WORKB, IONE, DL(L,LH+1), LDDL, XL(1,LH+1), LDXL)

                DO IT = LH+1, N
                    XL(1:M, IT) = XL(1:M, IT) - BL(L, IT) * WORKA(1:M) - SGN * DL(L,IT) * WORKB(1:M)
                END DO
            ...
            ELSE
                ...
                WORKB(1:M) = XL(1:M, LH)
                CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORKA, IONE)
                CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORKB, IONE)
                ...
                DO IT = 1, M-1
                    ...
                    END IF
                END DO
                ! CALL SGER(M, N-LH, -ONE, WORKA, IONE, BL(L,LH+1), LDBL, XL(1,LH+1), LDXL)
                ! CALL SGER(M, N-LH, -ONE, WORKB, IONE, BL(LH,LH+1), LDBL, XL(1,LH+1), LDXL)
                ...
                WORKA(1:M) = XL(1:M, L)
                WORKB(1:M) = XL(1:M, LH)
                CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, CL, LDCL, WORKA, IONE)
                CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, CL, LDCL, WORKB, IONE)

                ! CALL SGER(M, N-LH, -SGN*ONE, WORKA, IONE, DL(L,LH+1), LDDL, XL(1,LH+1), LDXL)
                ! CALL SGER(M, N-LH, -SGN*ONE, WORKB, IONE, DL(LH,LH+1), LDDL, XL(1,LH+1), LDXL)

                DO IT = LH+1, N
                    XL(1:M, IT) = XL(1:M, IT) - (SGN*DL(L,IT)) * WORKA(1:M) - (SGN*DL(LH, IT)) * WORKB(1:M)
                END DO
    ...
    DO WHILE ( L .GT. 0 )
        ...
        IF ( L .GT. IONE ) THEN
            IF ( LB .EQ. IONE ) THEN
                ...
                ! CALL SGER(M, LE, -ONE, WORKA, IONE, BL(1,L), IONE, XL(1,1), LDXL)
                ! CALL SGER(M, LE, -SGN*ONE, WORKB, IONE, DL(1,L), IONE, XL(1,1), LDXL)
                DO IT = 1, LE
                    XL(1:M, IT) = XL(1:M, IT) - BL(IT,L) * WORKA(1:M) - (SGN*DL(IT, L)) * WORKB(1:M)
                END DO
            ...
            ELSE
                ...
                ! CALL SGER(M, LE, -SGN*ONE, WORKB, IONE, DL(1,LH), IONE, XL(1,1), LDXL)
                DO IT = 1, LE
                    XL(1:M, IT) = XL(1:M, IT) - (SGN*DL(IT,L)) * WORKA(1:M) - (SGN*DL(IT, LH)) * WORKB(1:M)
                END DO
