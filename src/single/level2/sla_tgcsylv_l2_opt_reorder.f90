...
!  Definition:
!  ===========
!       SUBROUTINE SLA_TGCSYLV_L2_REORDER ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
SUBROUTINE SLA_TGCSYLV_L2_REORDER ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGCSYLV_L2_REORDER', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL SGER(M, N-LH, -SGN1*ONE, F(1,L), IONE, B(L,LH+1), LDB, E(1,LH+1), LDE)
                    CALL SGER(M, N-LH, -SGN2*ONE, F(1,L), IONE, D(L,LH+1), LDD, F(1,LH+1), LDF)

                ELSE
                    CALL SGER(M, N-LH, -SGN1*ONE, F(1,L), IONE, B(L,LH+1), LDB, E(1,LH+1), LDE)
                    CALL SGER(M, N-LH, -SGN1*ONE, F(1,LH), IONE, B(LH,LH+1), LDB, E(1,LH+1), LDE)
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( L .GT. 1 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL SGER(M, LE, -SGN1*ONE, F(1,L), IONE, B(1,L), IONE, E(1,1), LDE)
                    CALL SGER(M, LE, -SGN2*ONE, F(1,L), IONE, D(1,L), IONE, F(1,1), LDF)
                ELSE
                    CALL SGER(M, LE, -SGN1*ONE, F(1,L), IONE, B(1,L), IONE, E(1,1), LDE)
                    CALL SGER(M, LE, -SGN1*ONE, F(1,LH), IONE, B(1,LH), IONE, E(1,1), LDE)
                    CALL SGER(M, LE, -SGN2*ONE, F(1,L), IONE, D(1,L), IONE, F(1,1), LDF)
                    CALL SGER(M, LE, -SGN2*ONE, F(1,LH), IONE, D(1,LH), IONE, F(1,1), LDF)
                ENDIF
            END IF
            L = L - 1
        END DO
