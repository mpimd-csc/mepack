...
!  ===========
!
!       SUBROUTINE SLA_TGSYLV_L2_LOCAL_COPY ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, &
!                                          & SCALE, WORK, INFO)
...
SUBROUTINE SLA_TGSYLV_L2_LOCAL_COPY ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, &
    ...
    REAL AL(MAXNB,MAXNB), BL(MAXNB,MAXNB), CL(MAXNB,MAXNB), DL(MAXNB,MAXNB), XL(MAXNB,MAXNB)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGSYLV_L2_LOCAL_COPY', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    ...
                    CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORK, IONE)
                    DO IT = 1,M-1
                    ...
                    WORK(1:M) = XL(1:M,L)
                    CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORK, IONE)
                    CALL SGER(M, N-LH, -SGN*ONE, WORK, IONE, DL(L,LH+1), LDDL, XL(1,LH+1), LDXL)

                ELSE
                    WORK(1:M) = XL(1:M,L)
                    WORK(M+1:2*M) = XL(1:M,LH)
                    CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORK, IONE)
                    CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORK(M+1), IONE)
                    ...
                    WORK(M+1:2*M) = XL(1:M,LH)
                    CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORK, IONE)
                    CALL STRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORK(M+1), IONE)
                    CALL SGER(M, N-LH, -SGN*ONE, WORK, IONE, DL(L,LH+1), LDDL, XL(1,LH+1), LDXL)
                    CALL SGER(M, N-LH, -SGN*ONE, WORK(M+1), IONE, DL(LH,LH+1), LDDL, XL(1,LH+1), LDXL)
                ENDIF
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( L .GT. 1 ) THEN
                ...
                ELSE
                    ...
                    CALL SGER(M, LE, -SGN*ONE, WORK, IONE, DL(1,L), IONE, XL(1,1), LDXL)
                    CALL SGER(M, LE, -SGN*ONE, WORK(M+1), IONE, DL(1,LH), IONE, XL(1,1), LDXL)
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    WORK(1:M) = XL(1:M, L)
                    CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORK, IONE)
                    DO IT = 1, M-1
                ...
                ELSE
                    ...
                    WORK(M+1:2*M) = XL(1:M, LH)
                    CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORK, IONE)
                    CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORK(M+1), IONE)
                ...
                IF ( LB .EQ. IONE ) THEN
                    WORK(1:M) = XL(1:M, L)
                    CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, CL, LDCL, WORK, IONE)
                    CALL SGER(M, N-LH, -SGN*ONE, WORK, IONE, DL(L,LH+1), LDDL, XL(1,LH+1), LDXL)
                ...
                ELSE
                    ...
                    WORK(M+1:2*M) = XL(1:M, LH)
                    CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, CL, LDCL, WORK, IONE)
                    CALL STRMV_SEQ("Lower", "NoTrans", "NoUnit", M, CL, LDCL, WORK(M+1), IONE)
