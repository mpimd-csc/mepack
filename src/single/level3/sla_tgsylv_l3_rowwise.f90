...
SUBROUTINE SLA_TGSYLV_L3_ROWWISE ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGSYLV_L3_ROWWISE', -INFO)
        RETURN
    END IF
    ...
    IF ( M .LE. MB .AND. N .LE. NB ) THEN
        ...
        CASE(4)
            CALL SLA_TGSYLV_L2(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, B, LDB, &
                & C, LDC, D, LDD,  X, LDX, SCAL, WORK, INFO1)
        CASE(5)
            CALL SLA_TGSYLV_L2_ROWWISE(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, B, LDB, &
                & C, LDC, D, LDD,  X, LDX, SCAL, WORK, INFO1)
        CASE(6)
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            DO WHILE ( LH .LE. N )
                ...
                CASE(4)
                    CALL SLA_TGSYLV_L2(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                        & C(K,K), LDC, D(L,L), LDD,  X(K,L), LDX, SCAL, WORK, INFO1)
                CASE(5)
                    CALL SLA_TGSYLV_L2_ROWWISE(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                        & C(K,K), LDC, D(L,L), LDD,  X(K,L), LDX, SCAL, WORK, INFO1)
                CASE(6)
                ...
                ! in current row
                IF ( LH .LT. N ) THEN
                    CALL SGEMM('N','N', KB, LB, KB, ONE, A(K,K), LDA, X(K,L), LDX, ZERO, WORK, KB )
                    CALL SGEMM('N','N', KB, N-LH, LB, -ONE, WORK, KB, B(L,LH+1), LDB, ONE, X(K,LH+1), LDX)

                    CALL SGEMM('N','N', KB, LB, KB, ONE, C(K,K), LDC, X(K,L), LDX, ZERO, WORK, KB )
                    CALL SGEMM('N','N', KB, N-LH, LB, -ONE*SGN, WORK, KB, D(L,LH+1), LDD, ONE, X(K,LH+1), LDX)
            ...
            ! Update January 2021
            IF ( K .GT. 1 ) THEN
                CALL SLACPY('All', KB, N, X(K,1), LDX,  WORK, KB)
                CALL STRMM('Right', 'Upper', 'NoTrans', 'NoUnit', KB, N, ONE, B, LDB, WORK, KB)
                CALL SLA_QTRMM2('Right','NoTrans',KB, N, ONE, B, LDB, X(K,1), LDX, WORK, KB)
                CALL SGEMM('N', 'N', KE, N, KB, -ONE, A(1,K), LDA, WORK, KB, ONE, X(1,1), LDX)

                CALL SLACPY('All', KB, N, X(K,1), LDX, WORK, KB)
                CALL STRMM('Right', 'Upper', 'NoTrans', 'NoUnit', KB, N, ONE, D, LDD, WORK, KB)
                CALL SLA_QTRMM2('Right','NoTrans',KB, N, ONE, D, LDD, X(K,1), LDX, WORK, KB)
                CALL SGEMM('N', 'N', KE, N, KB, -ONE*SGN, C(1,K), LDC, WORK, KB, ONE, X(1,1), LDX)
            END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            DO WHILE ( L .GT. 0 )
                ...
                IF ( L .GT. 0 ) THEN
                    CALL SGEMM('N','N', KB, LB, KB, ONE, A(K,K), LDA, X(K,L), LDX, ZERO, WORK, KB )
                    CALL SGEMM('N','T', KB, LE, LB, -ONE, WORK, KB, B(1,L), LDB, ONE, X(K,1), LDX)

                    CALL SGEMM('N','N', KB, LB, KB, ONE, C(K,K), LDC, X(K,L), LDX, ZERO, WORK, KB )
                    CALL SGEMM('N','T', KB, LE, LB, -ONE*SGN, WORK, KB, D(1,L), LDD, ONE, X(K,1), LDX)
                END IF
            ...
            IF ( K .GT. 1 ) THEN
                CALL SLACPY('All', KB, N, X(K,1), LDX,  WORK, KB)
                CALL STRMM('Right', 'Upper', 'Trans', 'NoUnit', KB, N, ONE, B, LDB, WORK, KB)
                CALL SLA_QTRMM2('Right','Trans',KB, N, ONE, B, LDB, X(K,1), LDX, WORK, KB)
                CALL SGEMM('N', 'N', KE, N, KB, -ONE, A(1,K), LDA, WORK, KB, ONE, X(1,1), LDX)

                CALL SLACPY('All', KB, N, X(K,1), LDX, WORK, KB)
                CALL STRMM('Right', 'Upper', 'Trans', 'NoUnit', KB, N, ONE, D, LDD, WORK, KB)
                CALL SLA_QTRMM2('Right','Trans',KB, N, ONE, D, LDD, X(K,1), LDX, WORK, KB)
                CALL SGEMM('N', 'N', KE, N, KB, -ONE*SGN, C(1,K), LDC, WORK, KB, ONE, X(1,1), LDX)
            END IF
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( KH .LE. M )
            ...
            DO WHILE ( LH .LE. N )
                ...
                ! in current row
                IF ( LH .LT. N ) THEN
                    CALL SGEMM('T','N', KB, LB, KB, ONE, A(K,K), LDA, X(K,L), LDX, ZERO, WORK, KB )
                    CALL SGEMM('N','N', KB, N-LH, LB, -ONE, WORK, KB, B(L,LH+1), LDB, ONE, X(K,LH+1), LDX)

                    CALL SGEMM('T','N', KB, LB, KB, ONE, C(K,K), LDC, X(K,L), LDX, ZERO, WORK, KB )
                    CALL SGEMM('N','N', KB, N-LH, LB, -ONE*SGN, WORK, KB, D(L,LH+1), LDD, ONE, X(K,LH+1), LDX)
            ...
            IF ( KH .LT. M ) THEN
                ...
                CALL STRMM('Right', 'Upper', 'NoTrans', 'NoUnit', KB, N, ONE, B, LDB, WORK, KB)
                CALL SLA_QTRMM2('Right','NoTrans',KB, N, ONE, B, LDB, X(K,1), LDX, WORK, KB)
                CALL SGEMM('T', 'N', M-KH, N, KB, -ONE, A(K,KH+1), LDA, WORK, KB, ONE, X(KH+1,1), LDX)

                CALL SLACPY('All', KB, N, X(K,1), LDX,  WORK, KB)
                CALL STRMM('Right', 'Upper', 'NoTrans', 'NoUnit', KB, N, ONE, D, LDD, WORK, KB)
                CALL SLA_QTRMM2('Right','NoTrans',KB, N, ONE, D, LDD, X(K,1), LDX, WORK, KB)
                CALL SGEMM('T', 'N', M-KH, N, KB, -ONE*SGN, C(K,KH+1), LDC, WORK, KB, ONE, X(KH+1,1), LDX)
