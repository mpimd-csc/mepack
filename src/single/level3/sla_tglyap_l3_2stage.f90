...
SUBROUTINE SLA_TGLYAP_L3_2S ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGLYAP_L3_2S', -INFO)
        RETURN
    END IF
    ...
    IF ( ININFO .EQ. -1 ) THEN
        ...
        DAG_INFO = -1
        DAG_INFO2 = -1
        CALL SLA_TGLYAP_DAG ( TRANS, BIGNB, A, BIGNB, B, BIGNB,  X, BIGNB, SCALE, WORK, DAG_INFO)
    ...
    IF ( BTRANS ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( L.EQ.K) THEN
                    CALL SLA_TGLYAP_DAG (TRANSA, KB, A(K,K), LDA, B(K,K), LDB, X(K,K), LDX, SCAL, WORK, INFO1)
                    IF ( INFO1 .LT. 0 ) THEN
                        INFO = 100
                    END IF
    ...
    ELSE
        ...
        DO WHILE ( LH .LE. M )
            ...
            ! Update January 2021
            IF ( LH .LT. M ) THEN
                CALL SGEMM("T", "N", LB, M-LH, M-L+1, ONE, X(L,L), LDX, A(L,LH+1), LDA, ZERO, WORK, LB)
                CALL SSYR2K("L", "T", M-LH, LB, -ONE, WORK, LB, B(L,LH+1), LDB, ONE, X(LH+1,LH+1), LDX)

                CALL SGEMM("T", "N", LB, M-LH, M-LH, ONE, X(LH+1, L), LDX, B(LH+1,LH+1), LDB, ZERO, WORK, LB)
                CALL SSYR2K("L", "T", M-LH, LB, -ONE, WORK, LB,  A(L,LH+1), LDA, ONE, X(LH+1,LH+1), LDX)
