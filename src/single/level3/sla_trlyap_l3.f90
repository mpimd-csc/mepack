...
SUBROUTINE SLA_TRLYAP_L3 ( TRANS, M, A, LDA, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TRLYAP_L3', -INFO)
        RETURN
    END IF
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( K .EQ. L ) THEN
                    ...
                    IF ( ISOLVER .EQ. 6 ) THEN
                        ...
                        CALL SLA_TRLYAP_RECURSIVE(TRANSA, KB, A(K,K), LDA,  X(K,K), LDX, SCAL, WORK, INFO1)
                    ELSE
                        IF ( KB .LE. 64 ) THEN
                            CALL SLA_TRLYAP_L2_OPT(TRANSA, KB, A(K,K), LDA,  X(K,K), LDX, SCAL, WORK, INFO1)
                        ELSE
                            CALL SLA_TRLYAP_L2(TRANSA, KB, A(K,K), LDA,  X(K,K), LDX, SCAL, WORK, INFO1)
                        END IF
                        ! CALL SLA_TRLYAP_L2_OPT(TRANSA, KB, A(K,K), LDA,  X(K,K), LDX, SCAL, WORK, INFO1)
                    END IF
...
!                    KB = MIN(256, LE - K + 1)
!
!                    CALL SGEMM("N", "T", LE, KB, LB, -ONE, A(1,L), LDA, X(K,L), LDX, ONE, X(1,K), LDX)
!                    CALL SGEMM("N", "T", LE, KB, LB, -ONE, X(1,L), LDX, A(K,L), LDA, ONE, X(1,K), LDX)
!                END DO
!                !$omp end parallel do

                    ! CALL SGEMM("N", "T", LE, LE, LB, -ONE, A(1,L), LDA, X(1,L), LDX, ONE, X(1,1), LDX)
                    ! CALL SGEMM("N", "T", LE, LE, LB, -ONE, X(1,L), LDX, A(1,L), LDA, ONE, X(1,1), LDX)
            ...
            DO WHILE ( LH .LE. M )
                ...
                IF ( L.EQ.K) THEN
                    ...
                    IF ( ISOLVER .EQ. 6 ) THEN
                        ...
                        CALL SLA_TRLYAP_RECURSIVE(TRANSA,  KB, A(K,K), LDA, X(K,L), LDX, SCAL, WORK, INFO1)
                    ELSE
                        IF ( KB .LE. 64 ) THEN
                            CALL SLA_TRLYAP_L2_OPT(TRANSA, KB, A(K,K), LDA,  X(K,K), LDX, SCAL, WORK, INFO1)
                        ELSE
                            CALL SLA_TRLYAP_L2(TRANSA,  KB, A(K,K), LDA, X(K,L), LDX, SCAL, WORK, INFO1)
                        END IF
                    END IF
                ELSE
