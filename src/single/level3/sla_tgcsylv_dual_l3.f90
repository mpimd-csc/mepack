...
!  Definition:
!  ===========
!       SUBROUTINE SLA_TGCSYLV_DUAL_L3 ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
SUBROUTINE SLA_TGCSYLV_DUAL_L3 ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGCSYLV_DUAL_L3', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE ( KH .LE. M )
                ...
                IF ( KH .LT. M  ) THEN
                    ...
                    CALL SGEMM('T', 'N', M-KH, LB, KB, -ONE, C(K, KH+1), LDC, F(K,L), LDF, ONE, E(KH+1,L),LDE)
                END IF
                KH = KH + 1
            ...
            ! Update January 2021
            IF ( L .GT. 1) THEN
                CALL SGEMM('N', 'T', M, LE, LB, -ONE*SGN1, E(1,L), LDE, B(1, L), LDB, ONE, F(1,1), LDF)
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                CALL SGEMM("N", "N", M, N-LH, LB, -ONE*SGN1, E(1,L), LDE, B(L,LH+1), LDB, ONE, F(1,LH+1), LDF)
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( K .GT. 1 ) THEN
                    ...
                    CALL SGEMM('N', 'N', KE, LB, KB, -ONE, C(1,K), LDC, F(K,L), LDF, ONE, E(1,L), LDE)
                END IF
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( K .GT. 1 ) THEN
                    ...
                    CALL SGEMM('N', 'N', K-1, LB, KB, -ONE, C(1,K), LDC, F(K,L), LDF, ONE, E(1,L), LDE)
                END IF
