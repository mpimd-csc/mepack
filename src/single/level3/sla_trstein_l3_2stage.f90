...
SUBROUTINE SLA_TRSTEIN_L3_2S ( TRANS, M, A, LDA, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TRSTEIN_L3_2S', -INFO)
        RETURN
    END IF
    ...
    IF ( ININFO .EQ. -1 ) THEN
        ...
        DAG_INFO = -1
        DAG_INFO2 = -1
        CALL SLA_TRSTEIN_DAG ( TRANS, BIGNB, A, BIGNB, X, BIGNB, SCALE, WORK, DAG_INFO)
        CALL SLA_TRSYLV2_DAG( TRANSA, TRANSB, ONE, BIGNB, BIGNB,  A, BIGNB, A, BIGNB, &
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( L.EQ.K) THEN
                    ...
                    CALL SLA_TRSTEIN_DAG(TRANS, KB, A(K,K), LDA, X(K,L), LDX, SCAL, WORK, INFO1)
                    IF ( INFO1 .LT. 0 ) THEN
                        INFO = 100
                    END IF

                ELSE
                    CALL SLA_TRSYLV2_DAG(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, A(L,L), LDA, &
