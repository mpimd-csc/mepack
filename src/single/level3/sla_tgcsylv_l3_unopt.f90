...
!> \brief Level-3 Kagstroem-Westin/Bartels-Stewart Algorithm for the generalized coupled Sylvester equation (unoptimized)
!
!  Definition:
!  ===========
!       SUBROUTINE SLA_TGCSYLV_L3_UNOPT ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
SUBROUTINE SLA_TGCSYLV_L3_UNOPT ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGCSYLV_L3_UNOPT', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            DO WHILE ( LH .LE. N )
                ...
                ! in current row
                IF ( LH .LT. N ) THEN
                    CALL SGEMM('N','N', KB, N-LH, LB, -SGN1*ONE, F(K,L), LDF, B(L,LH+1), LDB, ONE, E(K,LH+1), LDE)
                    CALL SGEMM('N','N', KB, N-LH, LB, -SGN2*ONE, F(K,L), LDF, D(L,LH+1), LDD, ONE, F(K,LH+1), LDF)
                END IF
            ...
            ! Update January 2021
            IF ( K .GT. 1 ) THEN
                CALL SGEMM('N', 'N', KE, N, KB, -ONE, A(1,K), LDA, E(K,1), LDE, ONE, E(1,1), LDE)
                CALL SGEMM('N', 'N', KE, N, KB, -ONE, C(1,K), LDA, E(K,1), LDE, ONE, F(1,1), LDF)
            END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            DO WHILE ( L .GT. 0 )
                ...
                ! Update January 2021
                IF ( L .GT. 0 ) THEN
                    CALL SGEMM('N','T', KB, LE, LB, -SGN1*ONE, F(K,L), LDF, B(1,L), LDB, ONE, E(K,1), LDE)
                    CALL SGEMM('N','T', KB, LE, LB, -SGN2*ONE, F(K,L), LDF, D(1,L), LDD, ONE, F(K,1), LDF)
                END IF
            ...
            IF ( K .GT. 1 ) THEN
                CALL SGEMM('N', 'N', KE, N, KB, -ONE, A(1,K), LDA, E(K,1), LDE, ONE, E(1,1), LDE)
                CALL SGEMM('N', 'N', KE, N, KB, -ONE, C(1,K), LDC, E(K,1), LDE, ONE, F(1,1), LDF)
            END IF
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( KH .LE. M )
            ...
            IF ( KH .LT. M ) THEN
                CALL SGEMM('T', 'N', M-KH, N, KB, -ONE, A(K,KH+1), LDA, E(K,1), LDE, ONE, E(KH+1,1), LDE)
                CALL SGEMM('T', 'N', M-KH, N, KB, -ONE, C(K,KH+1), LDC, E(K,1), LDE, ONE, F(KH+1,1), LDF)
            END IF
