...
SUBROUTINE SLA_TGSTEIN_L3_2S ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGSTEIN_L3_2S', -INFO)
        RETURN
    END IF
    ...
    IF ( ININFO .EQ. -1 ) THEN
        ...
        DAG_INFO = -1
        DAG_INFO2 = -1
        CALL SLA_TGSTEIN_DAG ( TRANS, BIGNB, A, BIGNB, B, BIGNB,  X, BIGNB, SCALE, WORK, DAG_INFO)
        CALL SLA_TGSYLV_DAG(TRANSA, TRANSB, ONE, BIGNB, BIGNB,  A, BIGNB, A, BIGNB, &
    ...
    IF ( M .LE. NB ) THEN
        ...
        CASE(1)
            IF ( NB .LE. 32 ) THEN
                CALL SLA_TGSYLV_L2_LOCAL_COPY_32(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                    & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
            ELSE IF ( NB .LE. 64) THEN
                CALL SLA_TGSYLV_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                    & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
            ELSE IF ( NB .LE. 96) THEN
                CALL SLA_TGSYLV_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                    & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
            ELSE IF ( NB .LE. 128) THEN
                CALL SLA_TGSYLV_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                    & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
            ELSE
                CALL SLA_TGSYLV_L2_REORDER(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                    & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
            END IF
        CASE(2)
            IF ( M .GT. 128 ) THEN
                CALL SLA_TGSYLV_L2_REORDER(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                    & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
            ELSE
                CALL SLA_TGSYLV_L2_LOCAL_COPY(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                    & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
            END IF
        CASE(3)
            CALL SLA_TGSYLV_L2_REORDER(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
        CASE(4)
            CALL SLA_TGSYLV_L2(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
        CASE(5)
            CALL SLA_TGSYLV_L2_UNOPT(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
        CASE(6)
            CALL SLA_TGSYLV_RECURSIVE(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, A, LDA, &
                & B, LDB, B, LDB,  X, LDX, SCAL, WORK, INFO1)
        END SELECT
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( L.EQ.K) THEN
                    ...
                    END DO
                    CALL SLA_TGSTEIN_DAG ( TRANS, KB, A(K,K) , LDA, B(K,K), LDB, X(K,K), LDX, SCALE, WORK, INFO)
                    IF ( INFO1 .LT. 0 ) THEN
                        INFO = 100
                    END IF

                ELSE
                    CALL SLA_TGSYLV_DAG(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, A(L,L), LDA, &
            ...
            IF ( LE .GT. 0 ) THEN
                ...
                CALL SGEMM("N", "N", LE, LB, LE, ONE, B(1,1), LDB, X(1,L), LDX, ZERO, WORK, LE)
                CALL SSYR2K("U", "N", LE, LB, ONE, WORK, LE, B(1,L), LDB, ONE, X(1,1), LDX)
                ...
                CALL SSYMM("Right", "Upper", LE, LB, ONE, X(L,L), LDX, B(1,L), LDB, ZERO, WORK, LE)
                CALL SGEMM("N", "T", LE, LE, LB, ONE, WORK, LE, B(1,L), LDB, ONE, X(1,1), LDX)
    ...
    ELSE
        ...
        DO WHILE ( LH .LE. M )
            ...
            ! Update January 2021
            IF ( LH .LT. M ) THEN
                CALL SGEMM("N", "N", LB, M-LH, M-LH, ONE, X(L,LH+1), LDX, A(LH+1,LH+1), LDA, ZERO, WORK, LB)
                CALL SSYR2K("L", "T", M-LH, LB, -ONE, A(L,LH+1), LDA, WORK, LB, ONE, X(LH+1,LH+1), LDX)

                CALL SGEMM("N", "N", LB, M-LH, M-LH, ONE, X(L,LH+1), LDX, B(LH+1,LH+1), LDB, ZERO, WORK, LB)
                CALL SSYR2K("L", "T", M-LH, LB, ONE, B(L,LH+1), LDB, WORK, LB, ONE, X(LH+1,LH+1), LDX)
                ...
                CALL SSYMM("Left", "Lower", LB, M-LH, ONE, X(L,L), LDX, B(L,LH+1), LDB, ZERO, WORK, LB)
                CALL SGEMM("T", "N", M-LH, M-LH, LB,  ONE, B(L,LH+1), LDB, WORK, LB, ONE, X(LH+1,LH+1), LDX)
