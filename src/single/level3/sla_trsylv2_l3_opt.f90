...
!> \verbatim
!>
!> SLA_TRSYLV2_L3 solves a generalized Sylvester equation of the following forms
!>
...
SUBROUTINE SLA_TRSYLV2_L3 ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB,  X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TRSYLV2_L3', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                ! IF ( LH .LT. N  ) THEN
                !     CALL SGEMM("N", "N", KB, LB, M-K+1, ONE, A(K,K), LDA, X(K,L), LDX, ZERO, WORK(K), M)
                !     CALL SGEMM("N", "N", KB, N-LH, LB, -ONE, WORK(K), M, B(L,LH+1), LDB, ONE, X(K,LH+1), LDX)
                ! END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                ! IF ( L .GT. 1 ) THEN
                !     CALL SGEMM("N", "N", KB, LB, M-K+1, ONE, A(K,K), LDA, X(K,L), LDX, ZERO, WORK(K), M)
                !     CALL SGEMM('N', 'T', KB, LE, LB, -ONE, WORK(K), M, B(1,L), LDB, ONE, X(K,1), LDX)
                !
                ! END IF
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE ( KH .LE. M )
                ...
                ! IF ( LH .LT. N ) THEN
                !     CALL SGEMM("T", "N", KB, LB, KH, ONE, A(1,K), LDA, X(1,L), LDX, ZERO, WORK(K), M)
                !     CALL SGEMM("N", "N", KB, N-LH, LB, -ONE, WORK(K), M, B(L,LH+1), LDB, ONE, X(K,LH+1), LDX)
                ! END IF
