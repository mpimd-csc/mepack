...
!>
!> \verbatim
!> SLA_TRSYLV_L3_2S solves a Sylvester equation of the following forms
!>
...
!> parallel DAG scheduled solver from SLA_TRSYLV_DAG and optimized kernel solvers.
...
SUBROUTINE SLA_TRSYLV_L3_2S ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TRSYLV_L3_2S', -INFO)
        RETURN
    END IF
    ...
    IF ( ININFO .EQ. -1 ) THEN
        ! MEMORY DAG
        DAG_INFO = -1
        CALL SLA_TRSYLV_DAG(TRANSA, TRANSB, SGN, BIGNB, BIGNB,  A, BIGNB, B, BIGNB,  X, BIGNB, SCAL, WORK,DAG_INFO)
        INFO = MAX( DAG_INFO, 1)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                CALL SLA_TRSYLV_DAG(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, B(L,L), LDB,  X(K,L), LDX, SCAL, WORK,INFO)
                ...
                ! in current row
                IF ( K .GT. 1 ) THEN
                    CALL SGEMM('N', 'N', K-1, LB, KB, -ONE, A(1,K), LDA, X(K,L), LDX, ONE, X(1,L), LDX)
                END IF
            ...
            ! Update January 2021
            IF ( LH .LT. N  ) THEN
                CALL SGEMM("N", "N", M, N-LH, LB, -SGN*ONE, X(1,L), LDX, B(L,LH+1), LDB, ONE, X(1,LH+1), LDX)
            END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                CALL SLA_TRSYLV_DAG(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, B(L,L), LDB,  X(K,L), LDX, SCAL, WORK,INFO1)
            ...
            ! Update January 2021
            IF ( L .GT. 1 ) THEN
                CALL SGEMM('N', 'T', M, LE, LB, -SGN*ONE, X(1,L), LDX, B(1,L), LDB, ONE, X(1,1), LDX)
            END IF
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE ( KH .LE. M )
                ...
                ! in current row
                IF ( KH  .LT. M ) THEN
                    CALL SGEMM("T", "N", M-KH, LB, KB, -ONE, A(K,KH+1), LDA, X(K,L), LDX, ONE, X(KH+1, L), LDX)
                END IF
