...
!>
!> \remark The algorithm works without STRMM calls because of performance issues. All STRMM calls are replaced
...
SUBROUTINE SLA_TGSYLV_L3_COLWISE ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('SLA_TGSYLV_L3_COLWISE', -INFO)
        RETURN
    END IF
    ...
    IF ( M .LE. MB .AND. N .LE. NB ) THEN
        ...
        CASE(4)
            CALL SLA_TGSYLV_L2(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, B, LDB, &
                & C, LDC, D, LDD,  X, LDX, SCAL, WORK, INFO1)
        CASE(5)
            CALL SLA_TGSYLV_L2_COLWISE(TRANSA, TRANSB, SGN, KB, LB,  A, LDA, B, LDB, &
                & C, LDC, D, LDD,  X, LDX, SCAL, WORK, INFO1)
        CASE(6)
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            IF ( LH .LT. N  ) THEN
                ...
                CALL SLA_QTRMM2('Left', 'NoTrans', M, LB, ONE, C, LDC, X(1,L), LDX, WORK, M)
