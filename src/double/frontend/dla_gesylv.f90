...
!  ===========
!
!   SUBROUTINE DLA_GESYLV(FACTA, FACTB, TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, QA, LDQA, QB, LDQB,
...
!           DOUBLE PRECISION A(LDA,*), B(LDB, *), QA(LDQA, *), QB(LDQB, *), X(LDX, *), WORK(*)
!
!> \par Purpose:
!  =============
!>
!> \verbatim
!> DLA_GESYLV solves a Sylvester equation of the following forms
!>
...
SUBROUTINE DLA_GESYLV(FACTA, FACTB, TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, QA, LDQA, QB, LDQB, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_GESYLV', -INFO)
        RETURN
    END IF
    ...
    SELECT CASE(SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL DLA_TRSYLV_L3(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO )
        CASE (FRONTEND_SOLVER_LEVEL2)
            SELECT CASE(ISOLVER)
                CASE(1)
                    IF ( MAXMN .LE. 32 ) THEN
                        CALL DLA_TRSYLV_L2_LOCAL_COPY_32(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 64) THEN
                        CALL DLA_TRSYLV_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 96) THEN
                        CALL DLA_TRSYLV_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 128) THEN
                        CALL DLA_TRSYLV_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB,X, LDX, SCALE, WORK, IINFO)
                    ELSE
                        CALL DLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    END IF
                CASE(2)
                    IF ( M .GT. 128 .OR. N .GT. 128) THEN
                        CALL DLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE
                        CALL DLA_TRSYLV_L2_LOCAL_COPY(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    END IF
                CASE(3)
                    CALL DLA_TRSYLV_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                CASE(4)
                    CALL DLA_TRSYLV_L2(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                CASE(5)
                    CALL DLA_TRSYLV_L2_UNOPT(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                CASE(6)
                    CALL DLA_TRSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB,  X, LDX, SCALE, WORK, IINFO)

            END SELECT
        CASE (FRONTEND_SOLVER_DAG)
            CALL DLA_TRSYLV_DAG(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_2STAGE)
            CALL DLA_TRSYLV_L3_2S(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_RECURSIVE)
            CALL DLA_TRSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_LAPACK)
            IINFO = 1
    END SELECT
    ...
    !
    IF (.NOT. BFACTA) THEN
        CALL DGEES("Vectors", "NoSort", DUMMY, M, A, LDA, SDIM, WORK(1), WORK(M+1), QA, LDQA,  &
        ...
        IF ( SORTEV .NE. 0 ) THEN
            CALL DLA_SORT_EV(M, A, LDA, QA, LDQA, MB, WORK, IINFO)
            IF ( IINFO .NE. 0 ) THEN
                INFO = 2
                RETURN
            END IF
    ...
    !
    IF (.NOT. BFACTB) THEN
        CALL DGEES("Vectors", "NoSort", DUMMY, N, B, LDB, SDIM, WORK(1), WORK(N+1), QB, LDQB,  &
        ...
        IF ( SORTEV .NE. 0 ) THEN
            CALL DLA_SORT_EV(N, B, LDB, QB, LDQB, NB, WORK, IINFO)
            IF ( IINFO .NE. 0 ) THEN
                INFO = 2
                RETURN
            END IF
    ...
    CALL DLA_TRANSFORM_GENERAL("Transpose", M, N, X, LDX, QA, LDQA, QB, LDQB, WORK)
    ...
    SELECT CASE(SOLVER)
        ...
        CASE (FRONTEND_SOLVER_RECURSIVE)
            CALL DLA_TRSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_LAPACK)
            CALL DTRSYL(TRANSA, TRANSB, ISGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, IINFO)
    END SELECT
    IF ( IINFO .NE. 0) THEN
        INFO = 3
        RETURN
    ...
    CALL DLA_TRANSFORM_GENERAL("NonTranspose", M, N, X, LDX, QA, LDQA, QB, LDQB, WORK)
