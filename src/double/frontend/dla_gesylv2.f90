...
!>
!> \verbatim
!> DLA_GESYLV2 solves a Sylvester equation of the following forms
!>
...
SUBROUTINE DLA_GESYLV2(FACTA, FACTB, TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, QA, LDQA, QB, LDQB, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_GESYLV2', -INFO)
        RETURN
    END IF
    ...
    SELECT CASE(SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL DLA_TRSYLV2_L3(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO )
        CASE (FRONTEND_SOLVER_LEVEL2)
            SELECT CASE(ISOLVER)
                CASE(1)
                    IF ( MAXMN .LE. 32 ) THEN
                        CALL DLA_TRSYLV2_L2_LOCAL_COPY_32(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 64) THEN
                        CALL DLA_TRSYLV2_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 96) THEN
                        CALL DLA_TRSYLV2_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 128) THEN
                        CALL DLA_TRSYLV2_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB,X, LDX, SCALE, WORK, IINFO)
                    ELSE
                        CALL DLA_TRSYLV2_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    END IF
                CASE(2)
                    IF ( M .GT. 128 .OR. N .GT. 128) THEN
                        CALL DLA_TRSYLV2_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    ELSE
                        CALL DLA_TRSYLV2_L2_LOCAL_COPY(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                    END IF
                CASE(3)
                    CALL DLA_TRSYLV2_L2_REORDER(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                CASE(4)
                    CALL DLA_TRSYLV2_L2(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                CASE(5)
                    CALL DLA_TRSYLV2_L2_UNOPT(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
                CASE(6)
                    CALL DLA_TRSYLV2_RECURSIVE(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB,  X, LDX, SCALE, WORK, IINFO)

            END SELECT
        CASE (FRONTEND_SOLVER_DAG)
            CALL DLA_TRSYLV2_DAG(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_2STAGE)
            CALL DLA_TRSYLV2_L3_2S(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO)
