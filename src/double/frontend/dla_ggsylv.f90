...
!  ===========
!
!   SUBROUTINE DLA_GGSYLV(FACTA, FACTB, TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD,
...
!           DOUBLE PRECISION QA(LDQA, *), ZA(LDZA, *), QB(LDQB, *), ZB(LDZB, *), X(LDX, *), WORK(*)
!
!> \par Purpose:
!  =============
!>
...
SUBROUTINE DLA_GGSYLV(FACTA, FACTB, TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    DOUBLE PRECISION SCALE, SGN
    DOUBLE PRECISION A(LDA,*), B(LDB, *), C(LDC, *), D(LDD,*)
    DOUBLE PRECISION QA(LDQA, *), QB(LDQB, *), ZA(LDZA, *), ZB(LDZB, *), X(LDX, *), WORK(*)
    ...
    EXTERNAL DLA_TGSYLV_L3_COLWISE
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_GGSYLV', -INFO)
        RETURN
    END IF
    ...
    SELECT CASE(SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL DLA_TGSYLV_L3_COLWISE(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, IINFO)
        ...
        CASE (FRONTEND_SOLVER_DAG)
            CALL DLA_TGSYLV_DAG(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_2STAGE)
            CALL DLA_TGSYLV_L3_2S(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_RECURSIVE)
            CALL DLA_TGSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, IINFO)
        CASE (FRONTEND_SOLVER_GARDINER_LAUB)
            CALL DLA_TGSYLV_GARDINER_LAUB(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, IINFO)
    END SELECT
    ...
    IF (.NOT. BFACTA) THEN
        ...
        IF ( AHESS ) THEN
            ASHAPE = 'H'
        END IF
        CALL DHGGES(ASHAPE, "Vectors", "Vectors", "NoSort", DUMMY, M, A, LDA, C, LDC, SDIM, WORK(1), WORK(M+1), &
            & WORK(2*M+1), QA, LDQA,  ZA, LDZA,  WORK(3*M+1), LDWORKI-3*M, DUMMY, IINFO)
        IF ( IINFO .NE. 0 ) THEN
            INFO = 1
            RETURN
        END IF
    ...
    IF (.NOT. BFACTB) THEN
        ...
        IF ( BHESS ) THEN
            BSHAPE = 'H'
        END IF
        CALL DHGGES(BSHAPE, "Vectors", "Vectors", "NoSort", DUMMY, N, B, LDB, D, LDD, SDIM, WORK(1), WORK(N+1), &
            & WORK(2*N+1), QB, LDQB,  ZB, LDZB,  WORK(3*N+1), LDWORKI-3*N, DUMMY, IINFO)
        IF ( IINFO .NE. 0 ) THEN
            INFO = 1
            RETURN
        END IF
    ...
    IF ( BTRANSA .AND. BTRANSB ) THEN   !(N,N)
        CALL DLA_TRANSFORM_GENERAL("Transpose", M, N, X, LDX, QA, LDQA, ZB, LDZB, WORK)
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN   !(T,N)
        CALL DLA_TRANSFORM_GENERAL("Transpose", M, N, X, LDX, ZA, LDZA, ZB, LDZB, WORK)
    ELSE !(T,T)
        CALL DLA_TRANSFORM_GENERAL("Transpose", M, N, X, LDX, ZA, LDZA, QB, LDQB, WORK)
    END IF
    ...
    !   T            T             QA*Y*ZB**T
    IF ( BTRANSA .AND. BTRANSB ) THEN   !(N,N)
        CALL DLA_TRANSFORM_GENERAL("NonTranspose", M, N, X, LDX, ZA, LDZA, QB, LDQB, WORK)
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB ) THEN   !(N,T)
        CALL DLA_TRANSFORM_GENERAL("NonTranspose", M, N, X, LDX, ZA, LDZA, ZB, LDZB, WORK)
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN   !(T,N)
        CALL DLA_TRANSFORM_GENERAL("NonTranspose", M, N, X, LDX, QA, LDQA, QB, LDQB, WORK)
    ELSE !(T,T)
        CALL DLA_TRANSFORM_GENERAL("NonTranspose", M, N, X, LDX, QA, LDQA, ZB, LDZB, WORK)
    END IF
