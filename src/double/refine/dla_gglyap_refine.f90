...
!  ===========
!
!   SUBROUTINE DLA_GGLYAP_REFINE(TRANS, GUESS, M, A, LDA, B, LDB, X, LDX, Y, LDY, &
...
!           DOUBLE PRECISION A(LDA,*), B(LDB,*), Q(LDQ, *), Z(LDZ, *), X(LDX, *), Y(LDY, *)
...
!>
!> \verbatim
!> DLA_GGLYAP_REFINE solves a generalized Lyapunov equation of the following forms
!>
...
!> \see DLA_TGLYAP_L3
!> \see DLA_TGLYAP_L2
!> \see DLA_TGLYAP_L3_2S
!> \see DLA_TGLYAP_DAG
!> \see DLA_TGLYAP_RECURSIVE
!
!  Authors:
!  ========
!
...
SUBROUTINE DLA_GGLYAP_REFINE(TRANS, GUESS, M, A, LDA, B, LDB, X, LDX, Y, LDY, &
    ...
    EXTERNAL DLA_TGLYAP_L3
    EXTERNAL DLA_TGLYAP_DAG
    EXTERNAL DLA_TGLYAP_L3_2S
    EXTERNAL DLA_TGLYAP_RECURSIVE
    ...
    IF (INFO .NE. 0 ) THEN
        CALL XERROR_HANDLER ('DLA_GGLYAP_REFINE', -INFO)
        RETURN
    END IF
    ...
    SELECT CASE(L3SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL DLA_TGLYAP_L3(TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO )
        CASE (FRONTEND_SOLVER_LEVEL2)
            CALL DLA_TGLYAP_L2(TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO )
        CASE (FRONTEND_SOLVER_DAG)
            CALL DLA_TGLYAP_DAG(TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO )
        CASE (FRONTEND_SOLVER_2STAGE)
            CALL DLA_TGLYAP_L3_2S(TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO )
        CASE (FRONTEND_SOLVER_RECURSIVE)
            CALL DLA_TGLYAP_RECURSIVE(TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, IINFO )
    END SELECT
    ...
    TOL = DBLE(M) * ( TWO * NRMA * NRMB) * EPS * TAU
    ...
    DO WHILE ( IT .LE. MAXIT .AND. .NOT. LASTSTEP)
        ...
        ELSE
            ...
            IF ( BTRANS ) THEN  ! Transposed case
                ...
                CALL DGEMM("N", "N", M, M, M, MONE, WORK(WS), M, B, LDB, ONE, WORK(RS), M)
            ...
            ELSE
                ...
                CALL DGEMM("N", "T", M, M, M, MONE, WORK(WS), M, B, LDB, ONE, WORK(RS), M)
        ...
        SELECT CASE(L3SOLVER)
            CASE (FRONTEND_SOLVER_LEVEL3)
                CALL DLA_TGLYAP_L3(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_LEVEL2)
                CALL DLA_TGLYAP_L2(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_DAG)
                CALL DLA_TGLYAP_DAG(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_2STAGE)
                CALL DLA_TGLYAP_L3_2S(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
            CASE (FRONTEND_SOLVER_RECURSIVE)
                CALL DLA_TGLYAP_RECURSIVE(TRANS, M, AS, LDAS, BS, LDBS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        END SELECT
    ...
    RETURN
END SUBROUTINE DLA_GGLYAP_REFINE
