...
!> \brief Iterative Refinement for the  dual Coupled Generalized Sylvester Equations.
!
!  Definition:
!  ===========
!
!   SUBROUTINE DLA_GGCSYLV_DUAL_REFINE(TRANSA, TRANSB, GUESS, SGN1, SGN2, M , N,  &
...
!       INTEGER     :: M, N, LDA, LDB, LDC, LDD, LDR, LDL, LDE, LDF, LDAS, LDBS, LDCS, LDDS, LDQ, LDZ, LDU, LDV, LDWORK, INFO
!       INTEGER :: MAXIT
...
SUBROUTINE DLA_GGCSYLV_DUAL_REFINE(TRANSA, TRANSB, GUESS, SGN1, SGN2, M , N,  &
    ...
    DOUBLE PRECISION :: NRME, NRMF, NRMA, NRMB, NRMC, NRMD, EPS , TOL
    ...
    EXTERNAL DLA_TGCSYLV_DUAL_L3
    EXTERNAL DLA_TGCSYLV_DUAL_L3_2S
    ...
    L3SOLVER = TGCSYLV_DUAL_FRONTEND_SOLVER()
    MAXMN = MAX(M,N)
    ...
    END IF
    IF (INFO .NE. 0 ) THEN
        CALL XERROR_HANDLER ('DLA_GGCSYLV_DUAL_REFINE', -INFO)
        RETURN
    END IF
    ...
    SELECT CASE(L3SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL DLA_TGCSYLV_DUAL_L3(TRANSA, TRANSB, SGN1, SGN2, M, N, A, LDA, B, LDB, C, LDC, D, LDD, E, LDE, F, LDF, &
                & SCALE, WORK, IINFO )
            ...
            SELECT CASE(ISOLVER)
                CASE(1)
                    ...
                    ELSE IF ( MAXMN .LE. 64) THEN
                        CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN1, SGN2, M, N,  A, LDA, B, LDB, &
                            & C, LDC, D, LDD,  E, LDE, F, LDF, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 96) THEN
                        CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN1, SGN2, M, N,  A, LDA, B, LDB, &
                            & C, LDC, D, LDD,  E, LDE, F, LDF, SCALE, WORK, IINFO)
                    ELSE IF ( MAXMN .LE. 128) THEN
                        CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN1, SGN2,M, N,  A, LDA, B, LDB, &
                            & C, LDC, D, LDD,  E, LDE, F, LDF, SCALE, WORK, IINFO)
                    ELSE
                        CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, M, N,  A, LDA, B, LDB, &
                            & C, LDC, D, LDD,  E, LDE, F, LDF, SCALE, WORK, IINFO)
                    END IF
                CASE(2)
                    IF ( M .GT. 128 .OR. N .GT. 128) THEN
                        CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, M, N,  A, LDA, B, LDB, &
                            & C, LDC, D, LDD,  E, LDE, F, LDF, SCALE, WORK, IINFO)
                    ELSE
                        CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY(TRANSA, TRANSB, SGN1, SGN2, M, N,  A, LDA, B, LDB, &
                            & C, LDC, D, LDD,  E, LDE, F, LDF, SCALE, WORK, IINFO)
                    END IF
                CASE(3)
                    CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, M, N,  A, LDA, B, LDB, &
                ...
                CASE(5)
                    CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, M, N,  A, LDA, B, LDB, &
                        & C, LDC, D, LDD,  E, LDE, F, LDF, SCALE, WORK, IINFO)
                CASE(6)
                    CALL DLA_TGCSYLV_DUAL_RECURSIVE(TRANSA, TRANSB, SGN1, SGN2, M, N,  A, LDA, B, LDB, &
                        & C, LDC, D, LDD,  E, LDE, F, LDF, SCALE, WORK, IINFO)
            END SELECT
        CASE (FRONTEND_SOLVER_DAG)
            CALL DLA_TGCSYLV_DUAL_DAG(TRANSA, TRANSB, SGN1, SGN2, M, N, A, LDA, B, LDB, C, LDC, D, LDD, E, LDE, F, LDF, &
                & SCALE, WORK, IINFO )

        CASE (FRONTEND_SOLVER_2STAGE)
            CALL DLA_TGCSYLV_DUAL_L3_2S(TRANSA, TRANSB, SGN1, SGN2, M, N, A, LDA, B, LDB, C, LDC, D, LDD, E, LDE, F, LDF, &
                & SCALE, WORK, IINFO )
    ...
    TOL = DSQRT(DBLE(M*N)) * ( NRMA + NRMC + NRMB + NRMD) * EPS * TAU

    NRME = DLANGE("F", M, N, E(1,1), LDE, WORK(WS))
    NRMF = DLANGE("F", M, N, F(1,1), LDF, WORK(WS))
    NRMER = ONE
    NRMFR = ONE
    IT = 1
    ...
    DO WHILE ( IT .LE. MAXIT .AND. .NOT. LASTSTEP)
        ...
        ELSE
            ...
            IF ( LSAME(TRANSA,"N")) THEN
                CALL DGEMM("T", "N", M, N, M, MONE, A, LDA, R, LDR, ONE, WORK(ERS), M)
                CALL DGEMM("T", "N", M, N, M, MONE, C, LDC, L, LDL, ONE, WORK(ERS), M)
            ELSE
                CALL DGEMM("N", "N", M, N, M, MONE, A, LDA, R, LDR, ONE, WORK(ERS), M)
                CALL DGEMM("N", "N", M, N, M, MONE, C, LDC, L, LDL, ONE, WORK(ERS), M)
            END IF

            IF ( LSAME(TRANSB,"N")) THEN
                CALL DGEMM("N", "T", M, N, N, SGN1*MONE, R, LDR, B, LDB, ONE, WORK(FRS), M)
                CALL DGEMM("N", "T", M, N, N, SGN2*MONE, L, LDL, D, LDD, ONE, WORK(FRS), M)
            ELSE
                CALL DGEMM("N", "N", M, N, N, SGN1*MONE, R, LDR, B, LDB, ONE, WORK(FRS), M)
                CALL DGEMM("N", "N", M, N, N, SGN2*MONE, L, LDL, D, LDD, ONE, WORK(FRS), M)
            END IF
        END IF
        ...
        CONVLOG(IT) =  MAX(NRMER / NRME, NRMFR / NRMF)


        IF ( (NRMER .LT. NRME * TOL) .AND. (NRMFR .LT. NRMF * TOL) .AND. IT.GT.1) THEN
            LASTSTEP = .TRUE.
        END IF
        ...
        ELSE IF ( BTRANSA .AND. .NOT. BTRANSB ) THEN   !(N,T)M
        ...
        ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN   !(T,N)M
        ...
        SELECT CASE(L3SOLVER)
            CASE (FRONTEND_SOLVER_LEVEL3)
                CALL DLA_TGCSYLV_DUAL_L3(TRANSA, TRANSB, SGN1, SGN2, M, N, AS, LDAS, BS, LDBS, CS, LDCS, DS, LDDS, &
            ...
            CASE (FRONTEND_SOLVER_LEVEL2)
                SELECT CASE(ISOLVER)
                    CASE(1)
                        IF ( MAXMN .LE. 32 ) THEN
                            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_32(TRANSA, TRANSB, SGN1, SGN2, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(ERS), M, WORK(FRS), M, SCALE, WORK(WS), IINFO)
                        ELSE IF ( MAXMN .LE. 64) THEN
                            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN1, SGN2, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(ERS), M, WORK(FRS), M, SCALE, WORK(WS), IINFO)
                        ELSE IF ( MAXMN .LE. 96) THEN
                            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN1, SGN2, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(ERS), M, WORK(FRS), M, SCALE, WORK(WS), IINFO)
                        ELSE IF ( MAXMN .LE. 128) THEN
                            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN1, SGN2,M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(ERS), M, WORK(FRS), M, SCALE, WORK(WS), IINFO)
                        ELSE
                            CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(ERS), M, WORK(FRS), M, SCALE, WORK(WS), IINFO)
                        END IF
                    CASE(2)
                        IF ( M .GT. 128 .OR. N .GT. 128) THEN
                            CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(ERS), M, WORK(FRS), M, SCALE, WORK(WS), IINFO)
                        ELSE
                            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY(TRANSA, TRANSB, SGN1, SGN2, M, N,  AS, LDAS, BS, LDBS, &
                                & CS, LDCS, DS, LDDS,  WORK(ERS), M, WORK(FRS), M, SCALE, WORK(WS), IINFO)
                        END IF
                    CASE(3)
                        CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, M, N,  AS, LDAS, BS, LDBS, &
                    ...
                    CASE(5)
                        CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, M, N,  AS, LDAS, BS, LDBS, &
                            & CS, LDCS, DS, LDDS,  WORK(ERS), M, WORK(FRS), M, SCALE, WORK(WS), IINFO)
                    CASE(6)
                        CALL DLA_TGCSYLV_DUAL_RECURSIVE(TRANSA, TRANSB, SGN1, SGN2, M, N,  AS, LDAS, BS, LDBS, &
                            & CS, LDCS, DS, LDDS,  WORK(ERS), M, WORK(FRS), M, SCALE, WORK(WS), IINFO)
                END SELECT
            CASE (FRONTEND_SOLVER_DAG)
                CALL DLA_TGCSYLV_DUAL_DAG(TRANSA, TRANSB, SGN1, SGN2, M, N, AS, LDAS, BS, LDBS, CS, LDCS, DS, LDDS, &
                    & WORK(ERS), M, WORK(FRS), M, &
                    & SCALE, WORK(WS), IINFO )

            CASE (FRONTEND_SOLVER_2STAGE)
                CALL DLA_TGCSYLV_DUAL_L3_2S(TRANSA, TRANSB, SGN1, SGN2, M, N, AS, LDAS, BS, LDBS, CS, LDCS, DS, LDDS, &
                    & WORK(ERS), M, WORK(FRS), M, &
                    & SCALE, WORK(WS), IINFO )
    ...
    END IF
    MAXIT = IT-1
    TAU = MAX(NRMER/NRME, NRMFR / NRMF)


    RETURN
END SUBROUTINE DLA_GGCSYLV_DUAL_REFINE
