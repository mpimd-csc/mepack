...
!           DOUBLE PRECISION A(LDA,*), Q(LDQ, *), X(LDX, *), Y(LDY,*), CONVLOG(*), WORK(*)
!
!> \par Purpose:
!  =============
!>
...
SUBROUTINE DLA_GELYAP_REFINE(TRANS, GUESS, M ,   A, LDA,  X, LDX, Y, LDY, &
    ...
    EXTERNAL DLA_TRLYAP_L3
    EXTERNAL DLA_TRLYAP_DAG
    EXTERNAL DLA_TRLYAP_L3_2S
    ...
    IF (INFO .NE. 0 ) THEN
        CALL XERROR_HANDLER ('DLA_GELYAP_REFINE', -INFO)
        RETURN
    END IF
    ...
    CASE (FRONTEND_SOLVER_LEVEL3)
        CALL DLA_TRLYAP_L3(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    CASE (FRONTEND_SOLVER_LEVEL2)
        CALL DLA_TRLYAP_L2(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    CASE (FRONTEND_SOLVER_DAG)
        CALL DLA_TRLYAP_DAG(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    CASE (FRONTEND_SOLVER_2STAGE)
        CALL DLA_TRLYAP_L3_2S(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    CASE (FRONTEND_SOLVER_RECURSIVE)
        CALL DLA_TRLYAP_RECURSIVE(TRANS, M, A, LDA, X, LDX, SCALE, WORK, IINFO )
    END SELECT
    ...
    DO WHILE ( IT .LE. MAXIT .AND. .NOT. LASTSTEP)
        ...
        ELSE
            ...
            IF ( BTRANS ) THEN  ! Transposed case
                CALL DGEMM("T", "N", M, M, M, MONE, A, LDA, X, LDX, ONE, WORK(RS), M)
                CALL DGEMM("N", "N", M, M, M, MONE, X, LDX, A, LDA, ONE, WORK(RS), M)
            ELSE
                CALL DGEMM("N", "N", M, M, M, MONE, A, LDA, X, LDX, ONE, WORK(RS), M)
                CALL DGEMM("N", "T", M, M, M, MONE, X, LDX, A, LDA, ONE, WORK(RS), M)
            END IF
        END IF
        ...
        SELECT CASE(L3SOLVER)
        CASE (FRONTEND_SOLVER_LEVEL3)
            CALL DLA_TRLYAP_L3(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        CASE (FRONTEND_SOLVER_LEVEL2)
            CALL DLA_TRLYAP_L2(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        CASE (FRONTEND_SOLVER_DAG)
            CALL DLA_TRLYAP_DAG(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        CASE (FRONTEND_SOLVER_2STAGE)
            CALL DLA_TRLYAP_L3_2S(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        CASE (FRONTEND_SOLVER_RECURSIVE)
            CALL DLA_TRLYAP_RECURSIVE(TRANS, M, AS, LDAS, WORK(RS), M, SCALE, WORK(WS), IINFO)
        END SELECT
    ...
    RETURN
END SUBROUTINE DLA_GELYAP_REFINE
