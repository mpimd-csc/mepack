...
!> \verbatim
!>
!> DLA_TRSYLV_L2_REORDER solves a Sylvester equation of the following forms
!>
...
SUBROUTINE DLA_TRSYLV_L2_REORDER ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TRSYLV_L2_REORDER', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, N-LH, -SGN*ONE, X(1,L), IONE, B(L,LH+1), LDB, X(1,LH+1), LDX)
                ELSE
                    CALL DGER(M, N-LH, -SGN*ONE, X(1,L),  IONE, B(L,LH+1),  LDB, X(1,LH+1), LDX)
                    CALL DGER(M, N-LH, -SGN*ONE, X(1,LH), IONE, B(LH,LH+1), LDB, X(1,LH+1), LDX)
                ENDIF
            END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( L .GT. 1 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, LE, -SGN*ONE, X(1,L), IONE,  B(1,L), IONE, X(1,1), LDX)
                ELSE
                    CALL DGER(M, LE, -SGN*ONE, X(1,L),  IONE, B(1,L), IONE, X(1,1), LDX)
                    CALL DGER(M, LE, -SGN*ONE, X(1,LH), IONE, B(1,LH), IONE, X(1,1), LDX)
                ENDIF
            END IF
            L = L - 1
        END DO
