...
!  Definition:
!  ===========
!       SUBROUTINE DLA_TGCSYLV_DUAL_L2 ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
!>  \li Replaced all BLAS calls except of DGER by Fortran intrinsic,
...
SUBROUTINE DLA_TGCSYLV_DUAL_L2 ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGCSYLV_DUAL_L2', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE ( K .LE. M )
                ...
                IF ( KH .LT. M ) THEN
                    IF (KB .EQ. IONE .AND. LB .EQ. IONE) THEN
                        E(KH+1:M, L) = E(KH+1:M, L) - A(K,KH+1:M) * E(K,L)
                        E(KH+1:M, L) = E(KH+1:M, L) - C(K,KH+1:M) * F(K,L)

                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        E(KH+1:M, L)  = E(KH+1:M, L) -  A(K,KH+1:M) * E(K,L)
                        E(KH+1:M, LH) = E(KH+1:M, LH) - A(K,KH+1:M) * E(K,LH)

                        E(KH+1:M, L)  = E(KH+1:M, L)  - C(K,KH+1:M) * F(K,L)
                        E(KH+1:M, LH) = E(KH+1:M, LH) - C(K,KH+1:M) * F(K,LH)

                    ELSE IF ( KB .NE. IONE .AND. LB .EQ. IONE) THEN
                        E(KH+1:M,L) = E(KH+1:M,L) - A(K,KH+1:M)*E(K,L) - A(KH,KH+1:M)*E(KH,L)
                        E(KH+1:M,L) = E(KH+1:M,L) - C(K,KH+1:M)*F(K,L) - C(KH,KH+1:M)*F(KH,L)
                    ELSE
                        E(KH+1:M,L)  = E(KH+1:M,L)  - A(K,KH+1:M) * E(K,L)  - A(KH,KH+1:M) * E(KH,L)
                        E(KH+1:M,LH) = E(KH+1:M,LH) - A(K,KH+1:M) * E(K,LH) - A(KH,KH+1:M) * E(KH,LH)
                        E(KH+1:M,L)  = E(KH+1:M,L)  - C(K,KH+1:M) * F(K,L)  - C(KH,KH+1:M) * F(KH,L)
                        E(KH+1:M,LH) = E(KH+1:M,LH) - C(K,KH+1:M) * F(K,LH) - C(KH,KH+1:M) * F(KH,LH)
                    END IF
                END IF
            ...
            IF ( L .GT. 1 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, LE, -SGN1*ONE, E(1,L), IONE, B(1,L), IONE, F(1,1), LDF)
                    CALL DGER(M, LE, -SGN2*ONE, F(1,L), IONE, D(1,L), IONE, F(1,1), LDF)
                ELSE
                    CALL DGER(M, LE, -SGN1*ONE, E(1,L),  IONE, B(1,L),  IONE, F(1,1), LDF)
                    CALL DGER(M, LE, -SGN1*ONE, E(1,LH), IONE, B(1,LH), IONE, F(1,1), LDF)
                    CALL DGER(M, LE, -SGN2*ONE, F(1,L),  IONE, D(1,L),  IONE, F(1,1), LDF)
                    CALL DGER(M, LE, -SGN2*ONE, F(1,LH), IONE, D(1,LH), IONE, F(1,1), LDF)
                ENDIF
            END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, N-LH, -SGN1*ONE, E(1,L), IONE, B(L,LH+1), LDB, F(1,LH+1), LDF)
                    CALL DGER(M, N-LH, -SGN2*ONE, F(1,L), IONE, D(L,LH+1), LDD, F(1,LH+1), LDF)

                ELSE
                    CALL DGER(M, N-LH, -SGN1*ONE, E(1,L),  IONE, B(L,LH+1),  LDB, F(1,LH+1), LDF)
                    CALL DGER(M, N-LH, -SGN1*ONE, E(1,LH), IONE, B(LH,LH+1), LDB, F(1,LH+1), LDF)

                    CALL DGER(M, N-LH, -SGN2*ONE, F(1,L),  IONE, D(L,LH+1),  LDD, F(1,LH+1), LDF)
                    CALL DGER(M, N-LH, -SGN2*ONE, F(1,LH), IONE, D(LH,LH+1), LDD, F(1,LH+1), LDF)
                ENDIF
            END IF
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    ...
                    CALL DGER(M, N-LH, -SGN2*ONE, F(1,L), IONE, D(L,LH+1), LDD, F(1,LH+1), LDF)
                ELSE
                    CALL DGER(M, N-LH, -SGN1, E(1,L),  IONE, B(L,LH+1),  LDB, F(1,LH+1), LDF)
                    CALL DGER(M, N-LH, -SGN1, E(1,LH), IONE, B(LH,LH+1), LDB, F(1,LH+1), LDF)

                    CALL DGER(M, N-LH, -SGN2, F(1,L),  IONE, D(L,LH+1),  LDD, F(1,LH+1), LDF)
                    CALL DGER(M, N-LH, -SGN2, F(1,LH), IONE, D(LH,LH+1), LDD, F(1,LH+1), LDF)
                ENDIF
            END IF
