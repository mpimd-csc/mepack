...
!> \verbatim
!>
!> DLA_TRSYLV2_L2_LOCAL_COPY solves a generalized Sylvester equation of the following forms
!>
...
SUBROUTINE DLA_TRSYLV2_L2_LOCAL_COPY ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TRSYLV2_L2_LOCAL_COPY', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    ...
                    CALL DTRMV('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORK, IONE)
                    DO IT = 1,M-1
                ...
                ELSE
                    ...
                    CALL DTRMV('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORK, IONE)
                    CALL DTRMV('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORK(M+1), IONE)
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    WORK(1:M) = XL(1:M, L)
                    CALL DTRMV("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORK, IONE)
                    DO IT = 1, M-1
                ...
                ELSE
                    ...
                    WORK(M+1:2*M) = XL(1:M, LH)
                    CALL DTRMV("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORK, IONE)
                    CALL DTRMV("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORK(M+1), IONE)
