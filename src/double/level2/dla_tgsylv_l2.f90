...
SUBROUTINE DLA_TGSYLV_L2 ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGSYLV_L2', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( L .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( K .GT. IONE ) THEN
                    IF (KB .EQ. IONE .AND. LB .EQ. IONE) THEN
                        CALL DAXPY(K-1, -      X(K,L) * B(L,L), A(1,K), 1, X(1,L), 1)
                        CALL DAXPY(K-1, -SGN * X(K,L) * D(L,L), C(1,K), 1, X(1,L), 1)
                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        CALL DAXPY(K-1, -      (X(K,L) * B(L,L) + X(K,LH) * B(LH,L)), A(1,K), 1, X(1,L), 1)
                        CALL DAXPY(K-1, -SGN * (X(K,L) * D(L,L) + X(K,LH) * D(LH,L)), C(1,K), 1, X(1,L), 1)
                        CALL DAXPY(K-1, -      (X(K,L) * B(L,LH) + X(K,LH) * B(LH,LH)), A(1,K), 1, X(1,LH), 1)
                        CALL DAXPY(K-1, -SGN * (X(K,L) * D(L,LH) + X(K,LH) * D(LH,LH)), C(1,K), 1, X(1,LH), 1)
                    ELSE IF ( KB .NE. IONE .AND. LB .EQ. IONE) THEN
                        CALL DAXPY(K-1, -      (X(K,L)  * B(L,L)), A(1,K), 1,  X(1,L), 1)
                        CALL DAXPY(K-1, -      (X(KH,L) * B(L,L)), A(1,KH), 1, X(1,L), 1)
                        CALL DAXPY(K-1, -SGN*  (X(K,L)  * D(L,L)), C(1,K), 1 , X(1,L), 1)
                        CALL DAXPY(K-1, -SGN*  (X(KH,L) * D(L,L)), C(1,KH), 1, X(1,L), 1)
                    ELSE
                        ...
                        CALL DAXPY(K-1, -      MAT(1,1) , A(1,K) , 1,  X(1,L), 1)
                        CALL DAXPY(K-1, -      MAT(2,1) , A(1,KH), 1,  X(1,L), 1)
                        CALL DAXPY(K-1, - SGN *MAT(1,2) , C(1,K) , 1,  X(1,L), 1)
                        CALL DAXPY(K-1, - SGN *MAT(2,2) , C(1,KH), 1,  X(1,L), 1)

                        CALL DAXPY(K-1, -      MAT(3,1) , A(1,K) , 1,  X(1,LH), 1)
                        CALL DAXPY(K-1, -      MAT(4,1) , A(1,KH), 1,  X(1,LH), 1)
                        CALL DAXPY(K-1, - SGN *MAT(3,2) , C(1,K) , 1,  X(1,LH), 1)
                        CALL DAXPY(K-1, - SGN *MAT(4,2) , C(1,KH), 1,  X(1,LH), 1)
                    END IF
                END IF
                K = K - 1
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    ...
                    WORK(1:M) = X(1:M,L)
                    CALL DTRMV('Upper', 'NoTrans','NoUnit', M, C, LDC, WORK, IONE)
                    CALL DGER(M, N-LH, -SGN*ONE, WORK, IONE, D(L,LH+1), LDD, X(1,LH+1), LDX)
                ...
                ELSE
                    ...
                    WORK(M+1:2*M) = X(1:M,LH)
                    CALL DTRMV('Upper', 'NoTrans','NoUnit', M, C, LDC, WORK, IONE)
                    CALL DTRMV('Upper', 'NoTrans','NoUnit', M, C, LDC, WORK(M+1), IONE)
                    CALL DGER(M, N-LH, -SGN*ONE, WORK, IONE, D(L,LH+1), LDD, X(1,LH+1), LDX)
                    CALL DGER(M, N-LH, -SGN*ONE, WORK(M+1), IONE, D(LH,LH+1), LDD, X(1,LH+1), LDX)
                ENDIF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( K .GT. 1 ) THEN
                    IF ( KB .EQ. IONE .AND. LB .EQ. IONE ) THEN
                        CALL DAXPY(KE, -     (B(L,L) * X(K,L)), A(1, K), 1, X(1,L), 1)
                        CALL DAXPY(KE, - SGN*(D(L,L) * X(K,L)), C(1, K), 1, X(1,L), 1)

                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        CALL DAXPY(KE, -     (B(L,L) * X(K,L) + X(K,LH) * B(L,LH)), A(1, K), 1, X(1,L), 1)
                        CALL DAXPY(KE, - SGN*(D(L,L) * X(K,L) + X(K,LH) * D(L,LH)), C(1, K), 1, X(1,L), 1)
                        CALL DAXPY(KE, -     (B(LH,L) * X(K,L) + X(K,LH) * B(LH,LH)), A(1, K), 1, X(1,LH), 1)
                        CALL DAXPY(KE, - SGN*(D(LH,L) * X(K,L) + X(K,LH) * D(LH,LH)), C(1, K), 1, X(1,LH), 1)

                    ELSE IF ( KB .NE. IONE .AND. LB .EQ. IONE) THEN
                        CALL DAXPY(KE, -     (B(L,L) * X(K,L))  , A(1, K), 1, X(1,L), 1)
                        CALL DAXPY(KE, -     (B(L,L) * X(KH,L)) , A(1, KH), 1, X(1,L), 1)
                        CALL DAXPY(KE, -SGN* (D(L,L) * X(K,L))  , C(1, K), 1, X(1,L), 1)
                        CALL DAXPY(KE, -SGN* (D(L,L) * X(KH,L)) , C(1, KH), 1, X(1,L), 1)
                    ...
                    ELSE
                        ...
                        CALL DAXPY(KE, -     MAT(1,1) , A(1, K),  1, X(1,L), 1)
                        CALL DAXPY(KE, -     MAT(2,1) , A(1, KH), 1, X(1,L), 1)
                        CALL DAXPY(KE, -SGN* MAT(1,2) , C(1, K),  1, X(1,L), 1)
                        CALL DAXPY(KE, -SGN* MAT(2,2) , C(1, KH), 1, X(1,L), 1)
                        CALL DAXPY(KE, -     MAT(3,1) , A(1, K),  1, X(1,LH), 1)
                        CALL DAXPY(KE, -     MAT(4,1) , A(1, KH), 1, X(1,LH), 1)
                        CALL DAXPY(KE, -SGN* MAT(3,2) , C(1, K),  1, X(1,LH), 1)
                        CALL DAXPY(KE, -SGN* MAT(4,2) , C(1, KH), 1, X(1,LH), 1)
                    END IF
                END IF
                K = K - 1
            END DO
            ...
            IF ( L .GT. 1 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    ...
                    WORK(1:M) = X(1:M,L)
                    CALL DTRMV('Upper', 'NoTrans','NoUnit', M, C, LDC, WORK, IONE)
                    CALL DGER(M, LE, -SGN*ONE, WORK, IONE, D(1,L), IONE, X(1,1), LDX)
                ...
                ELSE
                    ...
                    CALL DGER(M, LE, -SGN*ONE, WORK, IONE, D(1,L), IONE, X(1,1), LDX)
                    CALL DGER(M, LE, -SGN*ONE, WORK(M+1), IONE, D(1,LH), IONE, X(1,1), LDX)
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            DO WHILE ( K .LE. M )
                ...
                IF ( KH .LT. M ) THEN
                    IF (KB .EQ. IONE .AND. LB .EQ. IONE) THEN
                        CALL DAXPY(M-KH, -     (B(L,L) * X(K,L)), A(K, KH+1), LDA, X(KH+1,L), 1)
                        CALL DAXPY(M-KH, - SGN*(D(L,L) * X(K,L)), C(K, KH+1), LDC, X(KH+1,L), 1)

                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        CALL DAXPY(M-KH, -     (B(L,L) * X(K,L) + X(K,LH) * B(LH,L)), A(K, KH+1), LDA, X(KH+1,L), 1)
                        CALL DAXPY(M-KH, - SGN*(D(L,L) * X(K,L) + X(K,LH) * D(LH,L)), C(K, KH+1), LDC, X(KH+1,L), 1)
                        CALL DAXPY(M-KH, -     (B(L,LH) * X(K,L) + X(K,LH) * B(LH,LH)), A(K, KH+1), LDA, X(KH+1,LH), 1)
                        CALL DAXPY(M-KH, - SGN*(D(L,LH) * X(K,L) + X(K,LH) * D(LH,LH)), C(K, KH+1), LDC, X(KH+1,LH), 1)

                    ELSE IF ( KB .NE. IONE .AND. LB .EQ. IONE) THEN
                        CALL DAXPY(M-KH, -     (B(L,L) * X(K,L)) ,  A(K, KH+1), LDA, X(KH+1,L), 1)
                        CALL DAXPY(M-KH, -     (B(L,L) * X(KH,L)) , A(KH, KH+1), LDA, X(KH+1,L), 1)
                        CALL DAXPY(M-KH, -SGN *(D(L,L) * X(K,L)) ,  C(K, KH+1),  LDC, X(KH+1,L), 1)
                        CALL DAXPY(M-KH, -SGN *(D(L,L) * X(KH,L)) , C(KH, KH+1), LDC, X(KH+1,L), 1)
                    ...
                    ELSE
                        ...
                        CALL DAXPY(M-KH, - MAT(1,1), A(K,  KH+1), LDA, X(KH+1, L), 1)
                        CALL DAXPY(M-KH, - MAT(2,1), A(KH, KH+1), LDA, X(KH+1, L), 1)
                        CALL DAXPY(M-KH, - SGN * MAT(1,2), C(K,  KH+1), LDC, X(KH+1, L), 1)
                        CALL DAXPY(M-KH, - SGN * MAT(2,2), C(KH, KH+1), LDC, X(KH+1, L), 1)

                        CALL DAXPY(M-KH, - MAT(3,1), A(K,  KH+1), LDA, X(KH+1, LH), 1)
                        CALL DAXPY(M-KH, - MAT(4,1), A(KH, KH+1), LDA, X(KH+1, LH), 1)
                        CALL DAXPY(M-KH, - SGN * MAT(3,2), C(K,  KH+1), LDC, X(KH+1, LH), 1)
                        CALL DAXPY(M-KH, - SGN * MAT(4,2), C(KH, KH+1), LDC, X(KH+1, LH), 1)
            ...
            IF ( LH .LT. N ) THEN
                ...
                IF ( LB .EQ. IONE ) THEN
                    WORK(1:M) = X(1:M, L)
                    CALL DTRMV("Upper", "Trans", "NoUnit", M, C, LDC, WORK, IONE)
                    CALL DGER(M, N-LH, -SGN*ONE, WORK, IONE, D(L,LH+1), LDD, X(1,LH+1), LDX)
                ...
                ELSE
                    ...
                    WORK(M+1:2*M) = X(1:M, LH)
                    CALL DTRMV("Upper", "Trans", "NoUnit", M, C, LDC, WORK, IONE)
                    CALL DTRMV("Upper", "Trans", "NoUnit", M, C, LDC, WORK(M+1), IONE)
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE ( K .LE. M )
                ...
                IF ( KH .LT. M  ) THEN
                    ...
                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        CALL DAXPY(M-KH, -     (B(L,L) * X(K,L) + X(K,LH) * B(L,LH)), A(K, KH+1), LDA, X(KH+1,L), 1)
                        CALL DAXPY(M-KH, - SGN*(D(L,L) * X(K,L) + X(K,LH) * D(L,LH)), C(K, KH+1), LDC, X(KH+1,L), 1)
                        CALL DAXPY(M-KH, -     (B(LH,L) * X(K,L) + X(K,LH) * B(LH,LH)), A(K, KH+1), LDA, X(KH+1,LH), 1)
                        CALL DAXPY(M-KH, - SGN*(D(LH,L) * X(K,L) + X(K,LH) * D(LH,LH)), C(K, KH+1), LDC, X(KH+1,LH), 1)
