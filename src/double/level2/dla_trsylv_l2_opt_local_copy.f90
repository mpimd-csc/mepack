...
!> \verbatim
!>
!> DLA_TRSYLV_L2_LOCAL_COPY solves a Sylvester equation of the following forms
!>
...
SUBROUTINE DLA_TRSYLV_L2_LOCAL_COPY ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TRSYLV_L2_LOCAL_COPY', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, N-LH, -SGN*ONE, XL(1,L), IONE, BL(L,LH+1), LDBL, XL(1,LH+1), LDXL)
                ELSE
                    CALL DGER(M, N-LH, -SGN*ONE, XL(1,L),  IONE, BL(L,LH+1),  LDBL, XL(1,LH+1), LDXL)
                    CALL DGER(M, N-LH, -SGN*ONE, XL(1,LH), IONE, BL(LH,LH+1), LDBL, XL(1,LH+1), LDXL)
                ENDIF
            END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( L .GT. 1 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, LE, -SGN*ONE, XL(1,L), IONE,  BL(1,L), IONE, XL(1,1), LDXL)
                ELSE
                    CALL DGER(M, LE, -SGN*ONE, XL(1,L),  IONE, BL(1,L), IONE, XL(1,1), LDXL)
                    CALL DGER(M, LE, -SGN*ONE, XL(1,LH), IONE, BL(1,LH), IONE, XL(1,1), LDXL)
                ENDIF
            END IF
            L = L - 1
        END DO
