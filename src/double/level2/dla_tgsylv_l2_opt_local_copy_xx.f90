...
DOUBLE PRECISION AL(MAXNB,MAXNB), BL(MAXNB,MAXNB), CL(MAXNB,MAXNB), DL(MAXNB,MAXNB), XL(MAXNB,MAXNB), WORKA(MAXNB)
...
IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
    ...
    DO WHILE ( L .LE. N )
        ...
        IF ( LH .LT. N ) THEN
            IF ( LB .EQ. IONE ) THEN
                ! WORKA(1:M) = XL(1:M,L)
                ! CALL DTRMV_SEQ('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORKA, IONE)
                ! DO IT = 1,M-1
                ...
                ! WORKA(1:M) = XL(1:M,L)
                ! CALL DTRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORKA, IONE)
            ...
            ELSE
                ...
                ! WORKB(1:M) = XL(1:M,LH)
                ! CALL DTRMV_SEQ('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORKA, IONE)
                ! CALL DTRMV_SEQ('Upper', 'NoTrans','NoUnit', M, AL, LDAL, WORKB, IONE)
                ! DO IT = 1,M-1
                ! IF (AL(IT+1,IT).NE.ZERO) THEN
                ! WORKA(IT+1) = WORKA(IT+1) + AL(IT+1,IT) * XL(IT,L)
                ...
                ! WORKB(1:M) = XL(1:M,LH)
                ! CALL DTRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORKA, IONE)
                ! CALL DTRMV_SEQ('Upper', 'NoTrans','NoUnit', M, CL, LDCL, WORKB, IONE)
        ...
        IF ( L .GT. 1 ) THEN
            ...
            ELSE
                ...
                DO IT = 1,M-1
                    ...
                    END IF
                END DO
                ! CALL DGER(M, LE, -ONE, WORKA, IONE, BL(1,L), IONE, XL(1,1), LDXL)
                ! CALL DGER(M, LE, -ONE, WORKB, IONE, BL(1,LH), IONE, XL(1,1), LDXL)
                ...
                ! CALL DGER(M, LE, -SGN*ONE, WORKA, IONE, DL(1,L), IONE, XL(1,1), LDXL)
                ! CALL DGER(M, LE, -SGN*ONE, WORKB, IONE, DL(1,LH), IONE, XL(1,1), LDXL)
                DO IT = 1, LE
...
ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
    ...
    DO WHILE ( L .LE. N )
        ...
        IF ( LH .LT. N ) THEN
            IF ( LB .EQ. IONE ) THEN
                WORKA(1:M) = XL(1:M, L)
                CALL DTRMV_SEQ("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORKA, IONE)
                DO IT = 1, M-1
                ...
                WORKB(1:M) = XL(1:M, L)
                CALL DTRMV_SEQ("Lower", "NoTrans", "NoUnit", M, CL, LDCL, WORKB, IONE)

                ! CALL DGER(M, N-LH, -ONE, WORK, IONE, BL(L,LH+1), LDBL, XL(1,LH+1), LDXL)
                ! CALL DGER(M, N-LH, -SGN*ONE, WORKB, IONE, DL(L,LH+1), LDDL, XL(1,LH+1), LDXL)
            ...
            ELSE
                ...
                WORKB(1:M) = XL(1:M, LH)
                CALL DTRMV_SEQ("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORKA, IONE)
                CALL DTRMV_SEQ("Lower", "NoTrans", "NoUnit", M, AL, LDAL, WORKB, IONE)
                ...
                DO IT = 1, M-1
                    ...
                    END IF
                END DO
                ! CALL DGER(M, N-LH, -ONE, WORKA, IONE, BL(L,LH+1), LDBL, XL(1,LH+1), LDXL)
                ! CALL DGER(M, N-LH, -ONE, WORKB, IONE, BL(LH,LH+1), LDBL, XL(1,LH+1), LDXL)
                ...
                WORKA(1:M) = XL(1:M, L)
                WORKB(1:M) = XL(1:M, LH)
                CALL DTRMV_SEQ("Lower", "NoTrans", "NoUnit", M, CL, LDCL, WORKA, IONE)
                CALL DTRMV_SEQ("Lower", "NoTrans", "NoUnit", M, CL, LDCL, WORKB, IONE)

                ! CALL DGER(M, N-LH, -SGN*ONE, WORKA, IONE, DL(L,LH+1), LDDL, XL(1,LH+1), LDXL)
                ! CALL DGER(M, N-LH, -SGN*ONE, WORKB, IONE, DL(LH,LH+1), LDDL, XL(1,LH+1), LDXL)
    ...
    DO WHILE ( L .GT. 0 )
        ...
        IF ( L .GT. IONE ) THEN
            IF ( LB .EQ. IONE ) THEN
                ...
                ! CALL DGER(M, LE, -ONE, WORKA, IONE, BL(1,L), IONE, XL(1,1), LDXL)
                ! CALL DGER(M, LE, -SGN*ONE, WORKB, IONE, DL(1,L), IONE, XL(1,1), LDXL)
                DO IT = 1, LE
