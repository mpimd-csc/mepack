...
!> \verbatim
!>
!> DLA_TRSYLV_L2 solves a Sylvester equation of the following forms
!>
...
SUBROUTINE DLA_TRSYLV_L2 ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TRSYLV_L2', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            IF ( K .GT. 1 ) THEN
                ! First half AXB
                IF ( KB .EQ. 1 ) THEN
                    CALL DGER(KE, N, -ONE, A(1,K) , IONE, X(K,1), LDX, X(1,1), LDX)
                ELSE
                    CALL DGER(KE, N, -ONE, A(1,K) , IONE, X(K,1), LDX, X(1,1), LDX)
                    CALL DGER(KE, N, -ONE, A(1,KH) , IONE, X(KH,1), LDX, X(1,1), LDX)
                ENDIF
            END IF
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( K .LE. M )
            ...
            IF ( KH .LT. M ) THEN
                IF (KB .EQ. IONE ) THEN
                    CALL DGER(M-KH, N, -ONE, A(K,KH+1), LDA, X(K,1), LDX, X(KH+1, 1), LDX)
                ELSE
                    CALL DGER(M-KH, N, -ONE, A(K,KH+1), LDA, X(K,1), LDX, X(KH+1, 1), LDX)
                    CALL DGER(M-KH, N, -ONE, A(KH,KH+1), LDA, X(KH,1), LDX, X(KH+1, 1), LDX)
                END IF
            END IF
            K = KH + 1
        END DO
