...
!  Definition:
!  ===========
!       SUBROUTINE DLA_TGCSYLV_L2_LOCAL_COPY ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
SUBROUTINE DLA_TGCSYLV_L2_LOCAL_COPY ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, N-LH, -SGN1*ONE, FL(1,L), IONE, BL(L,LH+1), LDBL, EL(1,LH+1), LDEL)
                    CALL DGER(M, N-LH, -SGN2*ONE, FL(1,L), IONE, DL(L,LH+1), LDDL, FL(1,LH+1), LDFL)

                ELSE
                    CALL DGER(M, N-LH, -SGN1*ONE, FL(1,L), IONE, BL(L,LH+1), LDBL, EL(1,LH+1), LDEL)
                    CALL DGER(M, N-LH, -SGN1*ONE, FL(1,LH), IONE, BL(LH,LH+1), LDBL, EL(1,LH+1), LDEL)

                    CALL DGER(M, N-LH, -SGN2*ONE, FL(1,L), IONE, DL(L,LH+1), LDDL, FL(1,LH+1), LDFL)
                    CALL DGER(M, N-LH, -SGN2*ONE, FL(1,LH), IONE, DL(LH,LH+1), LDDL, FL(1,LH+1), LDFL)
                ENDIF
            END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( L .GT. 1 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, LE, -SGN1*ONE, FL(1,L), IONE, BL(1,L), IONE, EL(1,1), LDEL)
                    CALL DGER(M, LE, -SGN2*ONE, FL(1,L), IONE, DL(1,L), IONE, FL(1,1), LDFL)
                ELSE
                    CALL DGER(M, LE, -SGN1*ONE, FL(1,L), IONE, BL(1,L), IONE, EL(1,1), LDEL)
                    CALL DGER(M, LE, -SGN1*ONE, FL(1,LH), IONE, BL(1,LH), IONE, EL(1,1), LDEL)
                    CALL DGER(M, LE, -SGN2*ONE, FL(1,L), IONE, DL(1,L), IONE, FL(1,1), LDFL)
                    CALL DGER(M, LE, -SGN2*ONE, FL(1,LH), IONE, DL(1,LH), IONE, FL(1,1), LDFL)
                ENDIF
            END IF
            L = L - 1
        END DO
