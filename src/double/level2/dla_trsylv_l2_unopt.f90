...
!> \brief Level-2 Bartels-Stewart Algorithm for the Sylvester equation (unoptimized)
!
!  Definition:
!  ===========
!
...
!> \verbatim
!>
!> DLA_TRSYLV_L2_UNOPT solves a Sylvester equation of the following forms
!>
...
SUBROUTINE DLA_TRSYLV_L2_UNOPT ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TRSYLV_L2_UNOPT', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            DO WHILE ( L .LE. N )
                ...
                ! in current row
                IF ( LH .LT. N ) THEN
                    CALL DGEMM('N','N', KB, N-LH, LB, -ONE*SGN, X(K,L), LDX, B(L,LH+1), LDB, ONE, X(K,LH+1), LDX)
                END IF
            ...
            ! Update January 2021
            IF ( K .GT. 1 ) THEN
                CALL DGEMM('N', 'N', KE, N, KB, -ONE, A(1,K), LDA, X(K,1), LDX, ONE, X(1,1), LDX)
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            DO WHILE ( L .GT. 0 )
                ...
                ! Update January 2021
                IF ( L .GT. 0 ) THEN
                    CALL DGEMM('N','T', KB, LE, LB, -ONE*SGN, X(K,L), LDX, B(1,L), LDB, ONE, X(K,1), LDX)
                END IF
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( K .LE. M )
            ...
            IF ( KH .LT. M ) THEN
                CALL DGEMM('T', 'N', M-KH, N, KB, -ONE, A(K,KH+1), LDA, X(K,1), LDX, ONE, X(KH+1,1), LDX)
            END IF
            K = KH + 1
        END DO
