...
!  Definition:
!  ===========
!       SUBROUTINE DLA_TGCSYLV_L2_REORDER ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
!> \li Column first computation of the solution.
!
!  Authors:
!  ========
!
...
SUBROUTINE DLA_TGCSYLV_L2_REORDER ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGCSYLV_L2_REORDER', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( K .GT. IONE ) THEN
                    ...
                    ELSE
                        ...
                        F(1:K-1,L) = F(1:K-1,L) - C(1:K-1,K) * E(K,L) - C(1:K-1,KH) * E(KH,L)
                        F(1:K-1,LH) = F(1:K-1,LH) - C(1:K-1,K) * E(K,LH) - C(1:K-1,KH) * E(KH,LH)
            ...
            IF ( LH .LT. N ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, N-LH, -SGN1*ONE, F(1,L), IONE, B(L,LH+1), LDB, E(1,LH+1), LDE)
                    CALL DGER(M, N-LH, -SGN2*ONE, F(1,L), IONE, D(L,LH+1), LDD, F(1,LH+1), LDF)

                ELSE
                    CALL DGER(M, N-LH, -SGN1*ONE, F(1,L), IONE, B(L,LH+1), LDB, E(1,LH+1), LDE)
                    CALL DGER(M, N-LH, -SGN1*ONE, F(1,LH), IONE, B(LH,LH+1), LDB, E(1,LH+1), LDE)
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( L .GT. 1 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    CALL DGER(M, LE, -SGN1*ONE, F(1,L), IONE, B(1,L), IONE, E(1,1), LDE)
                    CALL DGER(M, LE, -SGN2*ONE, F(1,L), IONE, D(1,L), IONE, F(1,1), LDF)
                ELSE
                    CALL DGER(M, LE, -SGN1*ONE, F(1,L), IONE, B(1,L), IONE, E(1,1), LDE)
                    CALL DGER(M, LE, -SGN1*ONE, F(1,LH), IONE, B(1,LH), IONE, E(1,1), LDE)
                    CALL DGER(M, LE, -SGN2*ONE, F(1,L), IONE, D(1,L), IONE, F(1,1), LDF)
            ...
            DO WHILE ( K .LE. M )
                ...
                IF ( KH .LT. M ) THEN
                    IF (KB .EQ. IONE .AND. LB .EQ. IONE) THEN
                        ...
                        F(KH+1:M, L) = F(KH+1:M, L) - C(K,KH+1:M) * E(K,L)
                    ...
                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        ...
                        F(KH+1:M, L) = F(KH+1:M, L) - C(K,KH+1:M) * E(K,L)
                        F(KH+1:M, LH) = F(KH+1:M, LH) - C(K,KH+1:M) * E(K,LH)

                    ELSE IF ( KB .NE. IONE .AND. LB .EQ. IONE) THEN
                        E(KH+1:M,L) = E(KH+1:M,L) - A(K,KH+1:M)*E(K,L) - A(KH,KH+1:M)*E(KH,L)
                        F(KH+1:M,L) = F(KH+1:M,L) - C(K,KH+1:M)*E(K,L) - C(KH,KH+1:M)*E(KH,L)
                    ELSE
                        E(KH+1:M,L) = E(KH+1:M,L) - A(K,KH+1:M) * E(K,L) - A(KH,KH+1:M) * E(KH,L)
                        E(KH+1:M,LH) = E(KH+1:M,LH) - A(K,KH+1:M) * E(K,LH) - A(KH,KH+1:M) * E(KH,LH)
                        F(KH+1:M,L) = F(KH+1:M,L) - C(K,KH+1:M) * E(K,L) - C(KH,KH+1:M) * E(KH,L)
                        F(KH+1:M,LH) = F(KH+1:M,LH) - C(K,KH+1:M) * E(K,LH) - C(KH,KH+1:M) * E(KH,LH)
                    END IF
                END IF
