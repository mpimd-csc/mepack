...
SUBROUTINE DLA_TGSYLV_L2_ROWWISE ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGSYLV_L2_ROWWISE', -INFO)
        RETURN
    END IF
        ...
        DO WHILE (K .GT. 0)
            ...
            DO WHILE ( L .LE. N )
                ...
                IF ( LH .LT. N ) THEN
                    ...
                    CALL DGEMM('N','N', KB, LB, KB, ONE, C(K,K), LDC, X(K,L), LDX, ZERO, MAT, KB )
                    CALL DGEMM('N','N', KB, N-LH, LB, -ONE*SGN, MAT, KB, D(L,LH+1), LDD, ONE, X(K,LH+1), LDX)
            ...
            IF ( K .GT. 1 ) THEN
                ...
                CALL DLACPY('All', KB, N, X(K,1), LDX, WORK, 2)
                CALL DTRMM('Right', 'Upper', 'NoTrans', 'NoUnit', KB, N, ONE, D, LDD, WORK, 2)
                CALL DLA_QTRMM2('Right','NoTrans',KB, N, ONE, D, LDD, X(K,1), LDX, WORK, 2)
                CALL DGEMM('N', 'N', KE, N, KB, -ONE*SGN, C(1,K), LDC, WORK, 2, ONE, X(1,1), LDX)
            END IF
...
SUBROUTINE DLA_TGSYLV_L2_UNOPT ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, INFO)
    ...
    IF (INFO.EQ.-1) THEN
        CALL DLA_TGSYLV_L2_ROWWISE ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, INFO)
        RETURN
    END IF
    INFO = 0
    BTRANSA = LSAME(TRANSA, 'T')
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGSYLV_L2_UNOPT', -INFO)
        RETURN
    END IF
