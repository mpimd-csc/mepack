...
!  Definition:
!  ===========
!       SUBROUTINE DLA_TGCSYLV_DUAL_L2_LOCAL_COPY ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
!>         the input data. For this reason M and N are limited to 128.
!>
!
!  Arguments:
!  ==========
...
SUBROUTINE DLA_TGCSYLV_DUAL_L2_LOCAL_COPY ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGCSYLV_DUAL_L2_LOCAL_COPY', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( L .GT. 1 ) THEN
                IF ( LB .EQ. IONE ) THEN
                    ...
                    END DO
                    ! CALL DGER(M, LE, -ONE, EL(1,L), IONE, BL(1,L), IONE, FL(1,1), LDFL)
                    ! CALL DGER(M, LE, -ONE, FL(1,L), IONE, DL(1,L), IONE, FL(1,1), LDFL)
                ELSE
                    DO IT = 1, LE
                    ...
                    ! CALL DGER(M, LE, -ONE, EL(1,L),  IONE, BL(1,L),  IONE, FL(1,1), LDFL)
                    ! CALL DGER(M, LE, -ONE, EL(1,LH), IONE, BL(1,LH), IONE, FL(1,1), LDFL)
                    ! CALL DGER(M, LE, -ONE, FL(1,L),  IONE, DL(1,L),  IONE, FL(1,1), LDFL)
                    ! CALL DGER(M, LE, -ONE, FL(1,LH), IONE, DL(1,LH), IONE, FL(1,1), LDFL)
                ENDIF
            END IF
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                ...
                ! IF ( LB .EQ. IONE ) THEN
                !     CALL DGER(M, N-LH, -SGN1*ONE, EL(1,L), IONE, B(L,LH+1), LDB, FL(1,LH+1), LDFL)
                !     CALL DGER(M, N-LH, -SGN2*ONE, FL(1,L), IONE, D(L,LH+1), LDD, FL(1,LH+1), LDFL)
                !
                ! ELSE
                !     CALL DGER(M, N-LH, -SGN1*ONE, EL(1,L),  IONE, B(L,LH+1),  LDB, FL(1,LH+1), LDFL)
                !     CALL DGER(M, N-LH, -SGN1*ONE, EL(1,LH), IONE, B(LH,LH+1), LDB, FL(1,LH+1), LDFL)
                !
                !     CALL DGER(M, N-LH, -SGN2*ONE, FL(1,L),  IONE, D(L,LH+1),  LDD, FL(1,LH+1), LDFL)
                !     CALL DGER(M, N-LH, -SGN2*ONE, FL(1,LH), IONE, D(LH,LH+1), LDD, FL(1,LH+1), LDFL)
                ! ENDIF
            END IF
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( L .GT. 1 ) THEN
                ...
                ! IF ( LB .EQ. IONE ) THEN
                !     CALL DGER(M, LE, -SGN1*ONE, EL(1,L), IONE, B(1,L), IONE, FL(1,1), LDFL)
                !     CALL DGER(M, LE, -SGN2*ONE, FL(1,L), IONE, D(1,L), IONE, FL(1,1), LDFL)
                ! ELSE
                !     CALL DGER(M, LE, -SGN1*ONE, EL(1,L),  IONE, B(1,L),  IONE, FL(1,1), LDFL)
                !     CALL DGER(M, LE, -SGN1*ONE, EL(1,LH), IONE, B(1,LH), IONE, FL(1,1), LDFL)
                !     CALL DGER(M, LE, -SGN2*ONE, FL(1,L),  IONE, D(1,L),  IONE, FL(1,1), LDFL)
                !     CALL DGER(M, LE, -SGN2*ONE, FL(1,LH), IONE, D(1,LH), IONE, FL(1,1), LDFL)
                ! ENDIF
            END IF
            L = L - 1
        END DO
    ...
    ELSE IF ( BTRANSA .AND. BTRANSB) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                ...
                ! ELSE
                !     CALL DGER(M, N-LH, -SGN1*ONE, EL(1,L),  IONE, B(L,LH+1),  LDB, FL(1,LH+1), LDFL)
                !     CALL DGER(M, N-LH, -SGN1*ONE, FL(1,LH), IONE, B(LH,LH+1), LDB, FL(1,LH+1), LDFL)
                !
                !     CALL DGER(M, N-LH, -SGN2*ONE, FL(1,L),  IONE, D(L,LH+1),  LDD, FL(1,LH+1), LDFL)
                !     CALL DGER(M, N-LH, -SGN2*ONE, FL(1,LH), IONE, D(LH,LH+1), LDD, FL(1,LH+1), LDFL)
                ! ENDIF
