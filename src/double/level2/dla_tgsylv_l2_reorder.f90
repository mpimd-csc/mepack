...
!  ===========
!
!       SUBROUTINE DLA_TGSYLV_L2_REORDER ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, &
!                                       & SCALE, WORK, INFO)
...
SUBROUTINE DLA_TGSYLV_L2_REORDER ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGSYLV_L2_REORDER', -INFO)
        RETURN
    END IF
                ...
                IF ( K .GT. IONE ) THEN
                    ...
                    ELSE
                        ...
                        X(1:K-1,L) = X(1:K-1,L) - MAT(1,1) * A(1:K-1,K) - MAT(2,1) * A(1:K-1,KH) &
                            & - SGN* MAT(1,2) * C(1:K-1,K) - SGN * MAT(2,2) * C(1:K-1,KH)
                        X(1:K-1,LH) = X(1:K-1,LH) - MAT(3,1) * A(1:K-1,K) - MAT(4,1) * A(1:K-1,KH) &
                            & - SGN* MAT(3,2) * C(1:K-1,K) - SGN * MAT(4,2) * C(1:K-1,KH)
    ...
    ELSE IF ( .NOT. BTRANSA .AND. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( K .GT. 1 ) THEN
                    IF ( KB .EQ. IONE .AND. LB .EQ. IONE ) THEN
                        X(1:KE, L ) = X(1:KE, L) - (B(L,L) * X(K,L))* A(1:KE, K) - (SGN*D(L,L)*X(K,L)) * C(1:KE,K)
                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        X(1:KE,L) = X(1:KE,L) - (X(K,L) * B(L,L)  + X(K,LH) * B(L,LH))*A(1:KE,K) &
                            & - (SGN * (X(K,L) * D(L,L) + X(K,LH) * D(L,LH)))* C(1:KE,K)
                        X(1:KE,LH) = X(1:KE,LH) - (X(K,L) * B(LH,L)  + X(K,LH) * B(LH,LH))*A(1:KE,K) &
                            & - (SGN * (X(K,L) * D(LH,L) + X(K,LH) * D(LH,LH)))* C(1:KE,K)
                    ELSE IF ( KB .NE. IONE .AND. LB .EQ. IONE) THEN
                        X(1:KE, L) = X(1:KE,L) - A(1:KE,K) *(X(K,L) * B(L,L)) - A(1:KE,KH) *(X(KH,L) * B(L,L)) &
                            &  - C(1:KE,K) *(X(K,L) * D(L,L) * SGN) - C(1:KE,KH) *(X(KH,L) * D(L,L) * SGN)
                    ...
                    ELSE
                        ...
                        X(1:KE,L) = X(1:KE,L) - MAT(1,1) * A(1:KE,K) - MAT(2,1) * A(1:KE,KH) &
                            & - SGN* MAT(1,2) * C(1:KE,K) - SGN * MAT(2,2) * C(1:KE,KH)
                        X(1:KE,LH) = X(1:KE,LH) - MAT(3,1) * A(1:KE,K) - MAT(4,1) * A(1:KE,KH) &
                            & - SGN* MAT(3,2) * C(1:KE,K) - SGN * MAT(4,2) * C(1:KE,KH)
                    END IF
    ...
    ELSE IF ( BTRANSA .AND. .NOT. BTRANSB) THEN
        ...
        DO WHILE ( L .LE. N )
            ...
            DO WHILE ( K .LE. M )
                ...
                IF ( KH .LT. M ) THEN
                    IF (KB .EQ. IONE .AND. LB .EQ. IONE) THEN
                        X(KH+1:M, L) = X(KH+1:M, L) - A(K,KH+1:M) * (X(K,L) * B(L,L))  - SGN * C(K,KH+1:M) * (X(K,L) * D(L,L))

                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        X(KH+1:M, L) = X(KH+1:M, L) - A(K,KH+1:M) * (X(K,L) * B(L,L) + X(K,LH) * B(LH,L) ) &
                            & - SGN * C(K,KH+1:M) * (X(K,L) * D(L,L)+X(K,LH)*D(LH,L))
                        X(KH+1:M, LH) = X(KH+1:M, LH) - A(K,KH+1:M) * (X(K,L) * B(L,LH) + X(K,LH) * B(LH,LH) ) &
                            & - SGN * C(K,KH+1:M) * (X(K,L) * D(L,LH)+X(K,LH)*D(LH,LH))

                    ELSE IF ( KB .NE. IONE .AND. LB .EQ. IONE) THEN
                        X(KH+1:M,L) = X(KH+1:M,L) - A(K,KH+1:M)*(X(K,L)*B(L,L)) - A(KH,KH+1:M)*(X(KH,L)*B(L,L))
                        X(KH+1:M,L) = X(KH+1:M,L) - SGN*C(K,KH+1:M)*(X(K,L)*D(L,L)) - SGN*C(KH,KH+1:M)*(X(KH,L)*D(L,L))
                    ...
                    ELSE
                        ...
                        X(KH+1:M,L) = X(KH+1:M,L) - MAT(1,1)*A(K,KH+1:M) - MAT(2,1) * A(KH,KH+1:M) &
                            & - SGN * MAT(1,2)*C(K,KH+1:M) - SGN*MAT(2,2) * C(KH,KH+1:M)
                        X(KH+1:M,LH) = X(KH+1:M,LH) - MAT(3,1)*A(K,KH+1:M) - MAT(4,1) * A(KH,KH+1:M) &
                            & - SGN * MAT(3,2)*C(K,KH+1:M) - SGN*MAT(4,2) * C(KH,KH+1:M)
            ...
            IF ( LH .LT. N ) THEN
                ...
                ! CALL DGEMM("T", "N", M, LB, M, ONE, C, LDC, X(1,L), LDX, ZERO, WORK, M)
                ! CALL DGEMM("N", "N", M, N-LH, LB, -SGN*ONE, WORK, M, D(L,LH+1), LDD, ONE, X(1,LH+1), LDX)
                ...
                IF ( KH .LT. M  ) THEN
                    ...
                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        X(KH+1:M, L) = X(KH+1:M, L) - A(K,KH+1:M) * (X(K,L) * B(L,L) + X(K,LH) * B(L,LH) ) &
                            & - SGN * C(K,KH+1:M) * (X(K,L) * D(L,L)+X(K,LH)*D(L,LH))
                        X(KH+1:M, LH) = X(KH+1:M, LH) - A(K,KH+1:M) * (X(K,L) * B(LH,L) + X(K,LH) * B(LH,LH) ) &
                            & - SGN * C(K,KH+1:M) * (X(K,L) * D(LH,L)+X(K,LH)*D(LH,LH))
