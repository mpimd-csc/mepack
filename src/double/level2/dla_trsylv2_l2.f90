...
!> \verbatim
!>
!> DLA_TRSYLV2_L2 solves a generalized Sylvester equation of the following forms
!>
...
SUBROUTINE DLA_TRSYLV2_L2 ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TRSYLV2_L2', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE (K .GT. 0)
            ...
            DO WHILE ( L .LE. N )
                ...
                IF ( LH .LT. N ) THEN
                    ...
                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE) THEN
                        X(K,LH+1:N) = X(K,LH+1:N) - A(K,K)*X(K,L) * B(L,LH+1:N) - A(K,K)* X(K,LH) * B(LH,LH+1:N)
                    ...
                    ELSE
                        MAT(1,1) = A(K,K) * X(K,L) + A(K,KH) * X(KH,L)
                        MAT(2,1) = A(KH,K) * X(K,L) + A(KH,KH) * X(KH,L)
                        MAT(3,1) = A(K,K) * X(K,LH) + A(K,KH) * X(KH,LH)
                        MAT(4,1) = A(KH,K) * X(K,LH) + A(KH,KH) * X(KH,LH)
                        X(K,LH+1:N) = X(K,LH+1:N) - MAT(1,1) * B(L,LH+1:N) - MAT(3,1) * B(LH,LH+1:N)
                        X(KH,LH+1:N) = X(KH,LH+1:N) - MAT(2,1) * B(L,LH+1:N) - MAT(4,1) * B(LH,LH+1:N)
                    END IF
                END IF
            ...
            IF ( K .GT. 1 ) THEN
                ...
                IF ( KB .EQ. 1 ) THEN
                    WORK(1:N) = X(K,1:N)
                    CALL DTRMV('Upper', 'Trans', 'NoUnit', N, B, LDB, WORK, IONE)
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE. ZERO) WORK(IT) = WORK(IT)+B(IT+1,IT)*X(K,IT+1)
                    END DO

                    CALL DGER(KE, N, -ONE, A(1,K) , IONE, WORK, IONE, X(1,1), LDX)
                ELSE
                    WORK(1:N) = X(K,1:N)
                    WORK(N+1:2*N) = X(KH,1:N)
                    CALL DTRMV('Upper', 'Trans', 'NoUnit', N, B, LDB, WORK, IONE)
                    CALL DTRMV('Upper', 'Trans', 'NoUnit', N, B, LDB, WORK(N+1), IONE)
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE. ZERO) THEN
                            WORK(IT)     = WORK(IT)+B(IT+1,IT)*X(K,IT+1)
                            WORK(IT+N)   = WORK(IT+N)+B(IT+1,IT)*X(KH,IT+1)
                        END IF
                    END DO

                    CALL DGER(KE, N, -ONE, A(1,K) , IONE, WORK, IONE, X(1,1), LDX)
                    CALL DGER(KE, N, -ONE, A(1,KH) , IONE, WORK(N+1), IONE, X(1,1), LDX)
                ENDIF
            END IF
        ...
        DO WHILE (K .GT. 0)
            ...
            DO WHILE ( L .GT. 0 )
                ...
                IF ( L .GT. 0 ) THEN
                    ...
                    ELSE IF ( KB .EQ. IONE .AND. LB .NE. IONE ) THEN
                        X(K,1:LE) =  X(K,1:LE) -A(K,K)*X(K,L)*B(1:LE, L) - A(K,K)*X(K,LH)*B(1:LE, LH)
                    ...
                    ELSE
                        ...
                        MAT(3,1) = A(K,K) * X(K,LH) + A(K,KH) * X(KH,LH)
                        MAT(4,1) = A(KH,K) * X(K,LH) + A(KH,KH) * X(KH,LH)
                        X(K, 1:LE) = X(K,1:LE) - MAT(1,1) * B(1:LE, L) - MAT(3,1) * B(1:LE, LH)
                        X(KH, 1:LE) = X(KH,1:LE) - MAT(2,1) *B(1:LE, L) - MAT(4,1) * B(1:LE, LH)
                    END IF
                END IF
            ...
            IF ( K .GT. 1 ) THEN
                IF ( KB .EQ. IONE ) THEN
                    WORK(1:N) = X(K,1:N)
                    CALL DTRMV('Upper', 'NoTrans', 'NoUnit', N, B, LDB, WORK, IONE)
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE.ZERO) THEN
                            WORK(IT+1) = WORK(IT+1) + B(IT+1,IT) * X(K,IT)
                        END IF
                    END DO
                    CALL DGER(KE, N, -ONE, A(1,K), IONE, WORK, IONE, X(1,1),LDX)
                ELSE
                    ...
                    CALL DTRMV('Upper', 'NoTrans', 'NoUnit', N, B, LDB, WORK, IONE)
                    CALL DTRMV('Upper', 'NoTrans', 'NoUnit', N, B, LDB, WORK(N+1), IONE)
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE.ZERO) THEN
                            L = 2*(IT+1)
                            WORK(IT+1)     = WORK(IT+1) + B(IT+1,IT) * X(K,IT)
                            WORK(IT+1+N)   = WORK(IT+1+N)   + B(IT+1,IT) * X(KH,IT)
                        END IF
                    END DO
        ...
        DO WHILE ( K .LE. M )
            ...
            DO WHILE ( L .LE. N )
                ...
                IF ( LH .LT. N ) THEN
                    ...
                    ELSE
                        MAT(1,1) = A(K,K) * X(K,L) + A(KH,K) * X(KH,L)
                        MAT(2,1) = A(K,KH) * X(K,L) + A(KH,KH) * X(KH,L)
                        MAT(3,1) = A(K,K) * X(K,LH) + A(KH,K) * X(KH,LH)
                        MAT(4,1) = A(K,KH) * X(K,LH) + A(KH,KH) * X(KH,LH)
            ...
            IF ( KH .LT. M ) THEN
                IF (KB .EQ. IONE ) THEN
                    ...
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE. ZERO) WORK(IT) = WORK(IT)+B(IT+1,IT)*X(K,IT+1)
                    END DO
                    CALL DGER(M-KH, N, -ONE, A(K,KH+1), LDA, WORK, IONE, X(KH+1, 1), LDX)
                ELSE
                    WORK(1:N) = X(K,1:N)
                    WORK(N+1:2*N) = X(KH,1:N)
                    ...
                    CALL DGER(M-KH, N, -ONE, A(K,KH+1), LDA, WORK, IONE, X(KH+1, 1), LDX)
                    CALL DGER(M-KH, N, -ONE, A(KH,KH+1), LDA, WORK(N+1), IONE, X(KH+1, 1), LDX)
            ...
            IF ( KH .LT. M ) THEN
                ...
                ELSE
                    ...
                    DO IT = 1,N-1
                        IF (B(IT+1,IT).NE.ZERO) THEN
                            WORK(IT+1) = WORK(IT+1) + B(IT+1,IT) * X(K,IT)
                            WORK(IT+N+1)   = WORK(IT+N+1)   + B(IT+1,IT) * X(KH,IT)
                        END IF
                    END DO
                    CALL DGER(M-KH, N, -ONE, A(K,KH+1), LDA, WORK, IONE, X(KH+1, 1), LDX)
                    CALL DGER(M-KH, N, -ONE, A(KH,KH+1), LDA, WORK(N+1), IONE, X(KH+1, 1), LDX)
