...
!  Definition:
!  ===========
!       FUNCTION DLA_RESIDUAL_SYLV2(TRANSA, TRANSB, SGN, M, N,  A, LDA, B, LDB, X, LDX, Y, LDY, SCALE)
...
FUNCTION DLA_RESIDUAL_SYLV2(TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, Y, LDY, SCALE)
    ...
    INTEGER :: M, N, LDA, LDB, LDX, LDY
    DOUBLE PRECISION :: A(LDA, *), B(LDB, *), X(LDX, *), Y(LDY, *)
    DOUBLE PRECISION :: SCALE, SGN, DLA_RESIDUAL_SYLV2
    ...
    IF (INFO .NE. 0 ) THEN
        DLA_RESIDUAL_SYLV2 = DBLE(INFO)
        CALL XERROR_HANDLER("DLA_RESIDUAL_SYLV2", -INFO)
        RETURN
    END IF

    IF ( M .EQ. 0 .OR. N .EQ. 0) THEN
        DLA_RESIDUAL_SYLV2 = 0.0D0
        RETURN
    END IF

    ALLOCATE(WORK(M,N), STAT = ALLOC_STATUS)
    IF ( ALLOC_STATUS .NE. 0) THEN
        DLA_RESIDUAL_SYLV2 = DBLE(-1000)
        RETURN
    END IF
    ...
    CALL DGEMM(TRANSA, "N", M, N, M, DONE, A, LDA, X, LDX, DZERO, WORK2(1,1), M)
    CALL DGEMM("N", TRANSB, M, N, N, MONE , WORK2(1,1), M, B, LDB, SCALE, WORK(1,1), M)
    WORK(1:M,1:N) = WORK(1:M,1:N) - SGN*X(1:M, 1:N)

    NRMR = DLANGE("F", M, N, WORK(1,1), M, DUMMY)

    DLA_RESIDUAL_SYLV2 = NRMR / ( (NRMA  * NRMB + 1 ) * NRMX + SCALE * NRMY)
...
END FUNCTION DLA_RESIDUAL_SYLV2
