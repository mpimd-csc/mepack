...
FUNCTION DLA_RESIDUAL_GSTEIN(TRANS, M, A, LDA, B, LDB, X, LDX, Y, LDY, SCALE)
    ...
    INTEGER :: M, LDA, LDB, LDX, LDY
    DOUBLE PRECISION :: A(LDA, *), B(LDB, *), X(LDX, *), Y(LDY, *)
    DOUBLE PRECISION :: SCALE, DLA_RESIDUAL_GSTEIN
    ...
    END IF
    IF (INFO .NE. 0 ) THEN
        DLA_RESIDUAL_GSTEIN = DBLE(INFO)
        CALL XERROR_HANDLER("DLA_RESIDUAL_GSTEIN", -INFO)
        RETURN
    END IF

    IF ( M .EQ. 0 ) THEN
        DLA_RESIDUAL_GSTEIN = 0.0D0
        RETURN
    END IF

    ALLOCATE(WORK(M,M), STAT = ALLOC_STATUS)
    IF ( ALLOC_STATUS .NE. 0) THEN
        DLA_RESIDUAL_GSTEIN = DBLE(-1000)
        RETURN
    END IF
    ...
    CALL DGEMM("N", TRANS2, M, M, M, MONE, WORK2, M, A, LDA, SCALE, WORK, M)

    CALL DGEMM(TRANS, "N", M, M, M, DONE, B, LDB, X, LDX, DZERO, WORK2, M)
    CALL DGEMM("N", TRANS2, M, M, M, DONE, WORK2, M, B, LDB, DONE, WORK, M)

    NRMR = DLANGE("F", M, M, WORK(1,1), M, DUMMY)

    DLA_RESIDUAL_GSTEIN = NRMR / ( (NRMA**2 + NRMB**2) + NRMX + SCALE * NRMY)
...
END FUNCTION DLA_RESIDUAL_GSTEIN
