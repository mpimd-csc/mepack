...
!> \brief Compute the relative residual for the dual coupled Sylvester equation.
!
!  Definition:
!  ===========
!       FUNCTION DLA_RESIDUAL CSYLV_DUAL(TRANSA, TRANSB, SGN1, SGN2, M, N,  A, LDA, B, LDB, C, LDC, D, LDD,
...
!> \ingroup residual
!
FUNCTION DLA_RESIDUAL_CSYLV_DUAL(TRANSA, TRANSB, SGN1, SGN2, M, N, A, LDA, B, LDB, C, LDC, D, LDD,  &
        & R, LDR, L, LDL, E, LDE, F, LDF, SCALE)
    IMPLICIT NONE
    CHARACTER :: TRANSA, TRANSB
    INTEGER :: M, N, LDA, LDB, LDC, LDD, LDE, LDF, LDR, LDL
    DOUBLE PRECISION :: A(LDA, *), B(LDB, *), C(LDC,*), D(LDD, *), E(LDE, *), F(LDF, *), R(LDR, *), L(LDL, *)
    DOUBLE PRECISION :: SCALE, SGN1, SGN2, DLA_RESIDUAL_CSYLV_DUAL

    DOUBLE PRECISION :: DLANGE
    EXTERNAL :: DGEMM, DLANGE, LSAME, DLACPY, XERROR_HANDLER
    LOGICAL :: LSAME
    CHARACTER(LEN=1) :: TRANSAI, TRANSBI
    ...
    IF (INFO .NE. 0 ) THEN
        DLA_RESIDUAL_CSYLV_DUAL = DBLE(INFO)
        CALL XERROR_HANDLER("DLA_RESIDUAL_CSYLV", -INFO)
        RETURN
    END IF

    IF ( M .EQ. 0 .OR. N .EQ. 0) THEN
        DLA_RESIDUAL_CSYLV_DUAL = 0.0D0
        RETURN
    END IF

    ALLOCATE(WORK(M,N), STAT = ALLOC_STATUS)
    IF ( ALLOC_STATUS .NE. 0) THEN
        DLA_RESIDUAL_CSYLV_DUAL = DBLE(-1000)
        RETURN
    END IF
    ...
    CALL DGEMM(TRANSAI, "N", M, N, M, MONE, A, LDA, R, LDR, SCALE, WORK, M)
    CALL DGEMM(TRANSAI, "N", M, N, M, MONE, C, LDC, L, LDL, DONE, WORK, M)
    NRMR1 = DLANGE("F", M, N, WORK(1,1), M, DUMMY)

    CALL DLACPY("A", M, N, F, LDF, WORK(1,1), M)
    CALL DGEMM("N", TRANSBI, M, N, N, MONE*SGN1, R, LDR, B, LDB, SCALE, WORK, M)
    CALL DGEMM("N", TRANSBI, M, N, N, MONE*SGN2 , L, LDL, D, LDD, DONE, WORK, M)
    NRMR2 = DLANGE("F", M, N, WORK(1,1), M, DUMMY)


    DLA_RESIDUAL_CSYLV_DUAL = MAX(NRMR1/ ( (NRMA  * NRMR + NRMC * NRML ) + SCALE * NRME), &
                        &  NRMR2 / ( (NRMB  * NRMR + NRMD * NRML ) + SCALE * NRMF))


    DEALLOCATE(WORK)
    RETURN

END FUNCTION DLA_RESIDUAL_CSYLV_DUAL
