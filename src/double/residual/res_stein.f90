...
!  Definition:
!  ===========
!       FUNCTION DLA_RESIDUAL_STEIN(TRANS, M, A, LDA, X, LDX, Y, LDY, SCALE)
!           CHARACTER TRANS(!)
...
FUNCTION DLA_RESIDUAL_STEIN(TRANS, M, A, LDA, X, LDX, Y, LDY, SCALE)
    ...
    INTEGER :: M, LDA, LDX, LDY
    DOUBLE PRECISION :: A(LDA, *), X(LDX, *), Y(LDY, *)
    DOUBLE PRECISION :: SCALE, DLA_RESIDUAL_STEIN
    ...
    END IF
    IF (INFO .NE. 0 ) THEN
        DLA_RESIDUAL_STEIN = DBLE(INFO)
        CALL XERROR_HANDLER("DLA_RESIDUAL_STEIN", -INFO)
        RETURN
    END IF

    IF ( M .EQ. 0 ) THEN
        DLA_RESIDUAL_STEIN = 0.0D0
        RETURN
    END IF

    ALLOCATE(WORK(M,M), STAT = ALLOC_STATUS)
    IF ( ALLOC_STATUS .NE. 0) THEN
        DLA_RESIDUAL_STEIN = DBLE(-1000)
        RETURN
    END IF
    ...
    WORK(1:M,1:M) = WORK(1:M, 1:M) + X(1:M,1:M)

    NRMR = DLANGE("F", M, M, WORK(1,1), M, DUMMY)

    DLA_RESIDUAL_STEIN = NRMR / ( (NRMA **2 + DONE) * NRMX + SCALE * NRMY)

    DEALLOCATE(WORK)
    DEALLOCATE(WORK2)
    RETURN

END FUNCTION DLA_RESIDUAL_STEIN
