...
!  Definition:
!  ===========
!       SUBROUTINE DLA_TGCSYLV_DUAL_DAG ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
SUBROUTINE DLA_TGCSYLV_DUAL_DAG ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    EXTERNAL IDLA_TGCSYLV_DUAL_BLOCK
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGCSYLV_DUAL_DAG', -INFO)
        RETURN
    END IF
            ...
            DO WHILE ( KH .LE. M )
                ...
                IF ( KH .LT. M  ) THEN
                    ...
                    DO WHILE ( JH .LE. M )
                        ...
                        CALL DGEMM('T', 'N', JB, LB, KB, -ONE, C(K, J), LDC, F(K,L), LDF, ONE, E(J,L),LDE)
                        !$omp end task
                ...
                IF ( L .GT. 1) THEN
                    ...
                    DO WHILE (J .GT. 0)
                        ...
                        CALL DGEMM('N', 'T', KB, JB, LB, -ONE*SGN1, E(K,L), LDE, B(J, L), LDB, ONE, F(K,J), LDF)
                        !$omp end task
                        !$omp task firstprivate(K,L,J,KB,LB,JB) default(shared) &
            ...
            DO WHILE ( KH .LE. M )
                ...
                IF ( LH .LT. N ) THEN
                    ...
                    DO WHILE ( JH .LE. N )
                        ...
                        !$omp task firstprivate(K,L,J,KB,LB,JB) default(shared) &
                        !$omp& depend(in:E(K,L)) depend(inout:F(K,J))
                        CALL DGEMM("N", "N", KB, JB, LB, -ONE*SGN1,  E(K,L), LDE, B(L,J), LDB, ONE, F(K,J), LDF)
                        !$omp end task
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( K .GT. 1 ) THEN
                    ...
                    DO WHILE (J .GT. 0)
                        ...
                        !$omp task firstprivate(K,L,J,KB,LB,JB) default(shared) &
                        !$omp& depend(in:F(K,L)) depend(inout:E(J,L))
                        CALL DGEMM('N', 'N', JB, LB, KB, -ONE, C(J,K), LDC, F(K,L), LDF, ONE, E(J,L), LDE)
                        !$omp end task
...
SUBROUTINE IDLA_TGCSYLV_DUAL_BLOCK(TRANSA, TRANSB, SGN1, SGN2, M, N ,K, L, KB, LB, A, LDA, B, LDB, C, LDC, D, LDD, &
        & E, LDE, F, LDF, SCAL, INFO1)
    USE MEPACK_OPTIONS_ISOLVER
    USE MEPACK_OPTIONS_BLOCKSIZE_DOUBLE
    IMPLICIT NONE
    ...
    CASE(1)
        IF ( MAXNB .LE. 32 ) THEN
            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_32(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                & C(K,K), LDC, D(L,L), LDD,  E(K,L), LDE, F(K,L), LDF, SCAL, WORK(1), INFO1)
        ELSE IF ( MAXNB .LE. 64) THEN
            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_64(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                & C(K,K), LDC, D(L,L), LDD,  E(K,L), LDE, F(K,L), LDF, SCAL, WORK(1), INFO1)
        ELSE IF ( MAXNB .LE. 96) THEN
            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_96(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                & C(K,K), LDC, D(L,L), LDD,  E(K,L), LDE, F(K,L), LDF, SCAL, WORK(1), INFO1)
        ELSE IF ( MAXNB .LE. 128) THEN
            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY_128(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                & C(K,K), LDC, D(L,L), LDD,  E(K,L), LDE, F(K,L), LDF, SCAL, WORK(1), INFO1)
        ELSE
            CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                & C(K,K), LDC, D(L,L), LDD,  E(K,L), LDE, F(K,L), LDF, SCAL, WORK(1), INFO1)
        END IF
    CASE(2)
        IF ( M .GT. 128 .OR. N .GT. 128) THEN
            CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                & C(K,K), LDC, D(L,L), LDD,  E(K,L), LDE, F(K,L), LDF, SCAL, WORK(1), INFO1)
        ELSE
            CALL DLA_TGCSYLV_DUAL_L2_LOCAL_COPY(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                & C(K,K), LDC, D(L,L), LDD,  E(K,L), LDE, F(K,L), LDF, SCAL, WORK(1), INFO1)
        END IF
    CASE(3)
        CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
    ...
    CASE(5)
        CALL DLA_TGCSYLV_DUAL_L2(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
            & C(K,K), LDC, D(L,L), LDD,  E(K,L), LDE, F(K,L), LDF, SCAL, WORK(1), INFO1)
    CASE(6)
        CALL DLA_TGCSYLV_DUAL_RECURSIVE(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
            & C(K,K), LDC, D(L,L), LDD,  E(K,L), LDE, F(K,L), LDF, SCAL, WORK(1), INFO1)
    END SELECT
    IF ( INFO1 .LT. 0 ) THEN
        INFO1 = 100
