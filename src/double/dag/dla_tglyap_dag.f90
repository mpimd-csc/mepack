...
!> \verbatim
!>
!> DLA_TGLYAP_DAG solves a generalized Lyapunov  equation of the following forms
!>
...
SUBROUTINE DLA_TGLYAP_DAG ( TRANS, M, A, LDA, B, LDB,  X, LDX, SCALE, WORK, INFO)
    ...
    EXTERNAL IDLA_TGLYAP_SOLVE_BLOCK_N
    EXTERNAL IDLA_TGLYAP_SOLVE_BLOCK_T
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGLYAP_DAG', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                KE = K - 1
                    ...
                    CALL IDLA_TGLYAP_SOLVE_BLOCK_N(TRANSA, TRANSB, M, MB,K, KB, KH, L, LB, LH, A, LDA, B, LDB, X, LDX, WORK, &
                        & SCAL, INFO)
    ...
    ELSE
        ...
        DO WHILE ( LH .LE. M )
            ...
            DO WHILE ( KH .LE. M )
                ...
                !$omp task firstprivate(TRANSA,TRANSB,M,K,KB,KH,L,LB,LH,INFO1,SCAL) depend(inout:X(K,L)) default(shared)
                CALL IDLA_TGLYAP_SOLVE_BLOCK_T(TRANSA, TRANSB, M, MB, K, KB, KH, L, LB, LH, A, LDA, B, LDB, X, LDX, &
                    & WORK, SCAL, INFO)
                !$omp end task
...
SUBROUTINE IDLA_TGLYAP_SOLVE_BLOCK_N(TRANSA, TRANSB, M, MB, K, KB, KH, L, LB, LH, A, LDA, B, LDB, X, LDX, WORK, SCAL, INFO)
    USE MEPACK_OPTIONS_ISOLVER
    USE MEPACK_OPTIONS_BLOCKSIZE_DOUBLE
    USE OMP_LIB
    IMPLICIT NONE
    ...
    IF ( L.EQ.K) THEN
        CALL DLA_TGLYAP_L2 (TRANSA, KB, A(K,K), LDA, B(K,K), LDB, X(K,K), LDX, SCAL, WORK(WORKOFFSET), INFO1)
    ELSE
        SELECT CASE(ISOLVER)
        CASE(1)
            IF ( MAXNB .LE. 32 ) THEN
...
SUBROUTINE IDLA_TGLYAP_SOLVE_BLOCK_T(TRANSA, TRANSB, M, MB, K, KB, KH, L, LB, LH, A, LDA, B, LDB, X, LDX, WORK, SCAL, INFO)
    USE MEPACK_OPTIONS_ISOLVER
    USE MEPACK_OPTIONS_BLOCKSIZE_DOUBLE
    USE OMP_LIB
    IMPLICIT NONE
