...
!> \verbatim
!>
!> DLA_TRSYLV2_DAG solves a discrete time Sylvester equation of the following forms
!>
...
!> like the DLA_TRSYLV2_L3 routine.
!>
!
!  Arguments:
!  ==========
...
SUBROUTINE DLA_TRSYLV2_DAG ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    DOUBLE PRECISION SGN, SCALE
    INTEGER M, N, LDA, LDB, LDX, INFO
    DOUBLE PRECISION A(LDA, M), B(LDB, N) , X(LDX, N)
    DOUBLE PRECISION WORK(*)
    ...
    EXTERNAL IDLA_TRSYLV2_SOLVE_BLOCK
    EXTERNAL LSAME
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TRSYLV2_DAG', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( KH .EQ. M .AND. L .EQ. IONE ) THEN
                    ...
                    CALL IDLA_TRSYLV2_SOLVE_BLOCK(TRANSA, TRANSB, M,N,BLO,K,KH,KB,L,LH,LB,A,LDA,B,LDB,&
                        & X,LDX,SGN,SCAL, WORK, INFO1 )
                    !$omp end task
                ELSE IF ( KH .EQ. M .AND. L .GT. IONE) THEN
                    !$omp task firstprivate(K,KH,KB,L,LH,LB,SCAL,INFO1) &
                    !$omp& depend(inout: X(K,L)) depend(in: X(K,LOLD)) default(shared)
                    CALL IDLA_TRSYLV2_SOLVE_BLOCK(TRANSA, TRANSB, M,N,BLO,K,KH,KB,L,LH,LB,A,LDA,B,LDB,&
                        & X,LDX,SGN,SCAL, WORK, INFO1 )
                    !$omp end task
                ELSE IF ( KH .LT. M .AND. L .EQ. IONE) THEN
                    !$omp task firstprivate(K,KH,KB,L,LH,LB,SCAL,INFO1) &
                    !$omp& depend(inout: X(K,L)) depend(in: X(KOLD,L))  default(shared)
                    CALL IDLA_TRSYLV2_SOLVE_BLOCK(TRANSA, TRANSB, M,N,BLO,K,KH,KB,L,LH,LB,A,LDA,B,LDB,&
                        & X,LDX,SGN,SCAL, WORK, INFO1 )
                    !$omp end task
                ELSE
                    !$omp task firstprivate(K,KH,KB,L,LH,LB,SCAL,INFO1) &
                    !$omp& depend(in: X(KOLD,L), X(K,LOLD)) depend(inout: X(K,L))  default(shared)
                    CALL IDLA_TRSYLV2_SOLVE_BLOCK(TRANSA, TRANSB, M,N,BLO,K,KH,KB,L,LH,LB,A,LDA,B,LDB,&
                        & X,LDX,SGN,SCAL, WORK, INFO1 )
                    !$omp end task
                END IF
...
!> \internal
SUBROUTINE IDLA_TRSYLV2_SOLVE_BLOCK(TRANSA, TRANSB, M,N,BLO,K,KH,KB,L,LH,LB,A,LDA,B,LDB,X,LDX,SGN,SCAL, &
        & WORK, INFO1 )
    USE MEPACK_OPTIONS_ISOLVER
    USE MEPACK_OPTIONS_BLOCKSIZE_DOUBLE
    USE OMP_LIB
    ...
    DOUBLE PRECISION A(LDA,M), B(LDB,N), X(LDX,N), SGN, SCAL, WORK(*)
