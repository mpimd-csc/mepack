...
!  ===========
!
!       SUBROUTINE DLA_TGSTEIN_RECURSIVE ( TRANS, M,  A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
!           CHARACTER TRANS(1)
...
!> \verbatim
!>
!> DLA_TGSTEIN_RECURSIVE solves a generalized Lyapunov  equation of the following forms
!>
...
RECURSIVE SUBROUTINE DLA_TGSTEIN_RECURSIVE ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGSTEIN_RECURSIVE', -INFO)
        RETURN
    END IF
    ...
    IF ( M .LE. BOUND) THEN
                ...
                ELSE
                    ...
                    DO
                        ...
                        IF ( I .EQ. 0 ) THEN
                            ...
                            I = 1
                        END IF
                        CALL DGETC2X(4, MAT, 4, IPIV, JPIV, INFO1)
                        INFO = INFO1
     ...
     IF ( BTRANS ) THEN
         ! (N,T)
         CALL DLA_TGSTEIN_RECURSIVE ( 'N' , M2, A(M1+1,M1+1), LDA, B(M1+1,M1+1), LDB, X(M1+1,M1+1), LDX, SCALE, WORK, INFO1)
         ...
         CALL DLA_TGSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M1, M2,  A(1,1), LDA, A(M1+1,M1+1), LDA, &
        ...
        IF (SCAL .NE. ONE) THEN
         ...
         CALL DSYR2K("U", "N", M1, M2, -ONE, WORK, M1, A(1,M1+1), LDA, SCALE, X(1,1), LDX)
         ...
         CALL DSYMM("Right", "Upper", M1, M2, ONE, X(M1+1,M1+1), LDX, A(1,M1+1), LDA, ZERO, WORK, M1)
         CALL DGEMM("N", "T", M1, M1, M2, -ONE, WORK, M1, A(1,M1+1), LDA, ONE, X(1,1), LDX)
         CALL DSYMM("Right", "Upper", M1, M2, ONE, X(M1+1,M1+1), LDX, B(1,M1+1), LDB, ZERO, WORK, M1)
         CALL DGEMM("N", "T", M1, M1, M2, ONE, WORK, M1, B(1,M1+1), LDB, ONE, X(1,1), LDX)

         CALL DLA_TGSTEIN_RECURSIVE ( 'N' , M1, A(1,1), LDA, B(1,1), LDB, X(1,1), LDX, SCAL, WORK, INFO1)
    ...
    ELSE
        ! (T, N)
        CALL DLA_TGSTEIN_RECURSIVE ( 'T' , M1, A(1,1), LDA, B(1,1), LDB, X(1,1), LDX, SCALE, WORK, INFO1)

        ! CALL DGEMM("N", "N", M1, M1, M1, ONE, X(1,1), LDX, A(1,1), LDA, ZERO, WORK, M1)
        CALL DSYMM("L", "L", M1, M1, ONE, X(1,1), LDX, A(1,1), LDA, ZERO, WORK, M1)
        CALL DGEMM("T", "N", M2, M1, M1, -ONE, A(1,M1+1), LDA, WORK, M1, ONE, X(M1+1, 1), LDX)

        ! CALL DGEMM("N", "N", M1, M1, M1, ONE, X(1,1), LDX, B(1,1), LDB, ZERO, WORK, M1)
        CALL DSYMM("L", "L", M1, M1, ONE, X(1,1), LDX, B(1,1), LDB, ZERO, WORK, M1)
        CALL DGEMM("T", "N", M2, M1, M1, ONE, B(1,M1+1), LDB, WORK, M1, ONE, X(M1+1, 1), LDX)

        CALL DLA_TGSYLV_RECURSIVE(TRANSA, TRANSB, SGN, M2, M1,  A(M1+1,M1+1), LDA, A(1,1), LDA, &
        ...
        CALL DSYR2K("L", "T", M2, M1, -ONE, A(1,M1+1), LDA, WORK, M1, SCALE, X(M1+1,M1+1), LDX)

        CALL DGEMM("N", "N", M1, M2, M2, ONE, X(1,M1+1), LDX, B(M1+1,M1+1), LDB, ZERO, WORK, M1)
        CALL DSYR2K("L", "T", M2, M1, ONE, B(1,M1+1), LDB, WORK, M1, ONE, X(M1+1,M1+1), LDX)

        CALL DSYMM("Left", "Lower", M1, M2, ONE, X(1,1), LDX, A(1,M1+1), LDA, ZERO, WORK, M1)
        CALL DGEMM("T", "N", M2, M2, M1, -ONE, A(1,M1+1), LDA, WORK, M1, ONE, X(M1+1,M1+1), LDX)

        CALL DSYMM("Left", "Lower", M1, M2, ONE, X(1,1), LDX, B(1,M1+1), LDB, ZERO, WORK, M1)
        CALL DGEMM("T", "N", M2, M2, M1,  ONE, B(1,M1+1), LDB, WORK, M1, ONE, X(M1+1,M1+1), LDX)

        CALL DLA_TGSTEIN_RECURSIVE ( 'T' , M2, A(M1+1,M1+1), LDA, B(M1+1,M1+1), LDB, X(M1+1,M1+1), LDX, SCAL, WORK, INFO1)
