...
!  ===========
!
!       SUBROUTINE DLA_TGLYAP_RECURSIVE ( TRANS, M,  A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
!           CHARACTER TRANS(1)
...
!> \verbatim
!>
!> DLA_TGLYAP_RECURSIVE solves a generalized Lyapunov  equation of the following forms
!>
...
!> \param[in] WORK
!> \verbatim
!>          WORK is DOUBLE PRECISION array, dimension M*M
...
RECURSIVE SUBROUTINE DLA_TGLYAP_RECURSIVE ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGLYAP_RECURSIVE', -INFO)
        RETURN
    END IF
            ...
            CASE(2)
                ...
                ELSE
                    ...
                    DO
                        ...
                        IF ( I .EQ. 0 ) THEN
                            CALL DLA_SMALL_SOLVE4( 4, MAT, RHS, INFO1, MACHINE_PRECISION, SMALL_NUMBER)
                            IF ( INFO1 .EQ. 0 ) EXIT
                            I = 1
                        ELSE
                            CALL DGETC2X(4, MAT, 4, IPIV, JPIV, INFO)
                            IF ( INFO .NE. 0 ) EXIT
    ...
    IF ( .NOT. BTRANS ) THEN
         ! (N, T)
         CALL DLA_TGLYAP_RECURSIVE('N', M2, A(M1+1,M1+1), LDA, B(M1+1,M1+1), LDB,  X(M1+1,M1+1), LDX, SCALE, WORK, INFO )

         ! CALL DGEMM("N", "T", M2, M2, M2,  ONE, X(M1+1,M1+1), LDX, B(M1+1,M1+1), LDB, ZERO, WORK, M2)
         CALL DLA_DTRMM_FIX_RIGHT("N", "T", M2, M2,  ONE, X(M1+1,M1+1), LDX, B(M1+1,M1+1), LDB, ZERO, WORK, M2)
         ...
         CALL DLA_DTRMM_FIX_RIGHT("N", "T", M2, M2,  ONE, X(M1+1,M1+1), LDX, A(M1+1,M1+1), LDA, ZERO, WORK, M2)
         CALL DGEMM("N","N",  M1, M2, M2, -ONE, B(1,M1+1), LDB, WORK, M2, ONE, X(1,M1+1), LDX)

         CALL DLA_TGSYLV_RECURSIVE('N', 'T', ONE, M1, M2, A(1,1), LDA, B(M1+1,M1+1),LDB, &
         ...
         ! CALL DGEMM("N", "N", M1, M2, M1, ONE, A(1,1), LDA, X(1,M1+1), LDX, ZERO, WORK, M1)
         CALL DLA_DTRMM_FIX("N", M1, M2, ONE, A(1,1), LDA, X(1,M1+1), LDX, ZERO, WORK, M1)
         CALL DGEMM("N", "N", M1, M2, M2, ONE, A(1,M1+1), LDA, X(M1+1,M1+1), LDX, ONE, WORK, M1)


         CALL DSYR2K("U", "N", M1, M2, -ONE, WORK, M1, B(1,M1+1), LDB, ONE, X(1,1), LDX)

         ! CALL DGEMM("N", "N", M1, M2, M1, ONE, B(1,1), LDB, X(1,M1+1), LDX, ZERO, WORK, M1)
         CALL DLA_DTRMM_FIX("N", M1, M2, ONE, B(1,1), LDB, X(1,M1+1), LDX, ZERO, WORK, M1)
         CALL DSYR2K("U", "N", M1, M2, -ONE, WORK, M1, A(1,M1+1), LDA, ONE, X(1,1), LDX)

         CALL DLA_TGLYAP_RECURSIVE('N', M1, A(1,1), LDA, B(1,1), LDB, X(1,1), LDX, SCAL, WORK, INFO )
         IF (SCAL .NE. ONE) THEN
             SCALE = SCALE * SCAL
     ...
     ELSE
         CALL DLA_TGLYAP_RECURSIVE('T', M1, A(1,1), LDA, B(1,1), LDB, X(1,1), LDX, SCALE, WORK, INFO )

         ! CALL DGEMM("N", "N", M1, M1, M1, ONE, X(1,1), LDX, B(1,1), LDB, ZERO, WORK, M1)
         CALL DLA_DTRMM_FIX_RIGHT("N", "N", M1,  M1, ONE, X(1,1), LDX, B(1,1), LDB, ZERO, WORK, M1)
         CALL DGEMM("T", "N", M2, M1, M1, -ONE, A(1,M1+1), LDA, WORK, M1, SCALE, X(M1+1,1), LDX)

         ! CALL DGEMM("N", "N", M1, M1, M1, ONE, X(1,1), LDX, A(1,1), LDA, ZERO, WORK, M1)
         CALL DLA_DTRMM_FIX_RIGHT("N", "N",  M1,  M1, ONE, X(1,1), LDX, A(1,1), LDA, ZERO, WORK, M1)
         CALL DGEMM("T", "N", M2, M1, M1, -ONE, B(1,M1+1), LDB, WORK, M1, ONE, X(M1+1,1), LDX)


         CALL DLA_TGSYLV_RECURSIVE('T', 'N', ONE, M2, M1, A(M1+1,M1+1), LDA, B(1,1),LDB, &
         ...
         CALL DGEMM("T", "N", M1, M2, M1, ONE, X(1,1), LDX, A(1,M1+1), LDA, ZERO, WORK, M1)
         CALL DGEMM("T", "N", M1, M2, M2, ONE, X(M1+1,1), LDX, A(M1+1,M1+1), LDA, ONE, WORK, M1)
         ! CALL DLA_DTRMM_FIX_RIGHT("T", "N",  M1, M2, ONE, X(M1+1,1), LDX, A(M1+1,M1+1), LDA, ONE, WORK, M1)

         CALL DSYR2K("L", "T", M2, M1, -ONE, WORK, M1, B(1,M1+1), LDB, SCALE, X(M1+1,M1+1), LDX)
         CALL DGEMM("T", "N", M1, M2, M2, ONE, X(M1+1, 1), LDX, B(M1+1,M1+1), LDB, ZERO, WORK, M1)
         ! CALL DLA_DTRMM_FIX_RIGHT("T", "N",  M1, M2, ONE, X(M1+1,1), LDX, B(M1+1,M1+1), LDB, ONE, WORK, M1)

         CALL DSYR2K("L", "T", M2, M1, -ONE, WORK, M1,  A(1,M1+1), LDA, ONE, X(M1+1,M1+1), LDX)

         CALL DLA_TGLYAP_RECURSIVE('T', M2, A(M1+1,M1+1), LDA, B(M1+1,M1+1), LDB, X(M1+1,M1+1), LDX, SCAL, WORK, INFO )
     ...
     END SUBROUTINE
     SUBROUTINE DLA_DTRMM_FIX_RIGHT(TRANSA, TRANSB, M, N , ALPHA, X, LDX, B, LDB, BETA, C, LDC)
    USE MEPACK_OPTIONS_MACHINE_DOUBLE
         IMPLICIT NONE
         CHARACTER(1) TRANSA,TRANSB
         INTEGER M, N, LDX, LDB, LDC
         DOUBLE PRECISION X(LDX, *) , B(LDB, *), C(LDC, *), ALPHA, BETA
                 ...
                 DO K = 1, N, NB
                     ...
                     IF ( K .GT. 1 ) THEN
                         IF ( B(K,K-1) .NE. ZERO )  THEN
                             CALL DGEMM("N", "T", M, KB, N-K+2 ,  ALPHA, X(1,K-1), LDX, B(K,K-1), LDB, BETA, C(1,K), LDC)
                         ELSE
                             CALL DGEMM("N", "T", M, KB, N-K+1 ,  ALPHA, X(1,K), LDX, B(K,K), LDB, BETA, C(1,K), LDC)
                         END IF
                 ...
                 DO K = 1, N, NB
                     ...
                     IF ( K .LT. N ) THEN
                         IF ( B(KH+1,KH) .NE. ZERO ) THEN
                             CALL DGEMM("N", "N", M, KB, KH+1, ALPHA , X, LDX, B(1,K), LDB, BETA, C(1,K), LDC)
                         ELSE
                             CALL DGEMM("N", "N", M, KB, KH, ALPHA, X, LDX, B(1,K), LDB, BETA, C(1,K), LDC)
