...
!> \verbatim
!>
!> DLA_TRSTEIN_L3 solves a standard Stein  equation of the following forms
!>
!>    A * X * A^T - X  = SCALE * Y                                              (2)
!>
!> or
...
SUBROUTINE DLA_TRSTEIN_L3 ( TRANS, M, A, LDA, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TRSTEIN_L3', -INFO)
        RETURN
    END IF
    ...
    IF ( M .LE. NB ) THEN
        ...
        IF ( ISOLVER .LT. 6 ) THEN
            CALL DLA_TRSTEIN_L2(TRANS,M,A,LDA,X,LDX, SCAL, WORK, INFO1)
        ELSE IF (ISOLVER .EQ. 6 ) THEN
            CALL DLA_TRSTEIN_RECURSIVE(TRANS,M,A,LDA,X,LDX, SCAL, WORK, INFO1)
        END IF
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( L.EQ. K ) THEN
                    ...
                    IF (ISOLVER .EQ. 6 ) THEN
                        CALL DLA_TRSTEIN_RECURSIVE(TRANS,KB,A(K,K),LDA,X(K,K),LDX, SCAL, WORK, INFO1)
                    ELSE
                        CALL DLA_TRSTEIN_L2(TRANS,KB,A(K,K),LDA,X(K,K),LDX, SCAL, WORK, INFO1)
                    END IF
                    IF ( INFO1 .LT. 0 ) THEN
                        INFO = 100
                    END IF
    ...
    ELSE
        ...
        DO WHILE ( LH .LE. M )
            ...
            ! Update January 2021
            IF ( LH .LT. M ) THEN
                CALL DGEMM("N", "N", LB, M-LH, M-LH, ONE, X(L,LH+1), LDX, A(LH+1,LH+1), LDA, ZERO, WORK, NB)
                CALL DSYR2K("L", "T", M-LH, LB, -ONE, A(L,LH+1), LDA, WORK, NB, ONE, X(LH+1,LH+1), LDX)
