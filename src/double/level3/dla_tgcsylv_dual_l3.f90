...
!> \brief Level-3 Kagstroem-Westin/Bartels-Stewart Algorithm for the dual generalized coupled Sylvester equation
!
!  Definition:
!  ===========
!       SUBROUTINE DLA_TGCSYLV_DUAL_L3 ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
SUBROUTINE DLA_TGCSYLV_DUAL_L3 ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGCSYLV_DUAL_L3', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE ( KH .LE. M )
                ...
                IF ( SCAL .NE. ONE ) THEN
                    CALL DLACPY("All", KB, LB, E(K,L), LDE, WORK, KB)
                    E(1:M, 1:N) = E(1:M, 1:N) * SCAL
                    CALL DLACPY("All", KB, LB, WORK, KB, E(K,L), LDE)

                    CALL DLACPY("All", KB, LB, F(K,L), LDF, WORK, KB)
                    F(1:M, 1:N) = F(1:M, 1:N) * SCAL
                    CALL DLACPY("All", KB, LB, WORK, KB, F(K,L), LDF)
