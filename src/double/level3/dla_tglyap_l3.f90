...
!> \verbatim
!>
!> DLA_TGLYAP_L3 solves a generalized Lyapunov  equation of the following forms
!>
...
SUBROUTINE DLA_TGLYAP_L3 ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGLYAP_L3', -INFO)
        RETURN
    END IF
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( L.EQ.K) THEN
                    ...
                    CALL DLA_TGLYAP_L2 (TRANSA, KB, A(K,K), LDA, B(K,K), LDB, X(K,K), LDX, SCAL, WORK, INFO1)
                ELSE
                    ! IF ( L.EQ.K) THEN
                    !     DO I = K,KH-1
                    !      CALL DCOPY(KH-I,X(I,I+1),LDX,X(I+1,I),IONE)
                    ...
                    CASE(4)
                        CALL DLA_TGSYLV_L2(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                            & B(K,K), LDB, A(L,L), LDA,  X(K,L), LDX, SCAL, WORK, INFO1)
                    CASE(5)
                        CALL DLA_TGSYLV_L2_UNOPT(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
                            & B(K,K), LDB, A(L,L), LDA,  X(K,L), LDX, SCAL, WORK, INFO1)
                    CASE(6)
    ...
    ELSE
        ...
        DO WHILE ( LH .LE. M )
            ...
            ! Update January 2021
            IF ( LH .LT. M ) THEN
                CALL DGEMM("T", "N", LB, M-LH, M-L+1, ONE, X(L,L), LDX, A(L,LH+1), LDA, ZERO, WORK, NB)
                CALL DSYR2K("L", "T", M-LH, LB, -ONE, WORK, NB, B(L,LH+1), LDB, ONE, X(LH+1,LH+1), LDX)

                CALL DGEMM("T", "N", LB, M-LH, M-LH, ONE, X(LH+1, L), LDX, B(LH+1,LH+1), LDB, ZERO, WORK, NB)
                CALL DSYR2K("L", "T", M-LH, LB, -ONE, WORK, NB,  A(L,LH+1), LDA, ONE, X(LH+1,LH+1), LDX)
