...
!> \verbatim
!>
!> DLA_TGSTEIN_L3 solves a generalized Lyapunov  equation of the following forms
!>
...
SUBROUTINE DLA_TGSTEIN_L3 ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGSTEIN_L3', -INFO)
        RETURN
    END IF
    ...
    IF ( M .LE. NB ) THEN
        ...
        IF ( ISOLVER .EQ. 6 ) THEN
            CALL DLA_TGSTEIN_RECURSIVE ( TRANS, KB, A(K,K), LDA, B(K,K), LDB, X(K,K), LDX, SCAL, WORK, INFO1)
        ELSE
            CALL DLA_TGSTEIN_L2 ( TRANS, KB, A(K,K), LDA, B(K,K), LDB, X(K,K), LDX, SCAL, WORK, INFO1)
        END IF
                ...
                ELSE
                    ...
                    CASE(4)
                        CALL DLA_TGSYLV_L2(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, A(L,L), LDA, &
                            & B(K,K), LDB, B(L,L), LDB,  X(K,L), LDX, SCAL, WORK, INFO1)
                    CASE(5)
                        CALL DLA_TGSYLV_L2_UNOPT(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, A(L,L), LDA, &
                            & B(K,K), LDB, B(L,L), LDB,  X(K,L), LDX, SCAL, WORK, INFO1)
                    CASE(6)
    ...
    ELSE
        ...
        DO WHILE ( LH .LE. M )
            ...
            IF ( LH .LT. M ) THEN
                ...
                CALL DGEMM("N", "N", LB, M-LH, M-LH, ONE, X(L,LH+1), LDX, B(LH+1,LH+1), LDB, ZERO, WORK, NB)
                CALL DSYR2K("L", "T", M-LH, LB, ONE, B(L,LH+1), LDB, WORK, NB, ONE, X(LH+1,LH+1), LDX)
