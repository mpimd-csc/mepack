...
!> \brief Level-3 Bartels-Stewart Algorithm for the generalized Lyapunov equation two stage
!
!  Definition:
!  ===========
!
...
!> \verbatim
!>
!> DLA_TGLYAP_L3_2S solves a generalized Lyapunov  equation of the following forms
!>
...
!> \endverbatim
!> \attention The algorithm is implemented using BLAS level 3 operations with two stage blocking (DAG inner solves)
!>
!
!  Arguments:
!  ==========
...
SUBROUTINE DLA_TGLYAP_L3_2S ( TRANS, M, A, LDA, B, LDB, X, LDX, SCALE, WORK, INFO)
    ...
    EXTERNAL DLA_TGLYAP_DAG
    EXTERNAL DLA_TGLYAP_L2
    ...
    MAXNB = NB
    BIGNB = TGLYAP_BLOCKSIZE_2STAGE(M)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGLYAP_L3_2S', -INFO)
        RETURN
    END IF
    ...
    IF ( ININFO .EQ. -1 ) THEN
        ...
        DAG_INFO = -1
        DAG_INFO2 = -1
        CALL DLA_TGLYAP_DAG ( TRANS, BIGNB, A, BIGNB, B, BIGNB,  X, BIGNB, SCALE, WORK, DAG_INFO)
        CALL DLA_TGSYLV_DAG( TRANSA, TRANSB, ONE, BIGNB, BIGNB,  A, BIGNB, B, BIGNB, &
                        & B, BIGNB, A, BIGNB,  X, BIGNB, SCAL, WORK, DAG_INFO2)
    ...
    IF ( M .LE. NB ) THEN
        ...
        CALL DLA_TGLYAP_L2 (TRANSA, KB, A, LDA, B, LDB, X, LDX, SCAL, WORK, INFO1)
    ...
    IF ( BTRANS ) THEN
        ...
        DO WHILE ( L .GT. 0 )
            ...
            DO WHILE (K .GT. 0)
                ...
                IF ( L.EQ.K) THEN
                    CALL DLA_TGLYAP_DAG (TRANSA, KB, A(K,K), LDA, B(K,K), LDB, X(K,K), LDX, SCAL, WORK, INFO1)
                    IF ( INFO1 .LT. 0 ) THEN
                        INFO = 100
                    END IF

                ELSE
                    CALL DLA_TGSYLV_DAG(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
            ...
            ! Update January 2021
            IF ( LE .GT. 0 ) THEN
                CALL DGEMM("N", "N", LE, LB, LH, ONE, A(1,1), LDA, X(1,L), LDX, ZERO, WORK, M)
                CALL DSYR2K("U", "N", LE, LB, -ONE, WORK, M, B(1,L), LDB, ONE, X(1,1), LDX)

                CALL DGEMM("N", "N", LE, LB, LE, ONE, B(1,1), LDB, X(1,L), LDX, ZERO, WORK, M)
                CALL DSYR2K("U", "N", LE, LB, -ONE, WORK, M, A(1,L), LDA, ONE, X(1,1), LDX)
            END IF
    ...
    ELSE
        ...
        DO WHILE ( LH .LE. M )
            ...
            ! Update January 2021
            IF ( LH .LT. M ) THEN
                CALL DGEMM("T", "N", LB, M-LH, M-L+1, ONE, X(L,L), LDX, A(L,LH+1), LDA, ZERO, WORK, LB)
                CALL DSYR2K("L", "T", M-LH, LB, -ONE, WORK, LB, B(L,LH+1), LDB, ONE, X(LH+1,LH+1), LDX)

                CALL DGEMM("T", "N", LB, M-LH, M-LH, ONE, X(LH+1, L), LDX, B(LH+1,LH+1), LDB, ZERO, WORK, LB)
                CALL DSYR2K("L", "T", M-LH, LB, -ONE, WORK, LB,  A(L,LH+1), LDA, ONE, X(LH+1,LH+1), LDX)
