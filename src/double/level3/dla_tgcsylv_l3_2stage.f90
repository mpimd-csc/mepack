...
!  Definition:
!  ===========
!       SUBROUTINE DLA_TGCSYLV_L3_2S ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
...
!> \endverbatim
!> \remark This algorithms uses 2 Stage blocking with DAG scheduled solves.
!>
!
!  Arguments:
!  ==========
...
SUBROUTINE DLA_TGCSYLV_L3_2S ( TRANSA, TRANSB, SGN1, SGN2,  M, N, A, LDA, B, LDB, C, LDC, D, LDD, &
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TGCSYLV_L3_2S', -INFO)
        RETURN
    END IF
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                CALL DLA_TGCSYLV_DAG(TRANSA, TRANSB, SGN1, SGN2, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
