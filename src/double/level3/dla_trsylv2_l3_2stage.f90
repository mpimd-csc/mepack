...
!> \verbatim
!>
!> DLA_TRSYLV2_L3_2S solves a generalized Sylvester equation of the following forms
!>
...
SUBROUTINE DLA_TRSYLV2_L3_2S ( TRANSA, TRANSB, SGN, M, N, A, LDA, B, LDB,  X, LDX, SCALE, WORK, INFO)
    ...
    IF ( INFO .LT. 0 ) THEN
        CALL XERROR_HANDLER('DLA_TRSYLV2_L3_2S', -INFO)
        RETURN
    END IF
    ...
    IF ( ININFO .EQ. -1 ) THEN
        ! MEMORY DAG
        DAG_INFO = -1
        CALL DLA_TRSYLV2_DAG(TRANSA, TRANSB, SGN, BIGNB, BIGNB,  A, BIGNB, B, BIGNB, &
    ...
    IF ( .NOT. BTRANSA .AND. .NOT. BTRANSB ) THEN
        ...
        DO WHILE ( LH .LE. N )
            ...
            DO WHILE (K .GT. 0)
                ...
                CALL DLA_TRSYLV2_DAG(TRANSA, TRANSB, SGN, KB, LB,  A(K,K), LDA, B(L,L), LDB, &
