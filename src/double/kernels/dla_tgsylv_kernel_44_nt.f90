...
!> \brief Solver for a 4x4 generalized Sylvester equation (TRANSA = N, TRANSB = T)
!
!  Definition:
!  ===========
!
!       SUBROUTINE DLA_TGSYLV_KERNEL_44NT (SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, INFO)
...
!> \verbatim
!>
!> DLA_TGSYLV_KERNEL_44NT solves a generalized Sylvester equation of the following form
!>
!>    A * X * B**T + SGN * C * X * D**T = SCALE * Y                              (1)
!>
            ...
            IF ( LE .GT. 0 ) THEN
                IF ( KB .EQ. 1 .AND. LB .EQ. 1 ) THEN
                    X(K,1:LE) = X(K,1:LE) - A(K,K) *X(K,L) * B(1:LE,L) &
                        & - SGN * C(K,K) *X(K,L) * D(1:LE,L)
                ELSE IF ( KB .EQ. 1 .AND. LB .EQ. 2 ) THEN
                    X(K,1:LE) = X(K,1:LE) - A(K,K) *(X(K,L) * B(1:LE,L) + X(K,LH) * B(1:LE,LH)) &
                        & - SGN * C(K,K) *(X(K,L) * D(1:LE,L) + X(K,LH) * D(1:LE, LH))

                ELSE IF ( KB .EQ. 2 .AND. LB .EQ. 1 ) THEN
                    X(K,1:LE) = X(K,1:LE) - ( A(K,K)  * X(K,L) + A(K,KH) * X(KH,L) ) * B(1:LE,L)
                    X(KH,1:LE) = X(KH,1:LE) - ( A(KH,K)  * X(K,L) + A(KH,KH) * X(KH,L) ) * B(1:LE,L)

                    X(K,1:LE)  = X(K,1:LE) - SGN * ( C(K,K)   * X(K,L) + C(K,KH) * X(KH,L) ) * D(1:LE,L)
                    X(KH,1:LE) = X(KH,1:LE) - SGN * ( C(KH,K)  * X(K,L) + C(KH,KH) * X(KH,L) ) * D(1:LE,L)
                ...
                ELSE
                    ...
                    WORK(1,1) = C(K,K) * X(K,L) + C(K,KH) * X(KH,L)
                    WORK(1,2) = C(K,K) * X(K,LH) + C(K,KH) * X(KH,LH)
                    WORK(2,1) = C(KH,K) * X(K,L) + C(KH,KH) * X(KH,L)
                    WORK(2,2) = C(KH,K) * X(K,LH) + C(KH,KH) * X(KH,LH)

                    X(K,  1:LE) = X(K, 1:LE ) - SGN * (WORK(1,1) *  D(1:LE,L) + WORK(1,2) * D(1:LE,LH))
                    X(KH, 1:LE) = X(KH, 1:LE) - SGN * (WORK(2,1) *  D(1:LE,L) + WORK(2,2) * D(1:LE,LH))
        ...
        IF ( K .GT. 1 ) THEN
            IF ( KB .EQ. 1 ) THEN
                ...
                CASE(3)
                    ...
                    WORK(1,2) = X(K,1) * D(1,1) + X(K,2) * D(1,2) + X(K,3) * D(1,3)
                    WORK(2,2) = X(K,1) * D(2,1) + X(K,2) * D(2,2) + X(K,3) * D(2,3)
                    WORK(3,2) =                   X(K,2) * D(3,2) + X(K,3) * D(3,3)
                ...
                CASE(4)
                    ...
                    WORK(1,2) = X(K,1) * D(1,1) + X(K,2) * D(1,2) + X(K,3) * D(1,3) + X(K,4) * D(1,4)
                    WORK(2,2) = X(K,1) * D(2,1) + X(K,2) * D(2,2) + X(K,3) * D(2,3) + X(K,4) * D(2,4)
                    WORK(3,2) =                   X(K,2) * D(3,2) + X(K,3) * D(3,3) + X(K,4) * D(3,4)
                    WORK(4,2) =                                     X(K,3) * D(4,3) + X(K,4) * D(4,4)
            ...
            ELSE
                ...
                CASE(1)
                    ...
                    X(1:KE,1) = X(1:KE, 1) -       (A(1:KE,K) * WORK(1,1) + A(1:KE,KH) * WORK(1,2))  &
                        &  - SGN * (C(1:KE,K) * WORK(1,3) + C(1:KE,KH) * WORK(1,4))
                ...
                CASE(2)
                    ...
                    WORK(1,3) = X(K,1)  * D(1,1) + X(K,2)  * D(1,2)
                    WORK(1,4) = X(KH,1) * D(1,1) + X(KH,2) * D(1,2)
                    WORK(2,3) = X(K,1)  * D(2,1) + X(K,2)  * D(2,2)
                    WORK(2,4) = X(KH,1) * D(2,1) + X(KH,2) * D(2,2)

                    X(1:KE,1) = X(1:KE, 1) -       (A(1:KE,K) * WORK(1,1) + A(1:KE,KH) * WORK(1,2))  &
                        &  - SGN * (C(1:KE,K) * WORK(1,3) + C(1:KE,KH) * WORK(1,4))
                    X(1:KE,2) = X(1:KE, 2) -       (A(1:KE,K) * WORK(2,1) + A(1:KE,KH) * WORK(2,2))  &
                        &  - SGN * (C(1:KE,K) * WORK(2,3) + C(1:KE,KH) * WORK(2,4))
                ...
                CASE(3)
                    ...
                    WORK(1,3) = X(K,1)  * D(1,1) + X(K,2)  * D(1,2) + X(K,3)  * D(1,3)
                    WORK(1,4) = X(KH,1) * D(1,1) + X(KH,2) * D(1,2) + X(KH,3) * D(1,3)
                    WORK(2,3) = X(K,1)  * D(2,1) + X(K,2)  * D(2,2) + X(K,3)  * D(2,3)
                    WORK(2,4) = X(KH,1) * D(2,1) + X(KH,2) * D(2,2) + X(KH,3) * D(2,3)
                    WORK(3,3) =                    X(K,2)  * D(3,2) + X(K,3)  * D(3,3)
                    WORK(3,4) =                    X(KH,2) * D(3,2) + X(KH,3) * D(3,3)
                    ...
                    X(1:KE,3) = X(1:KE, 3) -       (A(1:KE,K) * WORK(3,1) + A(1:KE,KH) * WORK(3,2))  &
                        &  - SGN * (C(1:KE,K) * WORK(3,3) + C(1:KE,KH) * WORK(3,4))
                ...
                CASE(4)
                    ...
                    WORK(1,3) = X(K,1)  * D(1,1) + X(K,2)  * D(1,2) + X(K,3)  * D(1,3) + X(K,4) * D(1,4)
                    WORK(1,4) = X(KH,1) * D(1,1) + X(KH,2) * D(1,2) + X(KH,3) * D(1,3) + X(KH,4)* D(1,4)
                    WORK(2,3) = X(K,1)  * D(2,1) + X(K,2)  * D(2,2) + X(K,3)  * D(2,3) + X(K,4) * D(2,4)
                    WORK(2,4) = X(KH,1) * D(2,1) + X(KH,2) * D(2,2) + X(KH,3) * D(2,3) + X(KH,4)* D(2,4)
                    WORK(3,3) =                    X(K,2)  * D(3,2) + X(K,3)  * D(3,3) + X(K,4)  * D(3,4)
                    WORK(3,4) =                    X(KH,2) * D(3,2) + X(KH,3) * D(3,3) + X(KH,4) * D(3,4)
                    WORK(4,3) =                                       X(K,3)  * D(4,3) + X(K,4)  * D(4,4)
                    WORK(4,4) =                                       X(KH,3) * D(4,3) + X(KH,4) * D(4,4)
                    ...
                    X(1:KE,4) = X(1:KE, 4) -       (A(1:KE,K) * WORK(4,1) + A(1:KE,KH) * WORK(4,2)) &
                        &  - SGN * (C(1:KE,K) * WORK(4,3) + C(1:KE,KH) * WORK(4,4))
