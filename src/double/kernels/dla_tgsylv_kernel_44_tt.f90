...
!> \brief Solver for a 4x4 generalized Sylvester equation (TRANSA = T, TRANSB = T)
!
!  Definition:
!  ===========
!
!       SUBROUTINE DLA_TGSYLV_KERNEL_44TT (SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, INFO)
...
!> \verbatim
!>
!> DLA_TGSYLV_KERNEL_44TT solves a generalized Sylvester equation of the following form
!>
!>    A**T * X * B**T + SGN * C**T * X * D**T = SCALE * Y                              (1)
!>
        ...
        DO WHILE ( L .GT. 0 )
            ...
            IF ( LE .GT. 0 ) THEN
                ...
                ELSE IF ( KB .EQ. 2 .AND. LB .EQ. 1 ) THEN
                    ...
                    X(K,1:LE)  = X(K,1:LE) -  SGN * ( C(K,K)   * X(K,L) + C(KH,K) * X(KH,L) ) * D(1:LE,L)
                    X(KH,1:LE) = X(KH,1:LE) - SGN * ( C(K,KH)  * X(K,L) + C(KH,KH) * X(KH,L) ) * D(1:LE,L)
