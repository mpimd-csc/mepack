...
!> \brief Solver for a 4x4 generalized Sylvester equation (TRANSA = N, TRANSB = N)
!
!  Definition:
!  ===========
!
!       SUBROUTINE DLA_TGSYLV_KERNEL_44NN (SGN, M, N, A, LDA, B, LDB, C, LDC, D, LDD, X, LDX, SCALE, INFO)
...
!> \verbatim
!>
!> DLA_TGSYLV_KERNEL_44NN solves a generalized Sylvester equation of the following form
!>
!>    A * X * B + SGN * C * X * D = SCALE * Y                              (1)
!>
        ...
        DO WHILE ( L .LE. N )
            ...
            IF ( LH .LT. N ) THEN
                ...
                ELSE IF (KB .EQ. 2 .AND. LB .EQ. 1) THEN
                    ...
                    X(K, LH+1:N) = X(K, LH+1:N) - SGN * (C(K,K) * X(K,L) + C(K,KH) * X(KH,L)) * D(L,LH+1:N)

                    X(KH, LH+1:N) = X(KH, LH+1:N) - (A(KH,K) * X(K,L) + A(KH,KH) * X(KH,L)) * B(L,LH+1:N)
                    X(KH, LH+1:N) = X(KH, LH+1:N) - SGN * (C(KH,K) * X(K,L) + C(KH,KH) * X(KH,L)) * D(L,LH+1:N)
                ELSE
