    ...
    CONTAINS
        ...
        FUNCTION TRLYAP_BLOCKSIZE_2STAGE(M) RESULT(MB)
            ...
            ELSE
                IF ( LUA_LOADED() .EQ. 1 ) THEN
                    LUA = GET_LUA()
                    CALL CSC_LUA_CALL_RETURN_INT(LUA, "trlyap_single_2stage", M, MB, INFO)
                    IF (MB.LT.1 .OR. INFO .NE. 0) THEN
                        MB = 2048
                        WRITE(*,*) "MEPACK_LUA_CONFIG(trlyap_single_2stage) returned a blocksize lower than 1 or failed. Use 2048."
                    END IF
                ELSE
                    MB = 2048
                END IF
        ...
        FUNCTION TRSTEIN_BLOCKSIZE_2STAGE(M) RESULT(MB)
            ...
            ELSE
                IF ( LUA_LOADED() .EQ. 1 ) THEN
                    LUA = GET_LUA()
                    CALL CSC_LUA_CALL_RETURN_INT(LUA, "trstein_single_2stage", M, MB, INFO)
                    IF (MB.LT.1 .OR. INFO .NE. 0) THEN
                        MB = 2048
                        WRITE(*,*) "MEPACK_LUA_CONFIG(trstein_single_2stage) returned a blocksize lower than 1 or failed. Use 2048."
                    END IF
                ELSE
                    MB = 2048
                END IF
        ...
        FUNCTION TGLYAP_BLOCKSIZE_2STAGE(M) RESULT(MB)
            ...
            ELSE
                IF ( LUA_LOADED() .EQ. 1 ) THEN
                    LUA = GET_LUA()
                    CALL CSC_LUA_CALL_RETURN_INT(LUA, "tglyap_single_2stage", M, MB, INFO)
                    IF (MB.LT.1 .OR. INFO .NE. 0) THEN
                        MB = 2048
                        WRITE(*,*) "MEPACK_LUA_CONFIG(tglyap_single_2stage) returned a blocksize lower than 1 or failed. Use 2048."
                    END IF
                ELSE
                    MB = 2048
                END IF
        ...
        FUNCTION TGSTEIN_BLOCKSIZE_2STAGE(M) RESULT(MB)
            ...
            ELSE
                IF ( LUA_LOADED() .EQ. 1 ) THEN
                    LUA = GET_LUA()
                    CALL CSC_LUA_CALL_RETURN_INT(LUA, "tgstein_single_2stage", M, MB, INFO)
                    IF (MB.LT.1 .OR. INFO .NE. 0) THEN
                        MB = 2048
                        WRITE(*,*) "MEPACK_LUA_CONFIG(tgstein_single_2stage) returned a blocksize lower than 1 or failed. Use 2048."
                    END IF
                ELSE
                    MB = 2048
                END IF
        ...
        FUNCTION TRSYLV_BLOCKSIZE_2STAGE(M, N) RESULT(MB)
            ...
            ELSE
                IF ( LUA_LOADED() .EQ. 1 ) THEN
                    LUA = GET_LUA()
                    CALL CSC_LUA_CALL_RETURN_INT(LUA, "trsylv_single_2stage", M, N, MB, INFO)
                    IF (MB.LT.1 .OR. INFO .NE. 0) THEN
                        MB = 2048
                        WRITE(*,*) "MEPACK_LUA_CONFIG(trsylv_single_2stage) returned a blocksize lower than 1 or failed. Use 2048."
                    END IF
                ELSE
                    MB = 2048
                END IF
        ...
        FUNCTION TRSYLV2_BLOCKSIZE_2STAGE(M, N) RESULT(MB)
            ...
            ELSE
                IF ( LUA_LOADED() .EQ. 1 ) THEN
                    LUA = GET_LUA()
                    CALL CSC_LUA_CALL_RETURN_INT(LUA, "trsylv2_single_2stage", M, N, MB, INFO)
                    IF (MB.LT.1 .OR. INFO .NE. 0) THEN
                        MB = 2048
                        WRITE(*,*) "MEPACK_LUA_CONFIG(trsylv2_single_2stage) returned a blocksize lower than 1 or failed. Use 2048."
                    END IF
                ELSE
                    MB = 2048
                END IF
        ...
        FUNCTION TGSYLV_BLOCKSIZE_2STAGE(M, N) RESULT(MB)
            ...
            ELSE
                 IF ( LUA_LOADED() .EQ. 1 ) THEN
                    LUA = GET_LUA()
                    CALL CSC_LUA_CALL_RETURN_INT(LUA, "tgsylv_single_2stage", M, N, MB, INFO)
                    IF (MB.LT.1 .OR. INFO .NE. 0) THEN
                        MB = 2048
                        WRITE(*,*) "MEPACK_LUA_CONFIG(tgsylv_single_2stage) returned a blocksize lower than 1 or failed. Use 2048."
                    END IF
                ELSE
                    MB = 2048
                END IF
        ...
        FUNCTION TGCSYLV_BLOCKSIZE_2STAGE(M,N) RESULT(MB)
            ...
            ELSE
                 IF ( LUA_LOADED() .EQ. 1 ) THEN
                    LUA = GET_LUA()
                    CALL CSC_LUA_CALL_RETURN_INT(LUA, "tgcsylv_single_2stage", M, N, MB, INFO)
                    IF (MB.LT.1 .OR. INFO .NE. 0) THEN
                        MB = 2048
                        WRITE(*,*) "MEPACK_LUA_CONFIG(tgcsylv_single_2stage) returned a blocksize lower than 1 or failed. Use 2048."
                    END IF
                ELSE
                    MB = 2048
                END IF
        ...
        FUNCTION TGCSYLV_DUAL_BLOCKSIZE_2STAGE(M,N) RESULT(MB)
            ...
            ELSE
                 IF ( LUA_LOADED() .EQ. 1 ) THEN
                    LUA = GET_LUA()
                    CALL CSC_LUA_CALL_RETURN_INT(LUA, "tgcsylv_dual_single_2stage", M, N, MB, INFO)
                    IF (MB.LT.1 .OR. INFO .NE. 0) THEN
                        MB = 2048
                        WRITE(*,*) "MEPACK_LUA_CONFIG(tgcsylv_dual_single_2stage) returned a blocksize lower than 1 or failed."
                        WRITE(*,*) "--> Use 2048."
                    END IF
                ELSE
                    MB = 2048
...
END MODULE MEPACK_OPTIONS_BLOCKSIZE_SINGLE
!> @}
