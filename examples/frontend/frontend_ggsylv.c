...
void usage(char *prgmname) {
    int is;
    printf("Solve a generalized Sylvester equation with different solvers.\n");
    printf("\n");
...
optional_argument: "::" */
        ...
        choice = getopt_long( argc, argv, "hm:n:M:N:A:B:t:r:s:S:caR:Ho:",
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("DLA_GGSYLV", reusestr, reusestr, M, N);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    if ( run == -1 || !reuse) {
                        if(hess){
                            mepack_double_ggsylv("H", "H", TRANSA, TRANSB, sign, M, N, A, M, B, N, C, M, D, N, QA, M, ZA, M, QB, N, ZB, N, X, M, &scale, Work, ldwork, &info);
                        } else{
                            mepack_double_ggsylv("N", "N", TRANSA, TRANSB, sign, M, N, A, M, B, N, C, M, D, N, QA, M, ZA, M, QB, N, ZB, N, X, M, &scale, Work, ldwork, &info);
                        }

                    } else {
                        mepack_double_ggsylv("F", "F", TRANSA, TRANSB, sign, M, N, A, M, B, N, C, M, D, N, QA, M, ZA, M, QB, N, ZB, N, X, M, &scale, Work, ldwork, &info);
                    }
                ...
                res += mepack_double_residual_gsylv(TRANSA, TRANSB, sign, M, N, Aorig, M, Borig, N, Corig, M, Dorig, N, X, M, RHS, M, scale);
