...
optional_argument: "::" */
    ...
    if ( reuse == 1 && hess == 1) {
        fprintf(stderr, "Both -R and -H can not be set simultaneously. \n");
        exit(-1);
    }
...
#pragma omp parallel for schedule(static,1)
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    if ( run == -1 || !reuse) {
                        if ( hess ) {
                            mepack_single_ggsylv("H", "H", TRANSA, TRANSB, sign, M, N, A, M, B, N, C, M, D, N, QA, M, ZA, M, QB, N, ZB, N, X, M, &scale, Work, ldwork, &info);
                        } else {
                ...
                res += mepack_single_residual_gsylv(TRANSA, TRANSB, sign, M, N, Aorig, M, Borig, N, Corig, M, Dorig, N, X, M, RHS, M, scale);
