...
void usage(char *prgmname) {
    int is;
    printf("Solve a generalized Stein equation with different solvers.\n");
    printf("\n");
...
optional_argument: "::" */
    ...
    mepack_double_tgstein_blocksize_2stage_set(256);
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("DLA_GGSTEIN", reusestr, reusestr, M, M);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    if ( run == -1 || !reuse) {
                        if(!hess){
                            if ( is == 27) {
                                Int infox = 0;
                                FC_GLOBAL_(sg03ad,SG03AD)("DisCont", "X", "NonFact", TRANSX, "Upper", &M,
                                        A, &M, B, &M, Q, &M, Z, &M, X, &M, &scale, &sep, &ferr,
                                        &Work[0], &Work[M], &Work[2*M], NULL, &Work[3*M], &ldworkx,  &infox, 1, 1, 1, 1, 1);
                            } else {
                                mepack_double_ggstein("N", TRANSA, M, A, M, B, M, Q, M, Z, M, X, M, &scale, Work, ldwork,  &info);
                            }
                        } else{
                            mepack_double_ggstein("H", TRANSA, M, A, M, B, M, Q, M, Z, M, X, M, &scale, Work, ldwork,  &info);
                        }
                    } else {
                        if ( is == 27 ) {
                            Int infox = 0;
                            FC_GLOBAL_(sg03ad,SG03AD)("DisCont", "X", "Fact", TRANSX, "upper", &M,
                                    A, &M, B, &M, Q, &M, Z, &M, X, &M, &scale, &sep, &ferr,
                                    &Work[0], &Work[M], &Work[2*M], NULL, &Work[3*M], &ldworkx,  &infox, 1, 1, 1, 1, 1);


                        } else {
                            mepack_double_ggstein("F", TRANSA, M, A, M, B, M, Q, M, Z, M, X, M, &scale, Work, ldwork,  &info);
                        }
                    }
                ...
                res += mepack_double_residual_gstein(TRANSA, M, Aorig, M, Borig, M, X, M, RHS, M, scale);
