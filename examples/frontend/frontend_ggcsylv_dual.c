...
#pragma omp parallel for schedule(static,1)
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    if ( run == -1 || !reuse) {
                        if(hess){
                            mepack_double_ggcsylv_dual("H", "H", TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N,  QA, M, ZA, M, QB, N, ZB, N, X, M, Y, M,  &scale, Work, ldwork, &info);
                        } else {
                ...
                res += mepack_double_residual_csylv_dual(TRANSA, TRANSB, sign1, sign2, M, N, Aorig, M, Borig, N, Corig, M, Dorig, N, X, M, Y, M, RHS, M, RHS2, M, scale);
