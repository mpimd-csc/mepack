...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("SLA_GESYLV", reusestr, reusestr, M, N);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    if ( run == -1 || !reuse) {
                        if ( !hess ) {
                            mepack_single_gesylv("N", "N", TRANSA, TRANSB, sign,  M, N, A, M, B, N, QA, M, QB, N, X, M, &scale, Work, ldwork,  &info);
                        } else {
                            mepack_single_gesylv("H", "H", TRANSA, TRANSB, sign,  M, N, A, M, B, N, QA, M, QB, N, X, M, &scale, Work, ldwork,  &info);
                        }
                    } else {
                        mepack_single_gesylv("F", "F", TRANSA, TRANSB, sign,  M, N, A, M, B, N, QA, M, QB, N, X, M, &scale, Work, ldwork,  &info);
                    }
                    te2 += (get_ctime()-ts2);
                    te += (get_wtime()-ts);
                ...
                res += mepack_single_residual_sylv(TRANSA, TRANSB, sign, M, N, Aorig, M, Borig, N, X, M, RHS, M, scale);
