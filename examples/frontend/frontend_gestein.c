...
void usage(char *prgmname) {
    int is;
    printf("Solve a standard Stein equation with different solvers.\n");
    printf("\n");
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("DLA_GESTEIN", reusestr, NULL, M, M);

            Work = (double *) malloc(sizeof(double) * (mem + 2 * M));
            ldwork = (Int) mem;
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    if ( run == -1 || !reuse) {
                        if ( !hess ) {
                            ...
                            mepack_double_gestein("H", TRANSA, M, A, M, Q, M, X, M, &scale, Work, ldwork, &info);
                        }
                    } else {
                        if ( is == 27 ) {
                            Int infox = 0;
                ...
                res += mepack_double_residual_stein(TRANSA, M, Aorig, M, X, M, RHS, M, scale);
