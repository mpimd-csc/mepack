...
#pragma omp parallel for schedule(static,1)
            ...
            for (mat = 0; mat < nMAT; mat++) {
                        ...
                        if ( !hess ) {
                            if ( is == 27 ) {
                                ...
                                FC_GLOBAL_(sb03md,SB03MD)("Cont", "X", "NonFact", TRANSX, &M,
                                        ...
                                        &Work[0], &Work[M], NULL, &Work[2*M], &ldworkx,  &infox, 4, 1, 7, 1);
                            } else {
                                mepack_double_gelyap("N", TRANSA, M, A, M, Q, M, X, M, &scale, Work, ldwork, &info);

                            }
                        } else {
                            mepack_double_gelyap("H", TRANSA, M, A, M, Q, M, X, M, &scale, Work, ldwork, &info);
                        }
                    } else {
                        if ( is == 27 ) {
                            Int infox = 0;
                            FC_GLOBAL_(sb03md,SB03MD)("Cont", "X", "Fact", TRANSX, &M,
                                    A, &M, Q, &M, X, &M, &scale, &sep, &ferr_slicot,
                ...
                res += mepack_double_residual_lyap(TRANSA, M, Aorig, M, X, M, RHS, M, scale);
