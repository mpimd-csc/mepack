...
void usage(char *prgmname) {
    int is;
    printf("Solve a generalized Lyapunov equation with iterative refinement in single precision.\n");
    printf("\n");
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("SLA_GGLYAP_REFINE", "N", "N", M, M);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                benchmark_rhs_glyap_float(TRANSA, M, Aorig, M, Borig, M, Xorig, M, RHS, M );
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    ts = get_wtime();
                    ts2 = get_ctime();
                        mepack_single_gglyap_refine(TRANSA, "N", M, Aorig, M, Borig, M, X, M, RHS, M,
                ...
                res += mepack_single_residual_glyap(TRANSA, M, Aorig, M, Borig, M, X, M, RHS, M, 1.0);
