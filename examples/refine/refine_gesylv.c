...
void usage(char *prgmname) {
    int is;
    printf("Solve a standard Sylvester equation with iterative refinement.\n");
    printf("\n");
...
optional_argument: "::" */
    ...
    for (M = M_MIN, N = N_MIN; M <= M_MAX && N<=N_MAX; M = (N>=N_MAX) ? (M+M_STEP): M,  N=(N<N_MAX)?(N+N_STEP):(N_MIN)) {
        ...
        for (MB = MB_MIN, NB = NB_MIN; MB <= MB_MAX && NB<=NB_MAX; MB = (NB>=NB_MAX) ? (MB+MB_STEP): MB,  NB=(NB<NB_MAX)?(NB+NB_STEP):(NB_MIN)) {
            ...
            mepack_trsylv_isolver_set(1);
            mepack_trsylv_frontend_solver_set(is);
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("DLA_GESYLV_REFINE", "N", "N", M, N);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                /* Setup the Problem  */
                Int ldwork = mem;
                benchmark_random_evp_double(M, iseed, A, QA, Aorig, align_on*gcd(MB, NB));
                benchmark_random_evp_double(N, iseed, B, QB, Borig, align_on*gcd(MB, NB));

                benchmark_rhs_sylv_double(TRANSA, TRANSB, sign,  M, N, Aorig, M, Borig, N, Xorig, M, RHS, M );
                ...
                for (run = 0; run < RUNS; run++) {
                    ...
                    mepack_double_gesylv_refine( TRANSA, TRANSB, "N", sign, M, N, Aorig, M, Borig, N, X, M, RHS, M,
                ...
                res += mepack_double_residual_sylv(TRANSA, TRANSB, sign, M, N, Aorig, M, Borig, N, X, M, RHS, M, 1.0);
                ferr += benchmark_check_X_double(M,N,X, M, Xorig, M);
