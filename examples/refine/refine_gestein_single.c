...
void usage(char *prgmname) {
    int is;
    printf("Solve a standard Stein equation with iterative refinement in single precision.\n");
    printf("\n");
...
int main(int argc, char **argv)
{
    ...
    csc_table_comment_printf(tab, "Solution of the Stein Equation");
    ...
    for (M = M_MIN ;  M <= M_MAX ; M = M + M_STEP ) {
        for (MB = MB_MIN; MB <= MB_MAX ; MB += MB_STEP) {
            ...
            mepack_trstein_isolver_set(1);
            mepack_trstein_frontend_solver_set(is);
            /* Prepare  */
            times = 0;
            ctimes = 0;
            ferr = 0.0;
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("SLA_GESTEIN_REFINE", "N", "N", M, M);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                benchmark_rhs_stein_float(TRANSA, M, Aorig, M, Xorig, M, RHS, M );
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    ts = get_wtime();
                    ts2 = get_ctime();
                    mepack_single_gestein_refine(TRANSA,"N", M, Aorig, M, X, M, RHS, M,
                ...
                res += mepack_single_residual_stein(TRANSA, M, Aorig, M, X, M, RHS, M, 1.0);
