...
void usage(char *prgmname) {
    int is;
    printf("Solve a dual generalized coupled Sylvester equation with iterative refinement in single precision.\n");
    printf("\n");
...
optional_argument: "::" */
    ...
    csc_table_comment_printf(tab, "Solution of the Dual Generalized Coupled Sylvester Equation");
    ...
    for (M = M_MIN, N = N_MIN; M <= M_MAX && N<=N_MAX; M = (N>=N_MAX) ? (M+M_STEP): M,  N=(N<N_MAX)?(N+N_STEP):(N_MIN)) {
        ...
        for (MB = MB_MIN, NB = NB_MIN; MB <= MB_MAX && NB<=NB_MAX; MB = (NB>=NB_MAX) ? (MB+MB_STEP): MB,  NB=(NB<NB_MAX)?(NB+NB_STEP):(NB_MIN)) {
            ...
            mepack_tgcsylv_dual_frontend_solver_set(is);
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("SLA_GGCSYLV_DUAL_REFINE", "N", "N", M, N);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                benchmark_rhs_ggcsylv_dual_float(TRANSA, TRANSB, sign1, sign2,  M, N, Aorig, M, Borig, N, Corig, M, Dorig, N, Rorig, M, Lorig, M,  RHS_R, M, RHS_L, M );
                 ...
                 *-----------------------------------------------------------------------------*/
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts = get_wtime();
                        ts2 = get_ctime();
                        mepack_single_ggcsylv_dual_refine(TRANSA, TRANSB, "N", sign1, sign2,  M, N, Aorig, M, Borig, N, Corig, M, Dorig, N,  R, M, L, M, RHS_R, M, RHS_L, M,
                    ...
                    res += mepack_single_residual_csylv_dual(TRANSA, TRANSB, sign1, sign2, M, N, Aorig, M, Borig, N, Corig, M, Dorig, N, R, M, L, M, RHS_R, M, RHS_L, M, 1.0);
