...
void usage(char *prgmname) {
    int is;
    printf("Solve a standard Sylvester equation with iterative refinement in single precision.\n");
    printf("\n");
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("SLA_GESYLV_REFINE", "N", "N", M, N);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                benchmark_rhs_sylv_float(TRANSA, TRANSB, sign,  M, N, Aorig, M, Borig, N, Xorig, M, RHS, M );
                ...
                for (run = 0; run < RUNS; run++) {
                    ...
                    mepack_single_gesylv_refine( TRANSA, TRANSB, "N", sign, M, N, Aorig, M, Borig, N, X, M, RHS, M,
