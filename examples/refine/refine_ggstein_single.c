...
void usage(char *prgmname) {
    int is;
    printf("Solve a generalized Stein equation with iterative refinement in single precision.\n");
    printf("\n");
...
int main(int argc, char **argv)
{
    ...
    for (M = M_MIN ;  M <= M_MAX ; M = M + M_STEP ) {
        for (MB = MB_MIN; MB <= MB_MAX ; MB += MB_STEP) {
            ...
            mepack_tgstein_isolver_set(1);
            mepack_tgstein_frontend_solver_set(is);
            /* Prepare  */
            times = 0;
            ctimes = 0;
            ferr = 0.0;
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("SLA_GGSTEIN_REFINE", "N", "N", M, M);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                /* Setup the Problem  */
                benchmark_random_gevp_float(M, iseed, A, B, Q, Z, Aorig, Borig, align_on* MB);
                benchmark_rhs_gstein_float(TRANSA, M, Aorig, M, Borig, M, Xorig, M, RHS, M );
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    ts = get_wtime();
                    ts2 = get_ctime();
                    mepack_single_ggstein_refine(TRANSA, "N", M, Aorig, M, Borig, M, X, M, RHS, M,
                ...
                res += mepack_single_residual_gstein(TRANSA, M, Aorig, M, Borig, M, X, M, RHS, M, 1.0);
                ferr += benchmark_check_X_float(M,M,X, M, Xorig, M);
