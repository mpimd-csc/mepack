...
void usage(char *prgmname) {
    int is;
    printf("Solve a generalized Stein equation with iterative refinement.\n");
    printf("\n");
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("DLA_GGSTEIN_REFINE", "N", "N", M, M);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                benchmark_rhs_gstein_double(TRANSA, M, Aorig, M, Borig, M, Xorig, M, RHS, M );
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    ts = get_wtime();
                    ts2 = get_ctime();
                    mepack_double_ggstein_refine(TRANSA,"N", M, Aorig, M, Borig, M, X, M, RHS, M,
                ...
                res += mepack_double_residual_gstein(TRANSA, M, Aorig, M, Borig, M, X, M, RHS, M, 1.0);
