...
void usage(char *prgmname) {
    int is;
    printf("Solve a standard Lyapunov equation with iterative refinement.\n");
    printf("\n");
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("DLA_GELYAP_REFINE", "N", "N", M, M);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                benchmark_rhs_lyap_double(TRANSA, M, Aorig, M, Xorig, M, RHS, M );
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    alpha = 0;
                    beta = 0 ;
                    FC_GLOBAL_(dlaset,DLASET)("All", &M, &M, &alpha, &beta, X, &M, 1);


                    ts = get_wtime();
                    ts2 = get_ctime();
                    mepack_double_gelyap_refine(TRANSA, "N", M, Aorig, M, X, M, RHS, M,
