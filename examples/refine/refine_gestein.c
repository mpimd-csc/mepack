...
void usage(char *prgmname) {
    int is;
    printf("Solve a standard Stein equation with iterative refinement.\n");
    printf("\n");
...
#pragma omp parallel for schedule(static,1)
            ...
            mem = mepack_memory_frontend("DLA_GESTEIN_REFINE", "N", "N", M, M);
            ...
            for (mat = 0; mat < nMAT; mat++) {
                /* Setup the Problem  */
                benchmark_random_evp_double(M, iseed, A, Q, Aorig, align_on* MB);
                benchmark_rhs_stein_double(TRANSA, M, Aorig, M, Xorig, M, RHS, M );
                ...
                for (run = -1; run < RUNS; run++) {
                    ...
                    ts = get_wtime();
                    ts2 = get_ctime();
                    mepack_double_gestein_refine(TRANSA, "N", M, Aorig, M, X, M, RHS, M,
                ...
                res += mepack_double_residual_stein(TRANSA, M, Aorig, M, X, M, RHS, M, 1.0);
