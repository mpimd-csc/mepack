...
typedef struct _context_sylv2_t {
    Int M, N;
    Int MB, NB, BIGMB;
    FLOAT sgn;
    FLOAT scale;
...
static void context_sylv2_init(void *_ctx, Int M, Int N, Int ISolver, Int MB, Int NB, Int BIGMB, const char *TA, const char *TB, double sgn1, double sgn2){
    ...
    if ( ISolver < 19 ) {
        if ( ISolver < 7 )  {
            ...
            mepack_trsylv2_isolver_set(ISolver-6);
        } else if ( ISolver > 12 && ISolver < 19 )  {
            mepack_trsylv2_isolver_set(ISolver-12);
        } else {
...
#pragma omp parallel for schedule(static,1)
    ...
    if ( ISolver < 7 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L3), M, N);
    } else if ( ISolver > 6 && ISolver < 13 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_DAG), M, N);
    } else if ( ISolver > 12 && ISolver < 19 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L3_2S), M, N);
    } else if ( ISolver == 19 ) {
        if ( M <=32 && N <= 32)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_LOCAL_COPY_32), M, N);
        else if ( M <=64 && N <= 64)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_LOCAL_COPY_64), M, N);
        else if ( M <=96 && N <= 96)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_LOCAL_COPY_96), M, N);
        else if ( M <=128 && N <= 128)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_LOCAL_COPY_128), M, N);
        else
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_REORDER), M, N);
    } else if ( ISolver == 20 ) {
        if ( M <=128 && N <= 128)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_LOCAL_COPY), M, N);
        else
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_REORDER), M, N);
    } else if ( ISolver == 21)
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_REORDER), M, N);
    else if ( ISolver == 22)
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2), M, N);
    else if ( ISolver == 23)
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_UNOPT), M, N);
    else if ( ISolver == 24)
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_RECURSIVE), M, N);
    else
        ctx->mem = 2*M*N;

    if ( ISolver == 19 || ISolver == 20 ) {
        ctx->mem = MAX(mepack_memory(MEPACK_PRECISION_PREFIX(TRSYLV2_L2_REORDER), M, N), ctx->mem);
    }
...
static void context_sylv2_setup_rhs(void *_ctx){
    context_sylv2_t * ctx = _ctx;
...
static void context_sylv2_setup_problem(void *_ctx, Int iseed[4], Int align, Int changerole)
{
    context_sylv2_t * ctx = _ctx;
    (void) changerole;
#ifdef SINGLE_PRECISION
    ...
    benchmark_rhs_sylv2_float(ctx->TRANSA, ctx->TRANSB, ctx->sgn, ctx->M, ctx->N,
    ...
    benchmark_rhs_sylv2_double(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
            ctx->A, ctx->LDA, ctx->B, ctx->LDB,
            ctx->Xorig, ctx->LDXorig, ctx->RHS_X, ctx->LDRHS_X);
...
static void context_sylv2_iter_prepare(void * _ctx) {
    context_sylv2_t * ctx = _ctx;
#ifdef SINGLE_PRECISION
...
static void context_sylv2_iter_solve(void *_ctx){
    ...
    if ( ctx->isolver < 7 ) {
        ...
        MEPACK_PREFIX(trsylv2_dag)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver < 19 ) {
        MEPACK_PREFIX(trsylv2_level3_2stage)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver == 19 ){
        if ( ctx -> M <= 32 && ctx->N <= 32 )
            MEPACK_PREFIX(trsylv2_level2_local_copy_32)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
        else if ( ctx->M <= 64 && ctx->N <= 64 )
            MEPACK_PREFIX(trsylv2_level2_local_copy_64)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
        else if ( ctx->M <= 96 && ctx->N <= 96 )
            MEPACK_PREFIX(trsylv2_level2_local_copy_96)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
        else if ( ctx->M <= 128 && ctx->N <= 128 )
            MEPACK_PREFIX(trsylv2_level2_local_copy_128)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
        else
            MEPACK_PREFIX(trsylv2_level2_reorder)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver == 20 ) {
        if ( ctx->M <= 128 && ctx->N <= 128) {
            MEPACK_PREFIX(trsylv2_level2_local_copy)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
        } else {
            MEPACK_PREFIX(trsylv2_level2_reorder)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
        ...
        MEPACK_PREFIX(trsylv2_level2)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver == 23 ) {
        MEPACK_PREFIX(trsylv2_level2_unopt)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver == 24){
        MEPACK_PREFIX(trsylv2_recursive)(ctx->TRANSA, ctx->TRANSB, ctx->sgn,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    }
...
static void context_sylv2_check(void *_ctx, double *forward, double *rel)
{
    context_sylv2_t * ctx = _ctx;
    *rel += MEPACK_PREFIX(residual_sylv2)(ctx->TRANSA, ctx->TRANSB, ctx->sgn, ctx->M, ctx->N,
            ctx->A, ctx->LDA, ctx->B, ctx->LDB,
...
static void context_sylv2_clean(void *_ctx)
{
    context_sylv2_t * ctx = _ctx;
...
benchmark_context_t *benchmark_create(void)
{
    ...
    context_sylv2_t * ctx = malloc(sizeof(context_sylv2_t));
    memset(ctx, 0, sizeof(context_sylv2_t));
    bctx->ctx = ctx;
    bctx->init = context_sylv2_init;
    bctx->setup_rhs = context_sylv2_setup_rhs;
    bctx->setup_problem = context_sylv2_setup_problem;
    bctx->iter_prepare = context_sylv2_iter_prepare;
    bctx->iter_solve = context_sylv2_iter_solve;
    bctx->check = context_sylv2_check;
    bctx->clean = context_sylv2_clean;
    return bctx;
}
