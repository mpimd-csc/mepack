...
void usage(char *prgmname) {
    int is;
    printf("Solve a generalized Stein equation with triangular coefficient matrices.\n");
    printf("\n");
...
#pragma omp parallel for schedule(static,1)
            ...
            if ( is < 7 ){
                ...
                mem = mepack_memory("DLA_TGSTEIN_L3", M, M);
            } else if ( is > 6 && is < 13 ){
                mepack_tgstein_isolver_set(is-6);
                mem = mepack_memory("DLA_TGSTEIN_DAG", M, M);
            } else if ( is > 12 && is < 19 ){
                mepack_tgstein_isolver_set(is-12);
                mem = mepack_memory("DLA_TGSTEIN_L3_2S", M, M);
            } else if ( is == 19)
                mem = mepack_memory("DLA_TGSTEIN_L2", M, M);
            else if ( is == 24)
                mem = mepack_memory("DLA_TGSTEIN_RECURSIVE", M, M);
            else
                mem = 2*M*M;
               ...
               /*-----------------------------------------------------------------------------
                ...
                if ( is == 28 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts2 = get_ctime();
                        if ( tolower(TRANSA[0]) == 'n') {
                            FC_GLOBAL_(bkdis,BKDIS)(&M, A, &M, B, &M, X, &M, &Job, &infox);
                        }
