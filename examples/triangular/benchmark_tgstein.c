...
static void context_stein_iter_solve(void *_ctx){
    ...
    if ( ctx->isolver < 7 ) {
        ...
        FC_GLOBAL_(sg03ax,SG03AX)(TRANSX, &ctx->M, ctx->A, &ctx->LDA, ctx->B, &ctx->LDB, ctx->X, &ctx->LDX, &ctx->scale, &infox, 1);
    }
#endif
        ...
        Int infox = 0;
        if ( tolower(ctx->TRANSA[0]) == 'n') {
            FC_GLOBAL_(bkdis,BKDIS)(&ctx->M, ctx->A, &ctx->LDA, ctx->B, &ctx->LDB, ctx->X, &ctx->LDX, &Job, &infox);
        }
