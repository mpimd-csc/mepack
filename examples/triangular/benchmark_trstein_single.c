...
void usage(char *prgmname) {
    int is;
    printf("Solve a standard Stein equation with triangular coefficient matrices in single precision.\n");
    printf("\n");
...
#pragma omp parallel for schedule(static,1)
            ...
            if ( is < 7 ){
                ...
                mem = mepack_memory("SLA_TRSTEIN_L3", M, 0);
            } else if ( is > 6 && is < 13 ){
                mepack_trstein_isolver_set(is-6);
                mem = mepack_memory("SLA_TRSTEIN_DAG", M, 0);
            } else if ( is > 12 && is < 19 ){
                mepack_trstein_isolver_set(is-12);
                mem = mepack_memory("SLA_TRSTEIN_L3_2S", M, 0);
            } else if ( is == 19)
                mem = mepack_memory("SLA_TRSTEIN_L2", M, 0);
            else if ( is == 24)
                mem = mepack_memory("SLA_TRSTEIN_RECURSIVE", M, 0);
            else
                mem = 2*M*M;
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                if ( is < 19 ) {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        if ( is < 7 )
                            mepack_single_trstein_level3(TRANSA, M, A, M, X, M, &scale, Work, &info);
                        else if ( is < 12 )
                            mepack_single_trstein_dag(TRANSA, M, A, M, X, M, &scale, Work, &info);
                        else if ( is < 19 )
                            mepack_single_trstein_level3_2stage(TRANSA, M, A, M, X, M, &scale, Work, &info);
                ...
                if ( is > 18 && is < 25 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts2 = get_ctime();
                        if ( is == 19 ){
                            mepack_single_trstein_level2(TRANSA, M, A, M, X, M, &scale, Work, &info);
                        } else if ( is == 20 || is == 21 || is == 22 || is == 23 ) {
                            FC_GLOBAL_(slacpy,SLACPY)("All", &M, &M, Xorig, &M, X, &M);
                        } else if ( is == 24 ) {
                            mepack_single_trstein_recursive(TRANSA, M, A, M, X, M, &scale, Work, &info);
                        }
                        if (run >= 0 ) {
                            te2 += (get_ctime()-ts2);
                            te += (get_wtime()-ts);
    ...
    benchmark_exit();
    return (ress < 2.0*sqrt(eps)*TOL_SINGLE)? 0 : -1 ;
}
