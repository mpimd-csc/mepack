...
void usage(char *prgmname) {
    int is;
    printf("Solve a standard Lyapunov equation with triangular coefficient matrices in single precision.\n");
    printf("\n");
...
optional_argument: "::" */
    ...
    printf("# Solver: "); solver_name(is);
     mepack_single_trlyap_blocksize_2stage_set(256);
...
#pragma omp parallel for schedule(static,1)
            ...
            if ( is < 7 ) {
                ...
                mem = mepack_memory("SLA_TRLYAP_L3", M, 0);
            } else if ( is > 6 && is < 13 )  {
                mepack_trlyap_isolver_set(is-6);
                mem = mepack_memory("SLA_TRLYAP_DAG", M, 0);
            } else if ( is > 12 && is < 19 )  {
                mepack_trlyap_isolver_set(is-12);
                mem = mepack_memory("SLA_TRLYAP_L3_2S", M, 0);
            } else if ( is == 19 ) {
                mepack_trlyap_isolver_set(1);
                mem = mepack_memory("SLA_TRLYAP_L2_OPT", M, 0);
            } else if ( is == 20 ) {
                mepack_trlyap_isolver_set(1);
                mem = mepack_memory("SLA_TRLYAP_L2", M, 0);
            } else if ( is == 21 ) {
                mepack_trlyap_isolver_set(1);
                mem = mepack_memory("SLA_TRLYAP_L2", M, 0);
            } else if ( is == 24 ) {
                mepack_trlyap_isolver_set(1);
                mem = mepack_memory("SLA_TRLYAP_RECURSIVE", M, 0);
            } else {
                mem = 3 * M * M;
            }
            ...
            for (mat = 0; mat < nMAT; mat++) {
                /* Setup the Problem  */
                benchmark_random_evp_float(M, iseed, A, NULL, NULL, align_on* MB);
                benchmark_rhs_lyap_float(TRANSA, M, A, M, Xorig, M, RHS, M );
                ...
                if ( is < 19 ) {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        if ( is < 7 )
                            mepack_single_trlyap_level3(TRANSA, M, A, M, X, M, &scale, Work, &info);
                        else if ( is < 12 )
                            mepack_single_trlyap_dag(TRANSA, M, A, M, X, M, &scale, Work, &info);
                        else if ( is < 19 )
                            mepack_single_trlyap_level3_2stage(TRANSA, M, A, M, X, M, &scale, Work, &info);
                ...
                if ( is > 18 && is < 25 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        if ( is == 19 ){
                            mepack_single_trlyap_level2_opt(TRANSA, M, A, M, X, M, &scale, Work, &info);
                        } else if ( is == 20 || is == 21 ) {
                            mepack_single_trlyap_level2(TRANSA, M, A, M, X, M, &scale, Work, &info);
                        } else if ( is == 22 || is ==23) {
                            FC_GLOBAL_(slacpy,SLACPY)("All", &M, &M, Xorig, &M, X, &M, 1);
                        } else if ( is == 24 ) {
                            mepack_single_trlyap_recursive(TRANSA, M, A, M, X, M, &scale, Work, &info);
                        }
                        if ( run >= 0 ) {
                            te2 += (get_ctime()-ts2);
                            te += (get_wtime()-ts);
            ...
            printf("%5d %3d  %10.5e  %10.5e  %10.5e  %10.5f \n",
