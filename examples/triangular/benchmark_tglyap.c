...
static void context_lyap_init(void *_ctx, Int M, Int N, Int ISolver, Int MB, Int NB, Int BIGMB, const char *TA, const char *TB, double sgn1, double sgn2){
    ...
    MEPACK_PREFIX(tglyap_blocksize_2stage_set)(BIGMB);
    MEPACK_PREFIX(tglyap_blocksize_set)(MB);

    if ( ISolver < 19 ) {
        if ( ISolver < 7 )  {
            mepack_tglyap_isolver_set(ISolver);
        } else if ( ISolver > 6 && ISolver < 13 )  {
            mepack_tglyap_isolver_set(ISolver-6);
        } else if ( ISolver > 12 && ISolver < 19 )  {
            mepack_tglyap_isolver_set(ISolver-12);
        } else {
...
#pragma omp parallel for schedule(static,1)
    ...
    if ( ISolver < 7 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGLYAP_L3), M, M);
    } else if ( ISolver > 6 && ISolver < 13 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGLYAP_DAG), M, M);
    } else if ( ISolver > 12 && ISolver < 19 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGLYAP_L3_2S), M, M);
    } else if ( ISolver == 19 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGLYAP_L2), M, M);
    } else if ( ISolver == 20 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGLYAP_L2), M, M);
    } else if ( ISolver == 21)
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGLYAP_L2), M, M);
        ...
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGLYAP_RECURSIVE), M, M);
    else
        ctx->mem = 3*M*M;
...
static void context_lyap_setup_problem(void *_ctx, Int iseed[4], Int align, Int changerole)
{
    ...
    benchmark_rhs_glyap_float(ctx->TRANSA, ctx->M,
    ...
    benchmark_rhs_glyap_double(ctx->TRANSA, ctx->M,
            ctx->A, ctx->LDA, ctx->B, ctx->LDB,
            ctx->Xorig, ctx->LDXorig, ctx->RHS_X, ctx->LDRHS_X);
...
static void context_lyap_iter_solve(void *_ctx){
    ...
    if ( ctx->isolver < 7 ) {
        ...
        MEPACK_PREFIX(tglyap_dag)(ctx->TRANSA, ctx->M,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver < 19 ) {
        MEPACK_PREFIX(tglyap_level3_2stage)(ctx->TRANSA, ctx->M,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver == 19 ){
            MEPACK_PREFIX(tglyap_level2)(ctx->TRANSA,  ctx->M,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver == 20 ) {
            MEPACK_PREFIX(tglyap_level2)(ctx->TRANSA, ctx->M,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver == 24){
        MEPACK_PREFIX(tglyap_recursive)(ctx->TRANSA, ctx->M,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->X, ctx->LDX,  &ctx->scale, ctx->work, &info);
    }
...
{
    context_lyap_t * ctx = _ctx;
    *rel += MEPACK_PREFIX(residual_glyap)(ctx->TRANSA, ctx->M,
            ctx->A, ctx->LDA, ctx->B, ctx->LDB,
...
const char *equation_name(void) {
    return "Triangular Generalized Lyapunov Equation";
}
