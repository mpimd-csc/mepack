...
void usage(char *prgmname) {
    int is;
    printf("Solve a generalized Lyapunov equation with triangular coefficient matrices.\n");
    printf("\n");
...
#pragma omp parallel for schedule(static,1)
            ...
            if ( is < 7 ){
                ...
                mem = mepack_memory("DLA_TGLYAP_L3", M, M);
            } else if ( is > 6 && is < 13 ){
                mepack_tglyap_isolver_set(is-6);
                mem = mepack_memory("DLA_TGLYAP_DAG", M, M);
            } else if ( is > 12 && is < 19 ){
                mepack_tglyap_isolver_set(is-12);
                mem = mepack_memory("DLA_TGLYAP_L3_2S", M, M);
            } else if ( is == 19)
                mem = mepack_memory("DLA_TGLYAP_L2", M, M);
            else if ( is == 20)
                mem = mepack_memory("DLA_TGLYAP_L2_UNOPT", M, M);
            else if ( is == 24)
                mem = mepack_memory("DLA_TGLYAP_RECURSIVE", M, M);
            else
                mem = 2*M*M;
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                if ( is < 19 ) {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts2 = get_ctime();
                        if ( is < 7 )
                            mepack_double_tglyap_level3(TRANSA,  M, A, M, B, M, X, M, &scale, Work, &info);
                        else if ( is < 12 )
                            mepack_double_tglyap_dag(TRANSA,  M, A, M, B, M, X, M, &scale, Work, &info);
                        else if ( is < 19 )
                            mepack_double_tglyap_level3_2stage(TRANSA,  M, A, M, B, M, X, M, &scale, Work, &info);
                ...
                if ( is > 18 && is < 25 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts2 = get_ctime();
                        if ( is == 19 ) {
                            mepack_double_tglyap_level2(TRANSA,  M, A, M, B, M, X, M, &scale, Work, &info);
                        } else if  ( is == 20 ) {
                            mepack_double_tglyap_level2_unopt(TRANSA,  M, A, M, B, M, X, M, &scale, Work, &info);
                        } else if ( is == 21 || is == 22 || is == 23 ) {
                            FC_GLOBAL_(dlacpy,DLACPY)("All", &M, &M, Xorig, &M, X, &M, 1);
                        } else {
                            mepack_double_tglyap_recursive(TRANSA,  M, A, M, B, M, X, M, &scale, Work, &info);
                        }
                        if (run >= 0 ) {
                            te2 += (get_ctime()-ts2);
                            te += (get_wtime()-ts);
                ...
                if ( is == 25 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts = get_wtime();
                        ts2 = get_ctime();
                        FC_GLOBAL_(recglyct,RECGLYCT)(&type, &scale, &M, A, &M, B, &M,  X,  &M, &infox, MACHINE_RECSY, Work, &N2);
                ...
                if ( is == 26 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts = get_wtime();
                        ts2 = get_ctime();
                        FC_GLOBAL_(recglyct_p,RECGLYCT_P)(&proc, &type, &scale, &M, A, &M, B, &M,  X,  &M, &infox, MACHINE_RECSY, Work, &N2);
                ...
                if ( is == 27 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        if ( tolower(TRANSA[0]) == 'n') {
                            FC_GLOBAL_(sg03ay,SG03AY)("T", &M, A, &M, B, &M,  X,  &M, &scale,  &infox, 1);
                        } else {
                            FC_GLOBAL_(sg03ay,SG03AY)("N", &M, A, &M, B, &M,  X,  &M, &scale,  &infox, 1);
                        }
                ...
                if ( is == 28 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts2 = get_ctime();
                        if ( tolower(TRANSA[0]) == 'n') {
                            FC_GLOBAL_(bkcon,BKCON)(&M, A, &M, B, &M, X, &M, &Job, &infox);
                        }
