...
void usage(char *prgmname) {
    int is;
    printf("Solve a standard Sylvester equation with triangular coefficient matrices in single precision.\n");
    printf("\n");
...
optional_argument: "::" */
    ...
    mepack_single_trsylv_blocksize_2stage_set(256);
    ...
    for (M = M_MIN, N = N_MIN; M <= M_MAX && N<=N_MAX; M = (N>=N_MAX) ? (M+M_STEP): M,  N=(N<N_MAX)?(N+N_STEP):(N_MIN)) {
        ...
        for (MB = MB_MIN, NB = NB_MIN; MB <= MB_MAX && NB<=NB_MAX; MB = (NB>=NB_MAX) ? (MB+MB_STEP): MB,  NB=(NB<NB_MAX)?(NB+NB_STEP):(NB_MIN)) {
            ...
            /* Set the Block size   */
            mepack_single_trsylv_blocksize_mb_set(MB);
...
#pragma omp parallel for schedule(static,1)
            ...
            if ( is < 7 ){
                ...
                mem = mepack_memory("SLA_TRSYLV_L3", M, N);
            } else if ( is > 6 && is < 13 ){
                mepack_trsylv_isolver_set(is-6);
                mem = mepack_memory("SLA_TRSYLV_DAG", M, N);
            } else if ( is > 12 && is < 19 ){
                mepack_trsylv_isolver_set(is-12);
                mem = mepack_memory("SLA_TRSYLV_L3_2S", M, N);
            } else if ( is == 19 ) {
                if ( M <= 32 && N <= 32 )
                    mem = mepack_memory("SLA_TRSYLV_L2_LOCAL_COPY_32", M, N);
                else if ( M <= 64 && N <= 64 )
                    mem = mepack_memory("SLA_TRSYLV_L2_LOCAL_COPY_64", M, N);
                else if ( M <= 96 && N <= 96 )
                    mem = mepack_memory("SLA_TRSYLV_L2_LOCAL_COPY_96", M, N);
                else if ( M <= 128 && N <= 128 )
                    mem = mepack_memory("SLA_TRSYLV_L2_LOCAL_COPY_128", M, N);
                else
                    mem = mepack_memory("SLA_TRSYLV_L2_REORDER", M, N);
            }
            else if ( is == 20 ){
                if ( M <= 128 && N <= 128) {
                    mem = mepack_memory("SLA_TRSYLV_L2_LOCAL_COPY", M, N);
                } else {
                    mem = mepack_memory("SLA_TRSYLV_L2_REORDER", M, N);
                }
            }
                ...
                mem = mepack_memory("SLA_TRSYLV_L2", M, N);
            else if ( is == 23)
                mem = mepack_memory("SLA_TRSYLV_L2_UNOPT", M, N);
            else if ( is == 24)
                mem = mepack_memory("SLA_TRSYLV_RECURSIVE", M, N);
            else
                mem = 2*M*N;

            if ( is == 19 || is == 20 ) {
                mem = MAX(mepack_memory("SLA_TRSYLV_L2_REORDER", M, N), mem);
            }
            ...
            for (mat = 0; mat < nMAT; mat++) {
                /* Setup the Problem  */
                benchmark_random_evp_float(M, iseed, A, NULL, NULL, align_on*gcd(MB, NB));
                benchmark_random_evp_float(N, iseed, B, NULL, NULL, align_on*gcd(MB, NB));
                benchmark_rhs_sylv_float(TRANSA, TRANSB, sign,  M, N, A, M, B, N, Xorig, M, RHS, M );
                ...
                if ( is < 19 ) {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts2 = get_ctime();
                        if ( is < 7 )
                            mepack_single_trsylv_level3(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                        else if ( is < 12 )
                            mepack_single_trsylv_dag(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                        else if ( is < 19 )
                            mepack_single_trsylv_level3_2stage(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                ...
                if ( is > 18 && is < 25 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        if ( is == 19 ){
                            if ( M <= 32 && N <= 32 )
                                mepack_single_trsylv_level2_local_copy_32(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                            else if ( M <= 64 && N <= 64 )
                                mepack_single_trsylv_level2_local_copy_64(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                            else if ( M <= 96 && N <= 96 )
                                mepack_single_trsylv_level2_local_copy_96(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                            else if ( M <= 128 && N <= 128 )
                                mepack_single_trsylv_level2_local_copy_128(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                            else
                                mepack_single_trsylv_level2_reorder(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                        } else if ( is == 20 ) {
                            if ( M <= 128 && N <= 128) {
                                mepack_single_trsylv_level2_local_copy(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                            } else {
                                mepack_single_trsylv_level2_reorder(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                            }
                        } else if ( is == 21 ) {
                            ...
                            mepack_single_trsylv_level2(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                        } else if ( is == 23 ) {
                            mepack_single_trsylv_level2_unopt(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                        } else {
                            mepack_single_trsylv_recursive(TRANSA, TRANSB, sign,  M, N, A, M, B, N, X, M, &scale, Work, &info);
                        }
                        if (run >= 0 ) {
                            te2 += (get_ctime()-ts2);
                            te += (get_wtime()-ts);
                ...
                if ( is == 25 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        {
                            Int infox = info;
                            FC_GLOBAL_(strsyl,STRSYL)(TRANSA, TRANSB, &isgn, &M, &N, A, &M, B, &N,X,  &M, &scale, &infox);
                            info = infox;
                        }
                        if (run >= 0 ) {
                            te2 += (get_ctime()-ts2);
