...
#pragma omp parallel for schedule(static,1)
    ...
    if ( ISolver < 7 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L3), M, N);
    } else if ( ISolver > 6 && ISolver < 13 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_DAG), M, N);
    } else if ( ISolver > 12 && ISolver < 19 ) {
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L3_2S), M, N);
    } else if ( ISolver == 19 ) {
        if ( M <=32 && N <= 32)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_LOCAL_COPY_32), M, N);
        else if ( M <=64 && N <= 64)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_LOCAL_COPY_64), M, N);
        else if ( M <=96 && N <= 96)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_LOCAL_COPY_96), M, N);
        else if ( M <=128 && N <= 128)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_LOCAL_COPY_128), M, N);
        else
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_REORDER), M, N);
    } else if ( ISolver == 20 ) {
        if ( M <=128 && N <= 128)
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_LOCAL_COPY), M, N);
        else
            ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_REORDER), M, N);
    } else if ( ISolver == 21)
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_REORDER), M, N);
    else if ( ISolver == 22)
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2), M, N);
    else if ( ISolver == 23)
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_UNOPT), M, N);
    else if ( ISolver == 24)
        ctx->mem = mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_RECURSIVE), M, N);
    else
        ctx->mem = 2*M*N;

    if ( ISolver == 19 || ISolver == 20 ) {
        ctx->mem = MAX(mepack_memory(MEPACK_PRECISION_PREFIX(TGCSYLV_DUAL_L2_REORDER), M, N), ctx->mem);
    }
...
static void context_csylv_setup_problem(void *_ctx, Int iseed[4], Int align, Int changerole)
{
    ...
    benchmark_rhs_ggcsylv_dual_float(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
    ...
    benchmark_rhs_ggcsylv_dual_double(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
            ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD,
            ctx->Rorig, ctx->LDRorig, ctx->Lorig, ctx->LDLorig, ctx->RHS_R, ctx->LDRHS_R, ctx->RHS_L, ctx->LDRHS_L);
#endif
}
...
static void context_csylv_iter_solve(void *_ctx){
    ...
    if ( ctx->isolver < 7 ) {
        ...
        MEPACK_PREFIX(tgcsylv_dual_dag)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver < 19 ) {
        MEPACK_PREFIX(tgcsylv_dual_level3_2stage)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver == 19 ){
        if ( ctx -> M <= 32 && ctx->N <= 32 )
            MEPACK_PREFIX(tgcsylv_dual_level2_local_copy_32)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
        else if ( ctx->M <= 64 && ctx->N <= 64 )
            MEPACK_PREFIX(tgcsylv_dual_level2_local_copy_64)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
        else if ( ctx->M <= 96 && ctx->N <= 96 )
            MEPACK_PREFIX(tgcsylv_dual_level2_local_copy_96)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
        else if ( ctx->M <= 128 && ctx->N <= 128 )
            MEPACK_PREFIX(tgcsylv_dual_level2_local_copy_128)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
        else
            MEPACK_PREFIX(tgcsylv_dual_level2)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
    } else if ( ctx->isolver == 20 ) {
        if ( ctx->M <= 128 && ctx->N <= 128) {
            MEPACK_PREFIX(tgcsylv_dual_level2_local_copy)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
        } else {
            MEPACK_PREFIX(tgcsylv_dual_level2)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
        ...
        MEPACK_PREFIX(tgcsylv_dual_recursive)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2,  ctx->M, ctx->N,
                ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD, ctx->R, ctx->LDR, ctx->L, ctx->LDL, &ctx->scale, ctx->work, &info);
    }
}
...
{
    context_csylv_t * ctx = _ctx;
    *rel += MEPACK_PREFIX(residual_csylv_dual)(ctx->TRANSA, ctx->TRANSB, ctx->sgn1, ctx->sgn2, ctx->M, ctx->N,
            ctx->A, ctx->LDA, ctx->B, ctx->LDB, ctx->C, ctx->LDC, ctx->D, ctx->LDD,
...
const char *equation_name(void) {
    return "Triangular Coupled Sylvester Equation (DUAL)";
}

int have_solver(int is){
    if ( is > 0 && is <=24){
        if(is == 22 || is == 23 ) return 0;
        return 1;
    }
    return 0;
