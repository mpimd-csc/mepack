...
void usage(char *prgmname) {
    int is;
    printf("Solve a dual generalized coupled Sylvester equation with triangular coefficient matrices in single precision.\n");
    printf("\n");
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                if ( is < 19 ) {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts2 = get_ctime();
                        if ( is < 7 ) {
                            mepack_single_tgcsylv_dual_level3(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        } else if ( is < 12 ) {
                            mepack_single_tgcsylv_dual_dag(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        } else if ( is < 19 ) {
                            mepack_single_tgcsylv_dual_level3_2stage(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        }
                        if (run >= 0 ) {
                            te2 += (get_ctime()-ts2);
                            te += (get_wtime()-ts);
                ...
                if ( is > 18 && is < 25 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        if ( is == 19 ){
                            if ( M <= 32 && N <= 32 )
                                mepack_single_tgcsylv_dual_level2_local_copy_32(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            else if ( M <= 64 && N <= 64 )
                                mepack_single_tgcsylv_dual_level2_local_copy_64(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            else if ( M <= 96 && N <= 96 )
                                mepack_single_tgcsylv_dual_level2_local_copy_96(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            else if ( M <= 128 && N <= 128 )
                                mepack_single_tgcsylv_dual_level2_local_copy_128(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            else
                                mepack_single_tgcsylv_dual_level2(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        } else if ( is == 20 ) {
                            if ( M <= 128 && N <= 128) {
                                mepack_single_tgcsylv_dual_level2_local_copy(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            } else {
                                mepack_single_tgcsylv_dual_level2(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            }
                        } else if ( is == 21 ) {
                            ...
                            mepack_single_tgcsylv_dual_recursive(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        }
