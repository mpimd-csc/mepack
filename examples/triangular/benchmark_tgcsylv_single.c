...
void usage(char *prgmname) {
    int is;
    printf("Solve a generalized coupled Sylvester equation with triangular coefficient matrices in single precision.\n");
    printf("\n");
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        choice = getopt_long( argc, argv, "hm:n:M:N:A:B:t:r:s:S:a",
    ...
    mepack_single_tgcsylv_blocksize_2stage_set(256);
...
#pragma omp parallel for schedule(static,1)
            ...
            if ( is < 7 ){
                ...
                mem = mepack_memory("SLA_TGCSYLV_L3", M, N);
            } else if ( is > 6 && is < 13 ){
                mepack_tgcsylv_isolver_set(is-6);
                mem = mepack_memory("SLA_TGCSYLV_DAG", M, N);
            } else if ( is > 12 && is < 19 ){
                mepack_tgcsylv_isolver_set(is-12);
                mem = mepack_memory("SLA_TGCSYLV_L3_2S", M, N);
            } else if ( is == 19 ) {
                if ( M <=32 && N <= 32)
                    mem = mepack_memory("SLA_TGCSYLV_L2_LOCAL_COPY_32", M, N);
                else if ( M <=64 && N <= 64)
                    mem = mepack_memory("SLA_TGCSYLV_L2_LOCAL_COPY_64", M, N);
                else if ( M <=96 && N <= 96)
                    mem = mepack_memory("SLA_TGCSYLV_L2_LOCAL_COPY_96", M, N);
                else if ( M <=128 && N <= 128)
                    mem = mepack_memory("SLA_TGCSYLV_L2_LOCAL_COPY_128", M, N);
                else
                    mem = mepack_memory("SLA_TGCSYLV_L2_REORDER", M, N);
            } else if ( is == 20 ) {
                if ( M <=128 && N <= 128)
                    mem = mepack_memory("SLA_TGCSYLV_L2_LOCAL_COPY", M, N);
                else
                    mem = mepack_memory("SLA_TGCSYLV_L2_REORDER", M, N);
            } else if ( is == 21)
                mem = mepack_memory("SLA_TGCSYLV_L2_REORDER", M, N);
            else if ( is == 22)
                mem = mepack_memory("SLA_TGCSYLV_L2", M, N);
            else if ( is == 23)
                mem = mepack_memory("SLA_TGCSYLV_L2_UNOPT", M, N);
            else if ( is == 24)
                mem = mepack_memory("SLA_TGCSYLV_RECURSIVE", M, N);
            else
                mem = 2*M*N;

            if ( is == 19 || is == 20 ) {
                mem = MAX(mepack_memory("SLA_TGCSYLV_L2_REORDER", M, N), mem);
            }
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                benchmark_rhs_ggcsylv_float(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, Rorig, M, Lorig, M,  RHS_R, M, RHS_L, M );
                ...
                if ( is < 19 ) {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        ts2 = get_ctime();
                        if ( is < 7 )
                            mepack_single_tgcsylv_level3(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        else if ( is < 12 )
                            mepack_single_tgcsylv_dag(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        else if ( is < 19 )
                            mepack_single_tgcsylv_level3_2stage(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                ...
                if ( is > 18 && is < 25 )  {
                    ...
                    for (run = -1; run < RUNS; run++) {
                        ...
                        if ( is == 19 ){
                            if ( M <= 32 && N <= 32 )
                                mepack_single_tgcsylv_level2_local_copy_32(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            else if ( M <= 64 && N <= 64 )
                                mepack_single_tgcsylv_level2_local_copy_64(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            else if ( M <= 96 && N <= 96 )
                                mepack_single_tgcsylv_level2_local_copy_96(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            else if ( M <= 128 && N <= 128 )
                                mepack_single_tgcsylv_level2_local_copy_128(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            else
                                mepack_single_tgcsylv_level2_reorder(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        } else if ( is == 20 ) {
                            if ( M <= 128 && N <= 128) {
                                mepack_single_tgcsylv_level2_local_copy(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            } else {
                                mepack_single_tgcsylv_level2_reorder(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                            }
                        } else if ( is == 21 ) {
                            ...
                            mepack_single_tgcsylv_level2(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        } else if ( is == 23 ) {
                            mepack_single_tgcsylv_level2_unopt(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        } else {
                            mepack_single_tgcsylv_recursive(TRANSA, TRANSB, sign1, sign2,  M, N, A, M, B, N, C, M, D, N, R, M, L, M,  &scale, Work, &info);
                        }
                        if (run >= 0 ) {
                            te2 += (get_ctime()-ts2);
                            te += (get_wtime()-ts);
