...
void usage(char * prgmname) {
    printf("Benchmark the different level 2 inside the level 3 solver for the standard Sylvester equation.\n");
    printf("\n");
...
int main(int argc, char **argv)
{
    ...
    for (M = M_MIN, N = N_MIN; M <= M_MAX && N<=N_MAX; M = (N>=N_MAX) ? (M+M_STEP): M,  N=(N<N_MAX)?(N+N_STEP):(N_MIN)) {
        ...
        for (MB = MB_MIN, NB = NB_MIN; MB <= MB_MAX && NB<=NB_MAX; MB = (NB>=NB_MAX) ? (MB+MB_STEP): MB,  NB=(NB<NB_MAX)?(NB+NB_STEP):(NB_MIN)) {
            ...
            /* Set the Block size   */
            if ( getenv("MEPACK_LUA_CONFIG")) {
                mepack_double_trsylv_blocksize_mb_set(0);
                mepack_double_trsylv_blocksize_nb_set(0);
            }
            else
            {
...
#pragma omp parallel for schedule(static,1)
            ...
            for (mat = 0; mat < nMAT; mat++) {
                /* Setup the Problem  */
                benchmark_random_evp_double(M, iseed, A, NULL, NULL, /* gcd(MB, NB)*/ 4);
                benchmark_random_evp_double(N, iseed, B, NULL, NULL, /* gcd(MB, NB)*/ 4);
                ...
                ress[0] += mepack_double_residual_sylv(TRANSA, TRANSB, sign, M, N, A, M, B, N, X, M, RHS, M, scale);
                ...
                for (is = 1; is < 9; is++) {
                    ...
                    ress[is] += mepack_double_residual_sylv(TRANSA, TRANSB, sign, M, N, A, M, B, N, X, M, RHS, M, scale);
                }
            }
            for (i = 0; i < 9; i++) {
                times[i] /= (double) nMAT;
