...
void usage(char * prgmname) {
    printf("Benchmark the different level 2 inside the level 3 solver for the generalized Stein equation.\n");
    printf("\n");
    ...
    printf("--trans=N,T              Transpose on the matrix pair (A, B). \n");
...
int main(int argc, char **argv)
{
    ...
    for (M = M_MIN ;  M <= M_MAX ; M = M + M_STEP ) {
        for (MB = MB_MIN; MB <= MB_MAX ; MB += MB_STEP) {
            ...
            /* Set the Block size   */
            if ( getenv("MEPACK_LUA_CONFIG")) {
                mepack_double_tgstein_blocksize_set(0);
            }
            else
            {
                mepack_double_tgstein_blocksize_set(MB);
            }
...
#pragma omp parallel for schedule(static,1)
            ...
            for (mat = 0; mat < nMAT; mat++) {
                benchmark_random_gevp_double(M, iseed, A, B, NULL, NULL, NULL, NULL, MB);
                benchmark_rhs_gstein_double (TRANSA,  M, A, M, B, M, Xorig, M, RHS, M );
                ...
                for (run = 0; run < RUNS; run++) {
                    ...
                    ts = get_wtime();
                    FC_GLOBAL_(recglydt,RECGLYDT)(&type, &scale, &M, A, &M, B, &M, X, &M, &infox, MACHINE_RECSY, Work, &j);
                    for (j = 0; j < M; j++) {
                        for (i = 0; i < j; i++) {
                            X[i*M+j] = X[j*M+i];
                        }
                ...
                te = te / RUNS;
                times[0] += te;
                ress[0] += mepack_double_residual_gstein(TRANSA, M, A, M, B, M, X, M, RHS, M, scale);
                ...
                for (is = 1; is < 9; is++) {
                    ...
                    for (run = 0; run < RUNS; run++) {
                        ...
                        if ( is < 7 ) {
                            ts = get_wtime();
                            mepack_double_tgstein_level3(TRANSA,  M, A, M, B, M, X, M, &scale, Work, &info);
                            te += (get_wtime()-ts);
                        } else if ( is == 7 ) {
                            mepack_tgstein_isolver_set(1);
                            ts = get_wtime();
                            mepack_double_tgstein_dag(TRANSA,  M, A, M, B, M, X, M, &scale, Work, &info);
                            te += (get_wtime()-ts);
                        } else if ( is == 8 ) {
                            mepack_tgstein_isolver_set(1);
                            ts = get_wtime();
                            mepack_double_tgstein_level3_2stage(TRANSA,  M, A, M, B, M, X, M, &scale, Work, &info);
                            te += (get_wtime()-ts);
                        }
                    }
                    te = te / RUNS;
                    ...
                    ress[is] += mepack_double_residual_gstein(TRANSA, M, A, M, B, M, X, M, RHS, M, scale);
                }
