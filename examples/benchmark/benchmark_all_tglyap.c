...
void usage(char *prgmname) {
    printf("Benchmark the level 2 and level3 solvers for the generalized Lyapunov equation.\n");
    printf("\n");
...
int main(int argc, char **argv)
{
    ...
    for (M = M_MIN ;  M <= M_MAX ; M = M + M_STEP ) {
        for (MB = MB_MIN; MB <= MB_MAX ; MB += MB_STEP) {
            ...
            /* Set the Block size   */
            if ( getenv("MEPACK_LUA_CONFIG")) {
                mepack_double_tglyap_blocksize_set(0);
            }
            else
            {
...
#pragma omp parallel for schedule(static,1)
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                for (run = 0; run < RUNS; run++) {
                    ...
                    ts = get_wtime();
                    FC_GLOBAL_(recglyct,RECGLYCT)(&type, &scale, &M, A, &M, B, &M, X, &M, &infox, MACHINE_RECSY, Work, &j);
                    for (j = 0; j < M; j++) {
                        for (i = 0; i < j; i++) {
                            X[i*M+j] = X[j*M+i];
                        }
