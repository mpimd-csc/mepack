...
#pragma omp parallel for schedule(static,1)
            ...
            for (mat = 0; mat < nMAT; mat++) {
                ...
                ress[0] += mepack_double_residual_csylv(TRANSA, TRANSB, sign1, sign2, M, N, A, M, B, N, C, M, D, N, R, M, L, M, RHS_R, M, RHS_L, M, scale);
#else
                times[0] = 0;
                ress[0] = 0;
#endif
                ...
                for (is = 1; is < 9; is++) {
                    ...
                    te = te / RUNS;
                    times[is] += te;
                    ress[is] += mepack_double_residual_csylv(TRANSA, TRANSB, sign1, sign2, M, N, A, M, B, N, C, M, D, N, R, M, L, M, RHS_R, M, RHS_L, M, scale);
                }
            }
            for (i = 0; i < 9; i++) {
                times[i] /= (double) nMAT;
